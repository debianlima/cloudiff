...
### L059 — polling terminal precisa assentar o estado visual após reconexão transitória
Em 2026-09-10, o hardness E2E real criou um projeto descartável e observou o backend de provisionamento chegar a `status=succeeded`, enquanto o modal permanecia com o cabeçalho **Confirmando provisionamento / reconectando ao provisionador** até reload. O corpo já era redesenhado por `drawLive(d)`, mas o estado lateral do wizard (`headTitle`, `headText`, botão de fechar e flag `provisioning`) não era assentado depois que uma tentativa transitória de polling finalmente retornava terminal. Regra: todo poll assíncrono que entra em modo de reconexão precisa de uma transição terminal explícita e idempotente que restaure todos os controles/labels do diálogo; renderizar somente o corpo terminal não é reconciliação visual completa. Gate: teste vermelho→verde em `test_project_provisioning_live_wizard`, mirrors current/legacy equivalentes e E2E pré-patch como evidência causal.
