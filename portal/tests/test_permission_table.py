...
# Rotas reais da v1 fora do inventario canonico (secao 2). Sao legitimas e
# tem decisao observada; entram no match para nao ficarem sem paridade.
for _r in OBSERVED.get("routes_outside_requirements_inventory", []):
    BY_KEY.setdefault((_r["path"], _r["method"]), _r)
...
class PermissionTableTest(unittest.TestCase):
    ...
    def test_no_route_lost(self):
        ...
        inventory = {(r["path"], r["method"]) for r in OBSERVED["routes_inventory_31"]}
