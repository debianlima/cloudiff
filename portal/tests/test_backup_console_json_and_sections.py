...
class BackupConsoleJsonAndSectionsTests(unittest.TestCase):
 ...
 def test_role_aware_backup_sections_are_explicit(self):
  for title in ('Backup da plataforma','Backup dos projetos e bancos vinculados','Backup global de todos os bancos'):
   self.assertIn(title,self.source)
  self.assertIn("${d.platform?`",self.source)
  self.assertIn("${d.tenants?`",self.source)
 def test_tenant_archives_are_in_inventory(self):
  ...
  self.assertIn('payload["tenants"] = {"items": _tenant_backup_items()',self.source)
