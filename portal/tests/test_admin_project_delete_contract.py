...
class AdminProjectDeleteContractTest(unittest.TestCase):
    ...
    def test_delete_module_preserves_tenant_and_requires_confirmation(self):
        ...
        self.assertIn("unicodedata.normalize('NFKC'", source)
        self.assertIn("unicodedata.category(ch) != 'Cf'", source)
        self.assertIn("return normalize(confirmation) == normalize(expected)", source)
