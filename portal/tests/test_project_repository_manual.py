...
class ProjectRepositoryManualTests(unittest.TestCase):
 ...
 def test_manual_explains_root_source_and_external_infrastructure(self):
  ...
  for marker in ("A raiz deste repositório é a raiz da aplicação","`index.php` ou `index.html`","`api/server.js`","gerados pela CloudIFF fora do Git","Cada versão `d1`, `d2`, `d3`"):
 ...
 def test_template_version_forces_layout_upgrade_for_new_projects(self):
  source=Path("components/control-plane/usr/local/sbin/cloudif-project-template-apply.py").read_text();self.assertIn("old.get('version') == 10",source);self.assertIn("'version': 10",source)
