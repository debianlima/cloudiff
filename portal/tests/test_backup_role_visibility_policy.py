...
class BackupRoleVisibilityPolicyTests(unittest.TestCase):
 ...
 def test_professor_has_all_project_backups(self):
  ...
  self.assertIn("if not (user.get('admin') or _pb_is_professor(user))",self.base)
 ...
 def test_platform_download_remains_admin_only(self):
  self.assertIn("if not user.get('admin')",self.base)
  self.assertIn('if not user.get("admin") or filename',self.ui)
