...
class KomodoForceRebuildTests(unittest.TestCase):
    ...
    def test_deploy_full_supports_explicit_local_rebuild(self):
        for marker in ('_cloudif_v132_force_local_rebuild','force_rebuild = bool','compose_args = ["docker", "compose"','build_cmd = compose_args + ["build"]','--force-recreate','force_rebuild_failed'):
