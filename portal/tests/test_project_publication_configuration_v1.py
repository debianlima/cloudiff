...
class ProjectPublicationConfigurationV1Tests(unittest.TestCase):
    ...
    def test_wizard_and_base_controls_are_in_project_publication_ui(self):
        ...
        self.assertIn('Abrir base no Komodo',ui);self.assertIn('data-publication-environments',ui);self.assertIn('Ambientes de publicação',ui);self.assertNotIn('data-environments-overview',ui);self.assertNotIn('>Gerenciar variáveis</button>',ui)
