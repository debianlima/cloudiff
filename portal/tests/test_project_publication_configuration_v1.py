...
class ProjectPublicationConfigurationV1Tests(unittest.TestCase):
    ...
    def test_wizard_and_base_controls_are_in_project_publication_ui(self):
        ...
        self.assertIn('Abrir base no Komodo',ui);self.assertIn('/cloudiff/portal/publication/base/',ui);self.assertNotIn('name=\"op\" value=\"open_base_workspace\"',ui);self.assertIn('data-publication-environments',ui);self.assertIn('Ambientes de publicação',ui);self.assertNotIn('data-environments-overview',ui);self.assertNotIn('>Gerenciar variáveis</button>',ui)
        ...
        self.assertIn('href=\"/cloudiff/portal/publication/base/{h(slug)}\"',final_renderer)
        self.assertNotIn('name=\"op\" value=\"open_base_workspace\"',final_renderer)
        self.assertIn('data-publication-environments data-project-slug=\"{h(slug)}\"',final_renderer)
        ...
        self.assertIn("data-env-environment=\"'+envEsc(name)+'\"",portal)
