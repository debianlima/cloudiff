...
class ProjectPublicationConfigurationV1Tests(unittest.TestCase):
    ...
    def test_wizard_and_base_controls_are_in_project_publication_ui(self):
        ...
        self.assertIn('data-environments-overview',portal)
        self.assertIn("function envOverviewDraw(root,data)",portal)
        ...
        self.assertIn('.project-environments-shell{',design)
        self.assertIn('.environment-overview-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr))',design)
        self.assertIn('.environment-card__actions{',design)
