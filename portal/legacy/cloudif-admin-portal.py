...
.cloudif-wizard{position:fixed!important;inset:0!important;z-index:1400!important;display:none;place-items:center;padding:24px;background:rgba(8,15,11,.62);overflow:auto}
.cloudif-wizard>.wizard-box,.cloudif-wizard>.card{width:min(860px,100%);max-height:92vh;overflow:auto;margin:auto!important;padding:22px;border-radius:18px;background:var(--c-surface,#fff);box-shadow:0 28px 80px rgba(0,0,0,.32)}
.cloudif-wizard .wizard-head{position:sticky;top:0;z-index:2;display:flex;justify-content:space-between;gap:18px;align-items:flex-start;padding-bottom:14px;background:var(--c-surface,#fff)}
.cloudif-wizard .wizard-close{border:0;background:transparent;font-size:1.7rem;line-height:1;cursor:pointer}
.pm-new-steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;padding:0;margin:18px 0;list-style:none}.pm-new-steps li{padding:10px;border-radius:10px;background:var(--c-surface-2,#f1f5f9);font-size:.78rem;font-weight:700}
.pm-new-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.pm-new-section{display:grid;gap:10px;padding:16px;border:1px solid var(--c-border,#dce3ed);border-radius:12px}.pm-new-section h4{margin:0}.pm-new-section.full{grid-column:1/-1}.pm-runtime-options{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.pm-runtime-options label{display:flex;gap:8px;align-items:flex-start;padding:10px;border:1px solid var(--c-border,#dce3ed);border-radius:10px}.pm-provision-list{display:grid;gap:8px;margin:0;padding:0;list-style:none}.pm-provision-list li{display:flex;gap:8px;align-items:flex-start}.pm-provision-list li:before{content:'✓';font-weight:900;color:#176b35}
body.cloudif-modal-open{overflow:hidden}
@media(max-width:760px){.pm-new-grid,.pm-runtime-options,.pm-new-steps{grid-template-columns:1fr}.cloudif-wizard{padding:10px}.cloudif-wizard>.wizard-box,.cloudif-wizard>.card{max-height:96vh;padding:16px}}
...
<div id="pm197_new" class="wizard cloudif-wizard" role="dialog" aria-modal="true" aria-labelledby="pm197_new_title"><div class="wizard-box"><div class="wizard-head"><div><h3 id="pm197_new_title">Novo projeto</h3><p class="small">Criação integrada de Forgejo, Komodo, tenant, publicação inicial e ACL do proprietário.</p></div><button class="wizard-close" type="button" onclick="cloudifHideWizard('pm197_new')" aria-label="Fechar">×</button></div><ol class="pm-new-steps"><li>1. Projeto</li><li>2. Banco</li><li>3. Tecnologia</li><li>4. Provisionamento</li></ol><form method="post" action="{url('/action/project_action')}"><input type="hidden" name="csrf_token" value="{h(csrf_token)}"><input type="hidden" name="action" value="create_project"><input type="hidden" name="create_repo" value="1"><input type="hidden" name="setup_komodo" value="1"><div class="pm-new-grid"><section class="pm-new-section"><h4>Projeto</h4><label>Nome<input name="name" required placeholder="Sistema de Biblioteca"></label><label>Descrição<textarea name="description" rows="3"></textarea></label></section><section class="pm-new-section"><h4>Banco / Tenant</h4><label><input type="radio" name="db_mode" value="link" checked> Vincular tenant existente</label><select name="tenant">{tenant_opts}</select><label><input type="radio" name="db_mode" value="create"> Criar novo tenant a partir do nome</label><label><input type="radio" name="db_mode" value="skip"> Projeto sem banco</label></section><section class="pm-new-section full"><h4>Tecnologia web</h4><div class="pm-runtime-options"><label><input type="radio" name="runtime_template" value="static-nginx" checked><span><strong>Site estático + Nginx</strong><br><small>HTML, CSS e JavaScript.</small></span></label><label><input type="radio" name="runtime_template" value="node20"><span><strong>Node.js 20</strong><br><small>Express e runtime LTS.</small></span></label><label><input type="radio" name="runtime_template" value="node22"><span><strong>Node.js 22</strong><br><small>Express e runtime LTS atual.</small></span></label><label><input type="radio" name="runtime_template" value="node24"><span><strong>Node.js 24</strong><br><small>Runtime homologado mais recente.</small></span></label><label><input type="radio" name="runtime_template" value="php83-apache"><span><strong>PHP 8.3 + Apache</strong><br><small>Aplicação PHP tradicional.</small></span></label></div></section><section class="pm-new-section full"><h4>Integrações automáticas</h4><ul class="pm-provision-list"><li>repositório Forgejo e webhooks;</li><li>stack e terminal no Komodo;</li><li>tenant Supabase quando selecionado;</li><li>ACL do proprietário e reconciliação de permissões;</li><li>template de tecnologia e publicação inicial.</li></ul></section></div><div class="cm-actions"><button class="btn" type="submit">Criar e provisionar projeto</button><button class="btn gray" type="button" onclick="cloudifHideWizard('pm197_new')">Cancelar</button></div></form></div></div>{''.join(wizards)}'''
