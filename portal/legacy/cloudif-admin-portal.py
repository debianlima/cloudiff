...
def render_projects(user):
    ...
    for p in rows:
        ...
        cards.append((_project_effective_owner(p),f"""
<div class="project-card" data-project-owner="{h(_project_effective_owner(p))}">
  <div class="project-line">
    <div>
      <h3>{h(p['name'])}</h3>
...
<div id="{acl_id}" class="wizard-panel cloudif-wizard">
  <div class="card">
    ...
    owner_groups={}
    for owner,markup in cards:
        key=owner or 'Sem usuário vinculado'
        owner_groups.setdefault(key,[]).append(markup)
    ...
    for owner in ordered_owners:
        ...
        grouped_projects.append(
            ...
            f'<div class="project-owner-group-body">{"".join(items)}</div></details>'
        )
...
(function(){
 ...
 const projectCards=list?[...list.querySelectorAll('.project-owner-group-body > .project-card')]:[];
