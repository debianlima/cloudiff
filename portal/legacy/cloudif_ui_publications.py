...
def _project_information(context):
    ...
    return (
        ...
        f'<div class="publication-info-card publication-runtime-card"><span>PHP</span><strong>Configuração do PHP</strong><small>Versão, módulos e parâmetros ativos</small><a class="publication-runtime-text" href="/cloudiff/portal/action/project-runtime-info?slug={h(context.get("slug") or "")}&amp;kind=php" data-runtime-kind="php" data-runtime-slug="{h(context.get("slug") or "")}">Ver informações do PHP</a></div>'
        f'<div class="publication-info-card publication-runtime-card"><span>Node.js</span><strong>Runtime do Node.js</strong><small>Versões, npm e dependências da API</small><a class="publication-runtime-text" href="/cloudiff/portal/action/project-runtime-info?slug={h(context.get("slug") or "")}&amp;kind=node" data-runtime-kind="node" data-runtime-slug="{h(context.get("slug") or "")}">Ver informações do Node.js</a></div>'
...
def publication_panel(slug, framework_hint=''):
    ...
    runtime_script = '<script>\n(()=>{\n  const root=document.currentScript.closest(\'.publication-manager-resource\');\n  const modal=root&&root.querySelector(\'[data-runtime-modal]\');\n  const body=modal&&modal.querySelector(\'[data-runtime-body]\');\n  const title=modal&&modal.querySelector(\'#runtime-modal-title\');\n  const subtitle=modal&&modal.querySelector(\'[data-runtime-subtitle]\');\n  if(!root||!modal)return;\n  const runtimeLinks=[...root.querySelectorAll(\'[data-runtime-kind]\')];\n  document.body.appendChild(modal);\n  const esc=value=>{const node=document.createElement(\'div\');node.textContent=String(value??\'\');return node.innerHTML};\n  const close=()=>{modal.hidden=true;document.body.classList.remove(\'runtime-modal-open\')};\n  modal.querySelectorAll(\'[data-runtime-close]\').forEach(button=>button.onclick=close);\n  modal.onclick=event=>{if(event.target===modal)close()};\n  document.addEventListener(\'keydown\',event=>{if(event.key===\'Escape\'&&!modal.hidden)close()});\n  async function openRuntime(kind,slug){\n    modal.hidden=false;\n    document.body.classList.add(\'runtime-modal-open\');\n    title.textContent=kind===\'php\'?\'Informações do PHP\':\'Informações do Node.js\';\n    subtitle.textContent=slug;\n    body.innerHTML=\'<div class="runtime-modal-loading">Consultando o container…</div>\';\n    try{\n      const response=await fetch(\'/cloudiff/portal/api/project-runtime-info?slug=\'+encodeURIComponent(slug)+\'&kind=\'+encodeURIComponent(kind),{credentials:\'same-origin\'});\n      const data=await response.json();\n      if(!response.ok||!data.ok)throw new Error(data.error||\'runtime_info_failed\');\n      body.innerHTML=\'<div class="runtime-modal-meta"><div><span>Projeto</span><strong>\'+esc(slug)+\'</strong></div><div><span>Container</span><strong>\'+esc(data.container||\'—\')+\'</strong></div></div><pre>\'+esc(data.output||\'\')+\'</pre>\';\n    }catch(error){\n      body.innerHTML=\'<div class="runtime-modal-error"><strong>Não foi possível consultar o runtime.</strong><span>\'+esc(error.message)+\'</span></div>\';\n    }\n  }\n  runtimeLinks.forEach(link=>link.addEventListener(\'click\',event=>{event.preventDefault();openRuntime(link.dataset.runtimeKind,link.dataset.runtimeSlug)}));\n  const query=new URLSearchParams(location.search),auto=query.get(\'runtime_info\');\n  if(auto===\'php\'||auto===\'node\')openRuntime(auto,query.get(\'project\')||\'__SLUG__\');\n})();\n</script></div>\n'.replace('__SLUG__', h(slug))
    return (
        '<div class="cm-resource publication-manager-resource">'+information+job_html+alias_html+
        ...
        '<div class="runtime-modal-backdrop" data-runtime-modal hidden><section class="runtime-modal" role="dialog" aria-modal="true" aria-labelledby="runtime-modal-title"><header><div><span data-runtime-eyebrow>Diagnóstico autenticado</span><h2 id="runtime-modal-title">Informações do runtime</h2><p data-runtime-subtitle></p></div><button type="button" class="runtime-modal-close" data-runtime-close aria-label="Fechar">×</button></header><div class="runtime-modal-body" data-runtime-body><div class="runtime-modal-loading">Carregando informações…</div></div><footer><button type="button" class="btn light" data-runtime-close>Fechar</button></footer></section></div>'
        + runtime_script
    )
