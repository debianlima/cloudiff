...
<section class="project-final__section"><h3>Ações do projeto</h3><p class="project-final__meta"><strong>Checar projeto</strong> verifica repositório, banco e vínculo do container sem alterar a configuração. <strong>Permissões</strong> controla quem pode acessar este projeto.</p><div class="project-final__actions"><form method="post" action="{url('/action/project_action')}"><input type="hidden" name="csrf_token" value="{h(csrf_token)}"><input type="hidden" name="slug" value="{h(slug)}"><button class="btn gray" name="op" value="check">Checar projeto</button></form>{resume_action}<button class="btn light" type="button" onclick="cloudifShowWizard('{acl_id}')">Gerenciar permissões</button>{delete_action}</div></section>
 ...
 function stateFor(data,index){{const step=data.current_step||'queued',status=data.status||'queued',components=data.components||{{}};if(status==='succeeded')return'done';if(index===0)return'done';if(index===1)return components.forgejo?.ok?'done':status==='failed'&&step==='provision'?'failed':step==='provision'?'running':'pending';if(index===2)return components.komodo?.ok?'done':status==='failed'&&components.forgejo?.ok?'failed':components.forgejo?.ok&&step==='provision'?'running':'pending';if(index===3)return !data.tenant?'done':components.supabase?.ok?'done':status==='failed'&&components.komodo?.ok?'failed':components.komodo?.ok&&step==='provision'?'running':'pending';if(status==='failed'){{const failIndex={{queued:0,'onboarding-reconcile':4,'backup-configuration':5,template:6,'initial-publication':7}}[step]??index;return index<failIndex?'done':index===failIndex?'failed':'pending'}}if(index===4)return ['onboarding-reconcile','backup-configuration','template','initial-publication','complete'].includes(step)?(step==='onboarding-reconcile'?'running':'done'):'pending';if(index===5)return ['backup-configuration','template','initial-publication','complete'].includes(step)?(step==='backup-configuration'?'running':'done'):'pending';if(index===6)return ['template','initial-publication','complete'].includes(step)?(step==='template'?'running':'done'):'pending';if(index===7)return ['initial-publication','complete'].includes(step)?(step==='initial-publication'?'running':'done'):'pending';if(index===8)return status==='succeeded'?'done':'pending';return'pending'}}
