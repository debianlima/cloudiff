...
if 'Portal' in globals() and not globals().get('_rd_wrapped'):
    ...
    def _rd_get(self):
        ...
        if path in ('/cloudiff/portal/action/project-runtime-info','/cloudif/portal/action/project-runtime-info','/action/project-runtime-info'):
            ...
            if not p or kind not in ('php','node'):return self.send_error(404)
            try:
                ...
                body=f'<section class="runtime-info-page"><div class="runtime-info-head"><div><span>Diagnóstico autenticado</span><h2>{h(title)}</h2><p>{h(slug)} · container {h(info.get("container") or "")}</p></div><a class="btn light" href="/cloudiff/portal/?tab=publicacao&amp;project={h(slug)}">Voltar à publicação</a></div><pre>{h(output)}</pre></section>'
                return self.send_html(page(user,'publicacao',body),200)
            except Exception as exc:
                body=f'<section class="card runtime-info-error"><span class="pill bad">Informação indisponível</span><h2>Não foi possível consultar o runtime</h2><p class="small">{h(str(exc)[:220])}</p><a class="btn" href="/cloudiff/portal/?tab=publicacao&amp;project={h(slug)}">Voltar à publicação</a></section>'
                return self.send_html(page(user,'publicacao',body),503)
        if path in ('/cloudiff/portal/action/open-project-terminal','/cloudif/portal/action/open-project-terminal','/action/open-project-terminal'):
            ...
            try:
                ensured=_rd_agent('/komodo/project/terminal/ensure',{'project':slug,'stack_id':p['stack_id'],'service':p['service'],'terminal':'cloudif-'+slug,'shell':'sh'},timeout=45)
