...
def _project_action(request: Request) -> Response:
    ...
    fields = {k: request.form.get(k) for k in ("name", "description", "repo_url", "komodo_status")}
    result = service.project_action(request.identity, slug, op, fields)
...
def _create_project(request: Request) -> Response:
    result = service.create_project(
        ...
        request.form.get("tenant") or "",
        request.form.get("description") or "")
    if result.get("redirect"):
        return Response.redirect(result["redirect"])
    status = result.get("status") or (200 if result.get("ok") else 400)
    return Response.json_body(json.dumps(result, ensure_ascii=False).encode(), status)
...
def _page(request: Request) -> Response:
    return Response.html(views.unavailable("projects"))
...
def endpoints() -> tuple[Endpoint, ...]:
    return (
        ...
        Endpoint("/action/create_project", "POST", "project.manage",
                 authenticated, _create_project, MODULE, csrf=True, origin=True),
