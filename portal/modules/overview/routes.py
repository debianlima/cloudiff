...
def _page(request: Request) -> Response:
    ...
    nav = sorted({e.module for e in all_endpoints() if e.method == "GET" and e.guard(request.identity)})
    if not is_global(request.identity):
        nav = [module for module in nav if module != "admin"]
