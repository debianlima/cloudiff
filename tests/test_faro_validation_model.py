...
first=json.load(open(files[0]));d={x['name']:x for x in first['decisions']};assert d['role']['status']=='fixed' and d['role']['allowed_values']==['edge'];assert d['capabilities']['status']=='derived' and 'agent-auto-update' in d['capabilities']['allowed_values'];assert 'portal-host' in d['capabilities']['allowed_values'];assert d['network_address']['status']=='fixed' and d['network_address']['allowed_values']==['10.62.91.5'];assert d['form_factor']['status']=='fixed'
...
print('FARO_VALIDATION_MODEL=PASS v2 verified=6 partial=0 R14=verified R19=verified')
