...
assert x['identity']['address']=='10.62.91.5';uuid.UUID(x['identity']['nodeId']);assert x['nats']['username']==x['identity']['nodeId']
...
assert '10.62.91.5/32' in x['nats']['sourceAllowlist'];assert x['nats']['externalIngress'].startswith('systemd socket proxy');assert x['verification']['nodeOperational'] is False and x['verification']['sshAvailable'] is False
...
assert dec['network_address']['status']=='fixed' and dec['network_address']['allowed_values']==['10.62.91.5'];assert dec['role']['status']=='unresolved' and dec['capabilities']['status']=='unresolved'
