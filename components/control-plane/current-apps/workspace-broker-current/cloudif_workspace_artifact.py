...
def _root(workroot: str) -> Path:
    return Path(workroot) / '.artifacts'


def _dir(workroot: str, artifact_id: str) -> Path:
    return _root(workroot) / artifact_id


def _meta_path(workroot: str, artifact_id: str) -> Path:
    return _dir(workroot, artifact_id) / 'metadata.json'


def _payload_path(workroot: str, artifact_id: str) -> Path:
    return _dir(workroot, artifact_id) / 'payload.bin'
...
def _load(workroot: str, artifact_id: str) -> dict[str, Any]:
    if not ARTIFACT_RE.fullmatch(str(artifact_id or '')):
        raise ArtifactError('invalid_artifact_id', 'O artifact_id é inválido.', 'artifact_id')
    path = _meta_path(workroot, artifact_id)
    ...
    if int(data.get('expires_at') or 0) <= now:
        cleanup_artifact(workroot, artifact_id)
        raise ArtifactError('artifact_expired', 'O artefato expirou. Faça o upload novamente.', 'artifact_id')
    return data


def cleanup_artifact(workroot: str, artifact_id: str) -> None:
    import shutil
    path = _dir(workroot, artifact_id)
    if path.exists() and not path.is_symlink():
        shutil.rmtree(path, ignore_errors=True)


def clean_expired_artifacts(workroot: str) -> int:
    root = _root(workroot)
    ...
    for path in root.iterdir():
        ...
        if int(data.get('expires_at') or 0) <= now:
            cleanup_artifact(workroot, path.name); removed += 1
    return removed


def start_artifact(workroot: str, project_slug: str, filename: str, expected_size: int, expected_sha256: str, ttl_seconds: int = DEFAULT_TTL) -> dict[str, Any]:
    ...
    clean_expired_artifacts(workroot)
    artifact_id = 'art_' + secrets.token_hex(12)
    root = _root(workroot); root.mkdir(parents=True, exist_ok=True); os.chmod(root, 0o700)
    directory = _dir(workroot, artifact_id); directory.mkdir(mode=0o700)
    payload = _payload_path(workroot, artifact_id)
    ...
    _write_json_atomic(_meta_path(workroot, artifact_id), data)
    return public_metadata(data)


def _append_chunk_unlocked(workroot: str, project_slug: str, artifact_id: str, chunk_index: int, content_base64: str, chunk_sha256: str) -> dict[str, Any]:
    data = _load(workroot, artifact_id)
...
def append_chunk(workroot: str, project_slug: str, artifact_id: str, chunk_index: int, content_base64: str, chunk_sha256: str) -> dict[str, Any]:
    with _artifact_lock(artifact_id):
        return _append_chunk_unlocked(workroot, project_slug, artifact_id, chunk_index, content_base64, chunk_sha256)


def _complete_artifact_unlocked(workroot: str, project_slug: str, artifact_id: str) -> dict[str, Any]:
    data = _load(workroot, artifact_id)
    if data.get('project_slug') != project_slug: raise ArtifactError('artifact_project_mismatch', 'O artefato pertence a outro projeto.', 'artifact_id')
...
def complete_artifact(workroot: str, project_slug: str, artifact_id: str) -> dict[str, Any]:
    with _artifact_lock(artifact_id):
        return _complete_artifact_unlocked(workroot, project_slug, artifact_id)
...
def _resolve_artifact_unlocked(workroot: str, project_slug: str, artifact_id: str, *, expected_sha256: str = '', expected_size: int | None = None, hold_until: int = 0) -> dict[str, Any]:
    data = _load(workroot, artifact_id)
    if data.get('project_slug') != project_slug: raise ArtifactError('artifact_project_mismatch', 'O artefato pertence a outro projeto.', 'artifact_id')
    ...
    if hold_until:
        ...
        data['updated_at'] = int(time.time()); _write_json_atomic(_meta_path(workroot, artifact_id), data)
    result = public_metadata(data); result['payload_path'] = str(_payload_path(workroot, artifact_id)); return result



def resolve_artifact(workroot: str, project_slug: str, artifact_id: str, *, expected_sha256: str = '', expected_size: int | None = None, hold_until: int = 0) -> dict[str, Any]:
    with _artifact_lock(artifact_id):
        return _resolve_artifact_unlocked(workroot, project_slug, artifact_id, expected_sha256=expected_sha256, expected_size=expected_size, hold_until=hold_until)


def read_artifact(workroot: str, project_slug: str, artifact_id: str, expected_sha256: str = '', expected_size: int | None = None) -> tuple[dict[str, Any], bytes]:
    data = resolve_artifact(workroot, project_slug, artifact_id, expected_sha256=expected_sha256, expected_size=expected_size)
