...
def init():
 ...
 x.execute("create unique index if not exists idx_approval_active on approvals(project_slug,action,requested_by) where status in ('pending','pending_second','approved','reserved')")
 ...
 def do_POST(self):
  ...
  try:
   ...
   if p=='/v1/approvals':
    ...
    privileged=requester_role in {'admin','professor'}
    status='approved' if production and self_authorize and privileged and not dual else 'pending';mode='dual_admin_or_professor' if dual else ('single_privileged_requester' if status=='approved' else ('single_admin_or_professor' if production else 'standard_single_decider'))
    aid='apr_'+uuid.uuid4().hex[:20];approved_by=user if status=='approved' else None;approved_at=now if status=='approved' else None;approver_role=requester_role if status=='approved' else None
    x.execute('insert into approvals(approval_id,project_slug,action,requested_by,approved_by,status,reason,created_at,expires_at,approved_at,trace_id,metadata_json,requester_role,approver_role,authorization_mode,two_approvers_required) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(aid,slug,action,user,approved_by,status,str(d.get('reason')or''),now,now+ttl,approved_at,str(d.get('trace_id')or''),json.dumps(d.get('metadata')or{},separators=(',',':')),requester_role,approver_role,mode,1 if dual else 0));x.commit();x.close();self.out(201,{'ok':True,'approval_id':aid,'status':status,'expires_at':now+ttl,'authorization_mode':mode,'two_approvers_required':dual});return
   if p.endswith('/approve'):
    aid=p.split('/')[-2];approver=str(d.get('approved_by')or'').strip();approver_role=str(d.get('approver_role') or '').strip().lower();assert approver
    ...
    if int(r['two_approvers_required'] or 0)==1:
     ...
     if r['status']=='pending':
      x.execute("update approvals set status='pending_second',approved_by=?,approved_at=?,approver_role=? where approval_id=? and status='pending'",(approver,now,approver_role,aid));x.commit();x.close();self.out(200,{'ok':True,'approval_id':aid,'status':'pending_second','first_approved_by':approver,'two_approvers_required':True});return
     if r['status']=='pending_second':
      if approver==r['approved_by']:raise ValueError('distinct_second_approver_required')
      x.execute("update approvals set status='approved',second_approved_by=?,second_approved_at=?,second_approver_role=? where approval_id=? and status='pending_second'",(approver,now,approver_role,aid));x.commit();x.close();self.out(200,{'ok':True,'approval_id':aid,'status':'approved','two_approvers_required':True});return
     raise ValueError('not_pending_or_expired')
    if r['status']!='pending':raise ValueError('not_pending_or_expired')
    x.execute("update approvals set status='approved',approved_by=?,approved_at=?,approver_role=?,authorization_mode=case when action like 'deployment.production%' then 'single_admin_or_professor' else coalesce(authorization_mode,'standard_single_decider') end where approval_id=?",(approver,now,approver_role,aid));x.commit();x.close();self.out(200,{'ok':True,'approval_id':aid,'status':'approved'});return
