...
try:
 ...
 standardized_ui='publication-projects-clean' in portal and 'publication_panel' in portal and 'publication-flow' not in portal and 'production-live' not in portal
 publish_action='publish_version' in Path('/srv/cloudif/lib/cloudif_ui_publications.py').read_text()
 ok=prod.get('ok') is True and cfg.get('public_production_active') is True and cfg.get('public_production_current_release_id')==3 and cfg.get('real_target_external_code')==200 and standardized_ui and publish_action and 'rollback_production' in main and tenant_ok and prof_ok and cfg.get('production_effects_enabled') is False and cfg.get('activation_allowed') is False
 checks.append({'name':'production-public-button-links-admin-scope','ok':ok,'public_traffic':prod.get('ok'),'public_url':'https://cloudiff.duckdns.org/production/atalhos-cloudif-iff1860746/','current_release_id':cfg.get('public_production_current_release_id'),'standardized_publication_ui':standardized_ui,'standard_publish_action':publish_action,'rollback_handler_preserved':'rollback_production' in main,'tenant_admin_global':tenant_ok,'professor_admin_global':prof_ok,'future_effects_closed':cfg.get('production_effects_enabled') is False})
