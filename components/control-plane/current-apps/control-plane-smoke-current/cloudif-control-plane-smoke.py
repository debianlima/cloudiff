...
try:
 ...
 with urllib.request.urlopen(req,timeout=30) as x:page=x.read().decode('utf-8','replace')
 pol=json.load(open('/etc/cloudif/project-capabilities-policy.json'));future=api.get('future_project_template') or {};ok=api.get('ok') is True and api.get('catalog_tools')==52 and len(api.get('projects') or [])==8 and future.get('tool_count')==52 and api.get('apply_to_new') is True and pol.get('apply_to_new') is True and pol.get('reconcile_after_onboarding') is True and pol.get('production_effect_scopes_enabled') is False and api.get('secrets_exposed') is False and 'Capacidades dos projetos' in page and 'class="active" href="/cloudiff/portal/?tab=capacidades"' in page and 'Ver todas as ferramentas' in page
...
try:
 ...
 con=sqlite3.connect('/var/lib/cloudif/portal/cloudif-portal.db');con.row_factory=sqlite3.Row;r=con.execute("select actor,rc,stdout from action_log where action='publication_publish_version' and target='sistema-de-biblioteca-teste' and rc=0 order by id desc limit 1").fetchone();con.close()
 out=json.loads(r['stdout']) if r and r['stdout'] else {};ok=good in main and bad not in main and r and r['actor']=='iff1742962' and r['rc']==0 and out.get('public_number')==1009 and out.get('deploy_number')>=2
 checks.append({'name':'tenant-publication-post-redirect','ok':ok,'correct_public_path':good in main,'legacy_wrong_path_absent':bad not in main,'last_actor':r['actor'] if r else None,'last_rc':r['rc'] if r else None,'public_number':out.get('public_number'),'deploy_number':out.get('deploy_number'),'publication_succeeded':bool(r and r['rc']==0),'secrets_exposed':False})
...
try:
 ...
 urls={'d8':'https://1009-d8.cloudiff.duckdns.org/','d9':'https://1009-d9.cloudiff.duckdns.org/','d10':'https://1009-d10.cloudiff.duckdns.org/','d11':'https://1009-d11.cloudiff.duckdns.org/','stable':'https://1009.cloudiff.duckdns.org/','textual':'https://sistema-de-biblioteca-teste.cloudiff.duckdns.org/'};bodies={};codes={}
 ...
 by={x['deploy_number']:x for x in valid};active=[x for x in valid if x['is_active']==1]
 repub10=json.loads(by[10]['detail_json'] or '{}');repub11=json.loads(by[11]['detail_json'] or '{}')
 ok={8,9,10,11}.issubset(set(by)) and len(legacy)==7 and len(active)==1 and active[0]['deploy_number']==11 and by[8]['commit_sha']!=by[9]['commit_sha'] and by[9]['commit_sha']==by[10]['commit_sha']==by[11]['commit_sha'] and codes=={k:200 for k in urls} and hashes['d8']!=hashes['d9'] and hashes['d9']==hashes['d10']==hashes['d11']==hashes['stable']==hashes['textual'] and b'Vers\xc3\xa3o 1' in bodies['d8'] and b'Vers\xc3\xa3o 2' in bodies['d9'] and repub10.get('republished') is True and repub11.get('republished') is True and repub10.get('republished_from')==9 and repub11.get('republished_from')==10 and "status='published'" in ui
 checks.append({'name':'git-backed-distinct-publication-versions','ok':ok,'valid_versions':[x['deploy_number'] for x in valid],'legacy_duplicates':len(legacy),'active_deploy':active[0]['deploy_number'] if active else None,'d8_code':codes['d8'],'d9_code':codes['d9'],'d10_code':codes['d10'],'d11_code':codes['d11'],'d8_sha256':hashes['d8'],'d9_sha256':hashes['d9'],'d10_sha256':hashes['d10'],'d11_sha256':hashes['d11'],'code_revisions_distinct':hashes['d8']!=hashes['d9'],'same_commit_republication':hashes['d9']==hashes['d10']==hashes['d11'],'stable_matches_d11':hashes['stable']==hashes['d11'],'textual_matches_d11':hashes['textual']==hashes['d11'],'ui_hides_legacy':"status='published'" in ui,'explicit_republication_enabled':True,'secrets_exposed':False})
