        ...
        if parsed.path.rstrip("/") in ["/cloudiff/portal/action/publication","/cloudif/portal/action/publication","/action/publication"]:
            ...
            try:
                if op=="publish_version":
                    ...
                    msg=f"Publicação enfileirada. Job {result['job_id']} · base r{result.get('baseRevision',0)} · ambiente r{result.get('environmentRevision',0)}."
                elif op=="open_base_workspace":
                    result=publications.base_workspace_preflight(slug,user)
                    try: log_action(user.get("username") or "portal", "publication_base_workspace_prepare", slug, 0, json.dumps(result,ensure_ascii=False), "")
                    except Exception: pass
                    return self.send_html(_cloudif_pub_base_preparing_page(slug,_prod_csrf_token(user)),200)
...
<section class="project-final__section project-final__publication"><h3>Publicação</h3><p><strong>Serviço web: {h(runtime_state['status'])}</strong></p><p class="project-final__meta">Framework: {h(runtime_state['label'])}</p><div class="project-final__status"><span class="pill {'ok' if runtime_state['healthy'] else 'warn'}">{h(runtime_state['service'])}</span><span class="pill info">{h(runtime_state['label'])}</span></div><p class="project-final__meta">A próxima publicação congela a base editável e a revisão atual das variáveis.</p><div class="project-final__actions"><form method="post" target="_blank" action="/cloudiff/portal/action/publication"><input type="hidden" name="csrf_token" value="{h(csrf_token)}"><input type="hidden" name="slug" value="{h(slug)}"><button class="btn light" name="op" value="open_base_workspace">Abrir base no Komodo</button></form><button class="btn light" type="button" data-publication-environments data-project-slug="{h(slug)}">Ambientes de publicação</button><a class="btn light" href="/cloudiff/portal/?tab=publicacao">Versões</a></div></section>
