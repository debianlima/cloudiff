...
(function(){
 ...
 function envClose(){envLayer.style.display='none';document.body.classList.remove('cloudif-modal-open');envMessage('')}
 ...
 const publicationEnvironmentLayer=document.createElement('div');publicationEnvironmentLayer.className='cloudif-wizard publication-environments-wizard';publicationEnvironmentLayer.id='cloudif-publication-environments';publicationEnvironmentLayer.setAttribute('role','dialog');publicationEnvironmentLayer.setAttribute('aria-modal','true');publicationEnvironmentLayer.setAttribute('aria-labelledby','publication-environments-title');publicationEnvironmentLayer.innerHTML='<section class="wizard-box publication-environments-box"><header class="wizard-head publication-environments-head"><div><span>Publicação</span><h3 id="publication-environments-title">Ambientes de publicação</h3><p>Visualize um estágio por vez, sem carregar a página principal com detalhes.</p><p class="publication-environments-project" data-publication-environments-project></p></div><button type="button" class="wizard-close publication-environments-close" aria-label="Fechar">×</button></header><nav class="publication-environments-tabs" role="tablist" aria-label="Ambientes"><button type="button" role="tab" data-publication-env-tab="preview">Preview</button><button type="button" role="tab" data-publication-env-tab="homologation">Homologação</button><button type="button" role="tab" data-publication-env-tab="production">Produção</button></nav><div class="publication-environments-body" data-publication-environments-body><div class="publication-environment-loading">Carregando ambiente…</div></div><footer class="publication-environments-footer"><button type="button" class="btn light" data-publication-environments-refresh>Atualizar</button><button type="button" class="btn light" data-publication-environments-close>Fechar</button></footer></section>';const publicationEnvironmentMount=document.querySelector('.legacy-content')||document.body;publicationEnvironmentMount.appendChild(publicationEnvironmentLayer);
 const publicationEnvironmentModel={slug:'',active:'production',data:null};const publicationEnvironmentBody=publicationEnvironmentLayer.querySelector('[data-publication-environments-body]'),publicationEnvironmentProject=publicationEnvironmentLayer.querySelector('[data-publication-environments-project]');
 function publicationEnvironmentClose(){publicationEnvironmentLayer.style.display='none';document.body.classList.remove('cloudif-modal-open')}
 function publicationEnvironmentSelect(name){publicationEnvironmentModel.active=['preview','homologation','production'].includes(name)?name:'production';publicationEnvironmentLayer.querySelectorAll('[data-publication-env-tab]').forEach(button=>{const selected=button.dataset.publicationEnvTab===publicationEnvironmentModel.active;button.setAttribute('aria-selected',String(selected));button.classList.toggle('is-active',selected)});publicationEnvironmentRender()}
 function publicationEnvironmentRender(){const data=publicationEnvironmentModel.data;if(!data){publicationEnvironmentBody.innerHTML='<div class="publication-environment-loading">Carregando ambiente…</div>';return}const item=(data.environments||[]).find(x=>x.name===publicationEnvironmentModel.active)||{},name=item.name||publicationEnvironmentModel.active,label=envLabels[name]||name,status=envStatusLabels[item.status]||item.status||'—',cfg=item.configuration||{},runtime=item.runtime||{},artifact=String(item.artifact||''),artifactLabel=item.deployNumber?'d'+item.deployNumber:(artifact?artifact.slice(0,12):'—'),missing=Number(cfg.missingRequired||0),detail=cfg.valid===true?'Configuração válida':(missing?missing+' obrigatória(s) pendente(s)':'Configuração ainda não validada'),expires=item.expiresAt?new Date(Number(item.expiresAt)*1000).toLocaleString('pt-BR'):'',url=String(item.url||''),isOk=['active','running','published'].includes(item.status),open=url?'<a class="btn" href="'+envEsc(url)+'" target="_blank" rel="noopener">Abrir '+envEsc(label)+'</a>':'<span class="publication-environment-hint">Ainda não há URL pública para este ambiente.</span>',variables=data.canWrite?'<button type="button" class="btn light" data-env-wizard data-project-slug="'+envEsc(publicationEnvironmentModel.slug)+'" data-env-environment="'+envEsc(name)+'">Variáveis</button>':'';publicationEnvironmentBody.innerHTML='<article class="publication-environment-detail"><div class="publication-environment-status"><div><span>'+envEsc(label)+'</span><h4>'+envEsc(status)+'</h4></div><span class="environment-state '+(isOk?'is-ok':'')+'">'+envEsc(status)+'</span></div><dl><div><dt>Configuração</dt><dd>r'+Number(cfg.revision||0)+'</dd></div><div><dt>Validação</dt><dd>'+envEsc(detail)+'</dd></div><div><dt>Artefato</dt><dd><code>'+envEsc(artifactLabel)+'</code></dd></div>'+(runtime.status?'<div><dt>Runtime</dt><dd>'+envEsc(envStatusLabels[runtime.status]||runtime.status)+'</dd></div>':'')+(expires?'<div><dt>Expira</dt><dd>'+envEsc(expires)+'</dd></div>':'')+'</dl><div class="publication-environment-actions">'+open+variables+'</div></article>'}
 async function publicationEnvironmentLoad(){publicationEnvironmentModel.data=null;publicationEnvironmentRender();try{const response=await fetch('/cloudiff/portal/api/projects/'+encodeURIComponent(publicationEnvironmentModel.slug)+'/environments-overview',{credentials:'same-origin',headers:{Accept:'application/json','Cache-Control':'no-store'}}),data=await response.json();if(!response.ok||!data.ok)throw new Error((data.error||{}).message||'environment_overview_failed');publicationEnvironmentModel.data=data;publicationEnvironmentRender()}catch(error){publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>Não foi possível consultar os ambientes.</strong><span>Tente novamente em alguns instantes.</span></div>'}}
 function publicationEnvironmentOpen(button){publicationEnvironmentModel.slug=button.dataset.projectSlug||'';publicationEnvironmentModel.active=['preview','homologation','production'].includes(button.dataset.environment)?button.dataset.environment:'production';publicationEnvironmentProject.textContent=publicationEnvironmentModel.slug;publicationEnvironmentLayer.style.display='grid';document.body.classList.add('cloudif-modal-open');publicationEnvironmentSelect(publicationEnvironmentModel.active);publicationEnvironmentLoad();setTimeout(()=>publicationEnvironmentLayer.querySelector('[aria-selected="true"]')?.focus(),0)}
 publicationEnvironmentLayer.querySelector('.publication-environments-close').onclick=publicationEnvironmentClose;publicationEnvironmentLayer.querySelector('[data-publication-environments-close]').onclick=publicationEnvironmentClose;publicationEnvironmentLayer.querySelector('[data-publication-environments-refresh]').onclick=publicationEnvironmentLoad;publicationEnvironmentLayer.querySelectorAll('[data-publication-env-tab]').forEach(button=>button.onclick=()=>publicationEnvironmentSelect(button.dataset.publicationEnvTab));publicationEnvironmentLayer.addEventListener('click',event=>{if(event.target===publicationEnvironmentLayer)publicationEnvironmentClose();if(event.target.closest('[data-env-wizard]'))publicationEnvironmentClose()});
 document.addEventListener('click',event=>{const button=event.target.closest('[data-publication-environments]');if(button)publicationEnvironmentOpen(button)});
...
_WHP_RELEASE_ASSETS=r"""<style id="cloudif-whp-release-style">
.publication-release-flow{display:grid;gap:16px;padding:18px;border:1px solid var(--c-border);border-radius:14px;background:var(--c-surface)}
.publication-release-flow__head{display:flex;align-items:flex-start;justify-content:space-between;gap:24px}.publication-release-flow__head>div{display:grid;gap:4px}.publication-release-flow__head>div>span,.release-kicker{font-size:.68rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--c-muted)}.publication-release-flow__head strong{font-size:1rem}.publication-release-flow__head p{margin:0;max-width:720px;color:var(--c-muted)}
.publication-release-flow__stages{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.publication-release-flow__stages article{display:flex;align-items:center;gap:10px;padding:12px;border:1px solid var(--c-border);border-radius:10px;background:var(--c-surface-2)}.publication-stage-code{display:grid;place-items:center;width:34px;height:34px;border-radius:9px;background:var(--c-primary-soft);color:var(--c-primary);font-weight:850}.publication-release-flow__stages article>div{display:grid;gap:1px;min-width:0}.publication-release-flow__stages small,.publication-stage-state{font-size:.7rem;color:var(--c-muted)}.publication-stage-state{margin-left:auto;text-align:right}.publication-release-flow__tools{display:flex;gap:8px;flex-wrap:wrap}
#cloudif-release-flow{position:fixed;inset:0;z-index:1900;display:none;place-items:center;padding:20px;background:rgba(6,10,18,.62)}#cloudif-release-flow.is-open{display:grid}.release-wizard{display:grid;grid-template-rows:auto auto minmax(0,1fr) auto;width:min(960px,100%);max-height:min(94vh,900px);overflow:hidden;border:1px solid var(--c-border);border-radius:18px;background:var(--c-surface);box-shadow:0 30px 90px rgba(0,0,0,.3)}.release-wizard>header{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:22px;border-bottom:1px solid var(--c-border)}.release-wizard>header h2{margin:4px 0 5px;font-size:clamp(1.35rem,3vw,1.8rem)}.release-wizard>header p{margin:0;color:var(--c-muted)}.release-wizard-close{border:0!important;background:transparent!important;color:var(--c-muted)!important;font-size:1.8rem;line-height:1;padding:4px!important;min-height:auto!important}.release-tabs{display:grid;grid-template-columns:repeat(3,1fr);gap:0;border-bottom:1px solid var(--c-border)}.release-tabs button{border:0!important;border-radius:0!important;background:transparent!important;color:var(--c-muted)!important;padding:13px!important}.release-tabs button.is-active{color:var(--c-primary)!important;box-shadow:inset 0 -2px var(--c-primary)}.release-wizard-body{overflow:auto;padding:22px}.release-wizard-footer{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 22px;border-top:1px solid var(--c-border);background:var(--c-surface)}.release-wizard-footer>span{font-size:.75rem;color:var(--c-muted)}
.release-stage{display:grid;gap:16px}.release-stage-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.release-stage-head h3{margin:3px 0}.release-stage-head p{margin:0;color:var(--c-muted)}.release-badge{display:inline-flex;align-items:center;padding:5px 9px;border:1px solid var(--c-border);border-radius:999px;background:var(--c-surface-2);font-size:.72rem;font-weight:800}.release-badge.ok{background:var(--c-success-soft)!important;color:var(--c-success)!important;border-color:var(--c-success-border)!important}.release-badge.warn{background:var(--c-warning-soft)!important;color:var(--c-warning)!important;border-color:var(--c-warning-border)!important}.release-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.release-box{display:grid;gap:6px;padding:14px;border:1px solid var(--c-border);border-radius:11px;background:var(--c-surface-2)}.release-box>span{font-size:.68rem;text-transform:uppercase;letter-spacing:.06em;color:var(--c-muted)}.release-box code{overflow-wrap:anywhere}.release-actions{display:flex;gap:8px;flex-wrap:wrap}.release-alert{display:grid;gap:4px;padding:12px 14px;border:1px solid var(--c-warning-border);border-radius:10px;background:var(--c-warning-soft);color:var(--c-warning)}.release-alert.bad{border-color:var(--c-danger-border);background:var(--c-danger-soft);color:var(--c-danger)}.release-alert.ok{border-color:var(--c-success-border);background:var(--c-success-soft);color:var(--c-success)}.release-diff{display:grid;gap:10px}.release-diff details{border:1px solid var(--c-border);border-radius:10px;background:var(--c-surface)}.release-diff summary{padding:12px 14px;font-weight:750;cursor:pointer}.release-diff-list{margin:0;padding:0 14px 14px;list-style:none;display:grid;gap:5px}.release-diff-list li{display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-top:1px solid var(--c-border);font-size:.78rem}.release-diff-list code{overflow-wrap:anywhere}.release-homologators{display:grid;gap:8px;padding-top:4px}.release-homologators label{display:grid;gap:6px}.release-homologators small{color:var(--c-muted)}.release-history{display:grid;gap:7px}.release-history-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 0;border-top:1px solid var(--c-border)}.release-job{display:grid;gap:7px;padding:12px 14px;border:1px solid var(--c-info-border);border-radius:10px;background:var(--c-info-soft);color:var(--c-info)}.release-job progress{width:100%}.publication-technical-history{padding:12px;border:1px solid var(--c-border);border-radius:10px}.publication-technical-history>summary{cursor:pointer;font-size:.78rem;color:var(--c-muted)}
@media(max-width:700px){.publication-release-flow__head,.release-stage-head,.release-wizard-footer{align-items:stretch;flex-direction:column}.publication-release-flow__stages,.release-grid{grid-template-columns:1fr}.release-wizard{max-height:96vh}.release-wizard-body,.release-wizard>header{padding:16px}.release-tabs button{font-size:.76rem;padding:11px 5px!important}.release-actions{display:grid;grid-template-columns:1fr}.release-actions .btn,.release-actions button,.release-actions a{width:100%!important;text-align:center}.publication-stage-state{text-align:left}}
</style><script id="cloudif-whp-release-script">
(function(){
