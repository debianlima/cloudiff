...
(function(){
 ...
 async function publicationTerminalOpen(button=null,requestId=publicationEnvironmentModel.toolRequest){
   ...
   try{
     ...
     const embed=new URL(target.href);embed.searchParams.set('_cloudiff_embed',String(Date.now()));
     publicationEnvironmentBody.innerHTML='<section class="stage-site-preview stage-terminal-embed"><header><div><span>'+envEsc(data.stageCode||ctx.stageCode)+' · '+envEsc(ctx.label)+'</span><strong>Terminal do container</strong><small>'+envEsc(data.container||ctx.container||'')+'</small></div><a class="btn light" href="'+envEsc(target.href)+'" target="_blank" rel="noopener">Abrir no Komodo</a></header><div class="stage-site-preview__frame stage-terminal-embed__frame"><iframe src="'+envEsc(embed.href)+'" title="Terminal '+envEsc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" allow="clipboard-read; clipboard-write; fullscreen" allowfullscreen></iframe></div><footer>Terminal carregado automaticamente no container deste estágio. “Abrir no Komodo” permanece disponível como alternativa.</footer></section>';
...
(function(){
 ...
 function renderEmbeddedTerminal(result,ctx){const target=new URL(String(result.terminalUrl||''),location.origin);if(target.protocol!=='https:'||target.hostname!=='komodoiff.duckdns.org'||!target.pathname.includes('/terminal/'))throw new Error('A plataforma retornou um terminal inválido.');const embed=new URL(target.href);embed.searchParams.set('_cloudiff_embed',String(Date.now()));body.innerHTML='<section class="stage-site-preview stage-terminal-embed"><header><div><span>'+esc(result.stageCode||ctx.stageCode)+' · '+esc(ctx.label)+'</span><strong>Terminal do container</strong><small>'+esc(result.container||ctx.container||'')+'</small></div><a class="btn light" href="'+esc(target.href)+'" target="_blank" rel="noopener">Abrir no Komodo</a></header><div class="stage-site-preview__frame stage-terminal-embed__frame"><iframe src="'+esc(embed.href)+'" title="Terminal '+esc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" allow="clipboard-read; clipboard-write; fullscreen" allowfullscreen></iframe></div><footer>Terminal carregado automaticamente no container deste estágio. “Abrir no Komodo” permanece disponível como alternativa.</footer></section>'}
