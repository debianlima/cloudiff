...
<section class="project-final__section"><h3>Terminal no Komodo</h3><p><strong>{'Stack '+h(runtime_project.get('stack_id')) if runtime_project.get('stack_id') else 'Container não vinculado'}</strong></p><p class="project-final__meta">Serviço: {h(runtime_project.get('service') or 'web')} · Stack vinculada</p><div class="project-final__actions">{terminal_action}</div></section>
<section class="project-final__section project-final__publication"><h3>Preview e publicação</h3><p><strong>W Preview · H Homologation · P Publication</strong></p><p class="project-final__meta">O Preview é vivo. Homologação congela código + runtime. Produção reutiliza exatamente o artefato homologado.</p><div class="project-final__status"><span class="pill info">W</span><span class="pill info">H</span><span class="pill ok">P</span></div><div class="project-final__actions"><a class="btn" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}&amp;open=release">Gerenciar publicação</a><a class="btn light" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}&amp;open=variables">Variáveis por ambiente</a><a class="btn light" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}">Abrir publicações</a></div></section>
...
<section class="project-final__section" data-provision-status="{h(provision_state['status'])}" data-provision-recoverable="{'1' if provision_state.get('recoverable') else '0'}"><h3>Provisionamento</h3><p><strong>{h({'queued':'Na fila','running':'Em execução','succeeded':'Concluído','failed':'Requer atenção','not_started':'Não iniciado'}.get(provision_state['status'],provision_state['status']))}</strong></p><p class="project-final__meta">Etapa: {h(provision_state['step'] or '—')} · Atualizado: {h(provision_state['updated_at'] or '—')}</p>{('<p class="project-final__meta">'+h(provision_state['error'])+'</p>') if provision_state['error'] else ''}{('<p class="project-final__meta"><strong>A infraestrutura está pronta.</strong> A retomada continuará da última etapa segura sem recriar projeto, repositório ou banco.</p>') if provision_state.get('recoverable') else ''}</section>
