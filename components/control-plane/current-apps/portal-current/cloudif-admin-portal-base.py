...
(function(){
 ...
 const envLayer=document.createElement('div');envLayer.className='cloudif-wizard';envLayer.id='cloudif-environment-wizard';envLayer.setAttribute('role','dialog');envLayer.setAttribute('aria-modal','true');envLayer.setAttribute('aria-labelledby','env-wizard-title');envLayer.innerHTML='<section class="wizard-box env-wizard-box"><header class="wizard-head"><div><span class="env-type">Configuração versionada</span><h3 id="env-wizard-title">Variáveis de ambiente</h3><p>Gerencie as variáveis do projeto sem expor valores secretos.</p><p class="env-type" data-env-project></p></div><button type="button" class="wizard-close env-wizard-close" aria-label="Fechar">×</button></header><div class="pm-new-shell env-wizard-shell"><div class="env-wizard-toolbar"><label class="pm-field"><span>Ambiente</span><select data-env-environment><option value="preview">Preview</option><option value="homologation">Homologação</option><option value="production" selected>Produção</option></select><small>Escolha em qual ambiente a configuração será aplicada.</small></label><span class="env-revision" data-env-revision></span></div><section class="pm-new-section"><div class="pm-new-section__head"><h4>Variáveis</h4><p>Valores públicos podem ser editados aqui. Secrets permanecem protegidos.</p></div><div class="env-list" data-env-list></div><div class="env-status" data-env-status hidden></div></section><footer class="pm-new-footer"><button type="button" class="btn light" data-env-cancel>Cancelar</button><button type="button" class="btn light" data-env-add>+ Nova variável</button><button type="button" class="btn" data-env-save>Salvar alterações</button></footer></div></section>';const envMount=document.querySelector('.legacy-content')||document.body;envMount.appendChild(envLayer);
