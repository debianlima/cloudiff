...
<section class="project-final__section"><h3>Ações do projeto</h3><p class="project-final__meta"><strong>Checar projeto</strong> verifica repositório, banco e vínculo do container sem alterar a configuração. <strong>Permissões</strong> controla quem pode acessar este projeto.</p><div class="project-final__actions"><form method="post" action="{url('/action/project_action')}"><input type="hidden" name="csrf_token" value="{h(csrf_token)}"><input type="hidden" name="slug" value="{h(slug)}"><button class="btn gray" name="op" value="check">Checar projeto</button></form><button class="btn light" type="button" onclick="cloudifShowWizard('{acl_id}')">Gerenciar permissões</button></div></section>
 ...
 form.addEventListener('input',updateSummary);form.addEventListener('change',updateSummary);if(previous)previous.addEventListener('click',()=>show(current-1));if(next)next.addEventListener('click',()=>{{if(validateStep())show(current+1)}});stepItems.forEach((item,i)=>item.addEventListener('click',()=>{{if(i<=current||validateStep())show(i)}}));const live=document.getElementById('pm197_provision_live'),shell=modal.querySelector('.pm-new-shell'),headTitle=document.getElementById('pm197_new_title'),headText=modal.querySelector('.wizard-head p'),closeButton=modal.querySelector('.wizard-close');const liveLabels=['Registro do projeto','Repositório Forgejo','Stack e containers','Banco e tenant','Identidade e permissões','Template da aplicação','Publicação inicial','Conclusão'];let provisioning=false,lastProvision=null;
