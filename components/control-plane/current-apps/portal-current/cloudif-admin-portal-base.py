...
class Portal(BaseHTTPRequestHandler):
    def end_headers(self):
        ...
        self.send_header("Content-Security-Policy", "default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; frame-src 'self' https://*.cloudiff.duckdns.org; object-src 'none'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self'")
        super().end_headers()
...
(function(){
 ...
 async function publicationTerminalOpen(button){
   ...
   try{
     ...
     if(popup)popup.location.replace(target.href);else location.assign(target.href);
   }catch(error){if(popup)popup.close();publicationEnvironmentBody.insertAdjacentHTML('afterbegin','<div class="publication-environment-error is-inline"><strong>Terminal não aberto.</strong><span>'+envEsc(error.message||'Não foi possível preparar o terminal.')+'</span></div>')}finally{button.disabled=false}
 }
...
(function(){
 ...
 async function openStageTerminal(){if(model.busy)return;model.busy=true;const popup=window.open('about:blank','_blank');try{if(popup){popup.document.title='Preparando terminal do ambiente';popup.document.body.innerHTML='<p style="font-family:system-ui;padding:24px">Preparando terminal do ambiente…</p>'}const environment=model.tab==='publication'?'production':model.tab,result=await post('stage/terminal',{environment}),target=new URL(String(result.terminalUrl||''),location.origin);if(target.protocol!=='https:'||target.hostname!=='komodoiff.duckdns.org'||!target.pathname.includes('/terminal/'))throw new Error('A plataforma retornou um terminal inválido.');if(popup)popup.location.replace(target.href);else location.assign(target.href)}catch(error){if(popup)popup.close();body.insertAdjacentHTML('afterbegin','<div class="release-alert bad"><strong>Terminal não aberto.</strong><span>'+esc(error.message)+'</span></div>')}finally{model.busy=false}}
 async function openPreviewTerminal(){if(model.busy)return;model.busy=true;const popup=window.open('about:blank','_blank');try{if(popup){popup.document.title='Preparando terminal do Preview';popup.document.body.innerHTML='<p style="font-family:system-ui;padding:24px">Preparando terminal do Preview…</p>'}const result=await post('preview/terminal',{}),target=new URL(String(result.terminalUrl||''),location.origin);if(target.protocol!=='https:'||target.hostname!=='komodoiff.duckdns.org'||!target.pathname.includes('/terminal/'))throw new Error('A plataforma retornou um terminal inválido.');if(popup)popup.location.replace(target.href);else location.assign(target.href)}catch(error){if(popup)popup.close();body.insertAdjacentHTML('afterbegin','<div class="release-alert bad"><strong>Terminal não aberto.</strong><span>'+esc(error.message)+'</span></div>')}finally{model.busy=false}}
