        ...
        if parsed.path.rstrip("/") in ["/cloudiff/portal/action/publication","/cloudif/portal/action/publication","/action/publication"]:
            ...
            try:
                if op=="publish_version":
                    ...
                    return _cloudif_pub_redirect(self,result['terminalUrl'])
                elif op=="set_alias":
...
.publication-configuration{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.publication-configuration>article{display:grid;align-content:start;gap:8px;padding:16px;border:1px solid var(--cif-border);border-radius:12px;background:var(--cif-surface)}.publication-configuration>article>span{font-size:.7rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--cif-muted)}.publication-configuration p{margin:0;color:var(--cif-muted);font-size:.8rem;line-height:1.5}.publication-configuration form{margin-top:4px}.env-wizard-layer{position:fixed;inset:0;z-index:9200;display:grid;place-items:center;padding:20px;background:color-mix(in srgb,var(--c-text,var(--cif-text)) 55%,transparent)}.env-wizard-layer[hidden]{display:none}.env-wizard{width:min(820px,100%);max-height:92vh;overflow:auto;background:var(--cif-surface);border-radius:16px;padding:22px;box-shadow:var(--c-shadow,var(--cif-shadow))}.env-wizard header{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;padding-bottom:16px;border-bottom:1px solid var(--cif-border)}.env-wizard header h2{margin:3px 0}.env-wizard header p{margin:0;color:var(--cif-muted)}.env-wizard-close{border:0;background:transparent;color:inherit;font-size:1.6rem;cursor:pointer}.env-toolbar{display:flex;align-items:end;justify-content:space-between;gap:12px;margin:18px 0}.env-toolbar label{display:grid;gap:5px}.env-toolbar select{min-width:190px}.env-revision{font-size:.8rem;color:var(--cif-muted)}.env-list{display:grid;gap:8px}.env-row{display:grid;grid-template-columns:minmax(160px,.8fr) minmax(220px,1.4fr) auto;gap:9px;align-items:center;padding:10px;border:1px solid var(--cif-border);border-radius:10px}.env-row input{width:100%}.env-row code{font-size:.8rem}.env-row.secret{background:var(--c-surface-2,var(--cif-bg))}.env-row .env-type{font-size:.72rem;color:var(--cif-muted)}.env-empty{padding:24px;border:1px dashed var(--cif-border);border-radius:10px;text-align:center;color:var(--cif-muted)}.env-actions{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-top:18px;padding-top:16px;border-top:1px solid var(--cif-border)}.env-actions>div{display:flex;gap:8px}.env-status{margin-top:14px;padding:11px 12px;border-radius:9px;background:var(--c-surface-2,var(--cif-bg));font-size:.82rem}.env-status.bad{background:var(--c-danger-soft);color:var(--c-danger)}.env-status.pending{background:var(--c-warning-soft);color:var(--c-warning)}@media(max-width:760px){.publication-configuration{grid-template-columns:1fr}.env-row{grid-template-columns:1fr}.env-toolbar,.env-actions{align-items:stretch;display:grid}.env-actions>div{display:grid;grid-template-columns:1fr 1fr}}
</style>
"""
...
(function(){
 ...
 const envLayer=document.createElement('div');envLayer.className='env-wizard-layer';envLayer.hidden=true;envLayer.innerHTML='<section class="env-wizard" role="dialog" aria-modal="true" aria-labelledby="env-wizard-title"><header><div><span class="env-type">Configuração versionada</span><h2 id="env-wizard-title">Variáveis de ambiente</h2><p data-env-project></p></div><button type="button" class="env-wizard-close" aria-label="Fechar">×</button></header><div class="env-toolbar"><label><span>Ambiente</span><select data-env-environment><option value="preview">Preview</option><option value="homologation">Homologação</option><option value="production" selected>Produção</option></select></label><span class="env-revision" data-env-revision></span></div><div class="env-list" data-env-list></div><div class="env-status" data-env-status hidden></div><div class="env-actions"><button type="button" class="btn light" data-env-add>+ Nova variável</button><div><button type="button" class="btn light" data-env-cancel>Cancelar</button><button type="button" class="btn" data-env-save>Salvar alterações</button></div></div></section>';document.body.appendChild(envLayer);
 ...
 function envClose(){envLayer.hidden=true;envMessage('')}
 ...
 async function envWaitApproval(planDigest,approval){envMessage('Aguardando aprovação humana. Você pode aprovar na aba Aprovações; este wizard continuará acompanhando.','pending');const deadline=Date.now()+10*60*1000;while(!envLayer.hidden&&Date.now()<deadline){await new Promise(r=>setTimeout(r,2500));const status=await envFetch('/cloudiff/portal/api/projects/'+encodeURIComponent(envModel.slug)+'/environment/approval/status?approvalId='+encodeURIComponent(approval.approvalId));if(status.status==='approved')return envExecute(planDigest,approval.approvalId);if(['rejected','expired'].includes(status.status))throw new Error('A aprovação foi '+(status.status==='rejected'?'rejeitada':'expirada')+'.')}if(!envLayer.hidden)throw new Error('Tempo de acompanhamento da aprovação encerrado. Abra o wizard novamente para continuar.')}
 ...
 envLayer.querySelector('.env-wizard-close').onclick=envClose;envLayer.querySelector('[data-env-cancel]').onclick=envClose;envLayer.querySelector('[data-env-add]').onclick=()=>{envModel.rows.push({name:'',value:'',service:'',secret:false,new:true});envDraw()};envLayer.querySelector('[data-env-save]').onclick=envSave;envSelect.onchange=()=>{envModel.environment=envSelect.value;envLoad()};
 document.addEventListener('click',event=>{const button=event.target.closest('[data-env-wizard]');if(!button)return;envModel.slug=button.dataset.projectSlug||'';envModel.environment='production';envSelect.value='production';envLayer.querySelector('[data-env-project]').textContent=envModel.slug;envLayer.hidden=false;envLoad()});
