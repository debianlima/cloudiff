...
(function(){
 ...
 const publicationTools=['overview','php','node','variables'];
 ...
 const publicationEnvironmentModel={slug:'',active:'production',tool:'overview',data:null,toolRequest:0,loadRequest:0,opener:null};
 ...
 function publicationEnvironmentOverview(){
   const data=publicationEnvironmentModel.data,item=(data.environments||[]).find(x=>x.name===publicationEnvironmentModel.active)||{},name=item.name||publicationEnvironmentModel.active,label=envLabels[name]||name,status=envStatusLabels[item.status]||item.status||'—',cfg=item.configuration||{},runtime=item.runtime||{},artifact=String(item.artifact||''),artifactLabel=item.deployNumber?'d'+item.deployNumber:(artifact?artifact.slice(0,12):'—'),missing=Number(cfg.missingRequired||0),detail=cfg.valid===true?'Configuração válida':(missing?missing+' obrigatória(s) pendente(s)':'Configuração ainda não validada'),expires=item.expiresAt?new Date(Number(item.expiresAt)*1000).toLocaleString('pt-BR'):'',url=String(item.url||''),isOk=['active','running','published'].includes(item.status),open=url?'<a class="btn" href="'+envEsc(url)+'" target="_blank" rel="noopener">Abrir '+envEsc(label)+'</a>':'<span class="publication-environment-hint">Ainda não há URL pública para este ambiente.</span>',variables=data.canWrite?'<button type="button" class="btn light" data-publication-tool-jump="variables">Ver variáveis</button>':'';
   publicationEnvironmentBody.innerHTML='<article class="publication-environment-detail"><div class="publication-environment-status"><div><span>'+envEsc(label)+'</span><h4>'+envEsc(status)+'</h4></div><span class="environment-state '+(isOk?'is-ok':'')+'">'+envEsc(status)+'</span></div><dl><div><dt>Configuração</dt><dd>r'+Number(cfg.revision||0)+'</dd></div><div><dt>Validação</dt><dd>'+envEsc(detail)+'</dd></div><div><dt>Artefato</dt><dd><code>'+envEsc(artifactLabel)+'</code></dd></div>'+(runtime.status?'<div><dt>Runtime</dt><dd>'+envEsc(envStatusLabels[runtime.status]||runtime.status)+'</dd></div>':'')+(expires?'<div><dt>Expira</dt><dd>'+envEsc(expires)+'</dd></div>':'')+'</dl><div class="publication-environment-actions">'+open+variables+'</div></article>';
 }
 ...
 async function publicationEnvironmentLoad(){
   ...
   publicationEnvironmentModel.data=null;
 ...
 publicationEnvironmentLayer.addEventListener('click',event=>{if(event.target===publicationEnvironmentLayer)publicationEnvironmentClose();const jump=event.target.closest('[data-publication-tool-jump]');if(jump)publicationEnvironmentToolSelect(jump.dataset.publicationToolJump)});
...
(function(){
 ...
 function render(){if(!model.data){body.innerHTML='<p>Carregando fluxo…</p>';return}body.innerHTML=model.tab==='preview'?renderPreview():model.tab==='homologation'?renderHomologation():renderPublication();bindActions()}
