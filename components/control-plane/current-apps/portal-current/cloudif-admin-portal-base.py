...
class Portal(BaseHTTPRequestHandler):
    def end_headers(self):
        ...
        self.send_header("Content-Security-Policy", "default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; frame-src 'self' https://*.cloudiff.duckdns.org https://komodoiff.duckdns.org; object-src 'none'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self'")
        super().end_headers()
...
(function(){
 ...
 function publicationTerminalRender(){
   const ctx=publicationStageContext(),canWrite=Boolean((publicationEnvironmentModel.data||{}).canWrite);
   ...
   publicationEnvironmentBody.innerHTML='<section class="stage-terminal-panel"><header><span>'+envEsc(ctx.stageCode)+' · '+envEsc(ctx.label)+'</span><h4>Terminal do container</h4><p>Abra um shell no container exato deste estágio. Alterações feitas pelo terminal afetam somente o runtime atual e não alteram o artefato versionado.</p></header><dl><div><dt>Projeto</dt><dd>'+envEsc(publicationEnvironmentModel.slug)+'</dd></div><div><dt>Container esperado</dt><dd><code>'+envEsc(ctx.container||'Resolvido no servidor')+'</code></dd></div><div><dt>Estado</dt><dd>'+envEsc(ctx.status||'—')+'</dd></div></dl><div class="stage-terminal-panel__actions">'+(canWrite?'<button type="button" class="btn" data-publication-terminal-open>Abrir terminal no Komodo</button>':'<span class="publication-environment-hint">Você possui acesso de leitura, mas não permissão para abrir o terminal.</span>')+'</div><p class="stage-terminal-panel__note">Produção exige permissão do responsável ou administrador. O terminal será incorporado aqui; “Abrir no Komodo” permanece disponível como alternativa.</p></section>';
 }
 async function publicationTerminalOpen(button){
   if(button.disabled)return;button.disabled=true;const ctx=publicationStageContext();publicationEnvironmentLoading('Preparando terminal de '+ctx.label+'…');
   try{
     ...
     publicationEnvironmentBody.innerHTML='<section class="stage-site-preview stage-terminal-embed"><header><div><span>'+envEsc(data.stageCode||ctx.stageCode)+' · '+envEsc(ctx.label)+'</span><strong>Terminal do container</strong><small>'+envEsc(data.container||ctx.container||'')+'</small></div><a class="btn light" href="'+envEsc(target.href)+'" target="_blank" rel="noopener">Abrir no Komodo</a></header><div class="stage-site-preview__frame stage-terminal-embed__frame"><iframe src="'+envEsc(target.href)+'" title="Terminal '+envEsc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" allow="clipboard-read; clipboard-write" sandbox="allow-forms allow-modals allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts allow-downloads"></iframe></div><footer>Terminal autenticado do container deste estágio. Se a sessão do Komodo exigir novo login, use “Abrir no Komodo” e retorne a esta tela.</footer></section>';
   }catch(error){publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>Terminal não aberto.</strong><span>'+envEsc(error.message||'Não foi possível preparar o terminal.')+'</span></div>'}
 }
 ...
 function publicationEnvironmentRender(){
   ...
   if(publicationEnvironmentModel.tool==='terminal')return publicationTerminalRender();
...
(function(){
 ...
 function renderReleaseTerminal(){const c=releaseStageContext();if(!c.exists)return '<div class="release-alert"><strong>'+esc(c.label)+' ainda não possui container correspondente.</strong><span>Prepare este estágio antes de abrir o terminal.</span></div>';const allowed=c.environment==='production'?!!(model.data&&model.data.canPublish):!!(model.data&&model.data.canWrite);return '<section class="stage-terminal-panel"><header><span>'+esc(c.stageCode)+' · '+esc(c.label)+'</span><h4>Terminal do container</h4><p>O shell será aberto no container exato deste estágio. Alterações afetam o runtime atual e não reescrevem o artefato versionado.</p></header><dl><div><dt>Projeto</dt><dd>'+esc(model.slug)+'</dd></div><div><dt>Container esperado</dt><dd><code>'+esc(c.container||'Resolvido no servidor')+'</code></dd></div><div><dt>Estado</dt><dd>'+esc(c.status||'—')+'</dd></div></dl><div class="stage-terminal-panel__actions">'+(allowed?'<button type="button" class="btn" data-release-stage-terminal>Abrir terminal no Komodo</button>':'<span class="publication-environment-hint">O terminal de Produção exige o responsável pelo projeto ou administrador.</span>')+'</div><p class="stage-terminal-panel__note">O terminal será incorporado no gerenciador. “Abrir no Komodo” continua disponível como alternativa.</p></section>'}
 ...
 function render(){if(!model.data){body.innerHTML='<p>Carregando fluxo…</p>';return}body.innerHTML=model.view==='site'?renderReleaseSite():model.view==='terminal'?renderReleaseTerminal():(model.tab==='preview'?renderPreview():model.tab==='homologation'?renderHomologation():renderPublication());bindActions()}
 ...
 function renderEmbeddedTerminal(result,ctx){const target=new URL(String(result.terminalUrl||''),location.origin);if(target.protocol!=='https:'||target.hostname!=='komodoiff.duckdns.org'||!target.pathname.includes('/terminal/'))throw new Error('A plataforma retornou um terminal inválido.');body.innerHTML='<section class="stage-site-preview stage-terminal-embed"><header><div><span>'+esc(result.stageCode||ctx.stageCode)+' · '+esc(ctx.label)+'</span><strong>Terminal do container</strong><small>'+esc(result.container||ctx.container||'')+'</small></div><a class="btn light" href="'+esc(target.href)+'" target="_blank" rel="noopener">Abrir no Komodo</a></header><div class="stage-site-preview__frame stage-terminal-embed__frame"><iframe src="'+esc(target.href)+'" title="Terminal '+esc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" allow="clipboard-read; clipboard-write" sandbox="allow-forms allow-modals allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts allow-downloads"></iframe></div><footer>Terminal autenticado do container deste estágio. Se a sessão do Komodo exigir novo login, use “Abrir no Komodo” e retorne ao gerenciador.</footer></section>'}
