...
class Portal(BaseHTTPRequestHandler):
    def end_headers(self):
        ...
        self.send_header("Content-Security-Policy", "default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; frame-src 'self' https://*.cloudiff.duckdns.org https://komodoiff.duckdns.org https://authiff.duckdns.org; object-src 'none'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline'; connect-src 'self'")
        super().end_headers()
...
def do_POST_v21(self):
    ...
    if parsed.path.endswith("/action/tenant_acl"):
        ...
        if op == "add":
            ...
            try:
                from cloudif_reconcile_client import enqueue as _tenant_acl_enqueue
                _tenant_acl_enqueue(
                    "tenant.membership.changed",
                    actor=user.get("username") or "portal",
                    username=subject if stype == "user" else "",
                    tenant=tenant,
                    payload={"source":"tenant_acl","operation":"add","principal":subject,"principal_type":stype},
                    dedupe_seconds=0,
                )
            except Exception as exc:
                log_action(user["username"], "tenant_acl_reconcile_enqueue_failed", tenant, 1, f"{stype}:{subject}", type(exc).__name__)
        elif op == "remove":
            rid = val("id")
            row = con.execute("SELECT * FROM tenant_acl WHERE id=?", (rid,)).fetchone()
            ...
            try:
                ...
                _tenant_acl_enqueue(
                    "tenant.membership.changed",
                    actor=user.get("username") or "portal",
                    username=(row["subject"] if row and row["subject_type"] == "user" else ""),
                    tenant=(row["tenant"] if row else ""),
                    payload={"source":"tenant_acl","operation":"remove","principal":(row["subject"] if row else ""),"principal_type":(row["subject_type"] if row else "")},
                    dedupe_seconds=0,
                )
            except Exception as exc:
                log_action(user["username"], "tenant_acl_reconcile_enqueue_failed", row["tenant"] if row else rid, 1, str(dict(row)) if row else "", type(exc).__name__)
        ...
        if parsed.path.rstrip("/") in ["/cloudiff/portal/action/project_acl", "/cloudif/portal/action/project_acl", "/action/project_acl"]:
            ...
            try:
                ...
                try:
                    ...
                    msg += " ACL central salva; reconciliação externa pendente."
            except Exception as e:
                msg = "Erro: " + str(e)
...
def _cloudif_pub_send_json(handler,payload,status=200):
...
def _cloudif_pub_base_preparing_page(slug,csrf_token):
    safe_slug=html.escape(str(slug),quote=True);safe_csrf=html.escape(str(csrf_token),quote=True)
    return f'''<!doctype html>
<html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Preparando base · CloudIFF</title>
<style>
:root{{font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#17211b;background:#f6f8f6}}
*{{box-sizing:border-box}}body{{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px}}main{{width:min(640px,100%);background:#fff;border:1px solid #dfe6e1;border-radius:18px;padding:28px}}.eyebrow{{font-size:.72rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:#66746b}}h1{{margin:.35rem 0 .6rem;font-size:clamp(1.45rem,4vw,2rem);letter-spacing:-.035em}}p{{color:#66746b;line-height:1.55}}progress{{width:100%;height:8px;margin:18px 0 20px}}ol{{list-style:none;padding:0;margin:0;display:grid;gap:9px}}li{{display:flex;gap:11px;align-items:center;padding:11px 0;border-bottom:1px solid #edf1ee;color:#66746b}}li:last-child{{border-bottom:0}}.dot{{width:10px;height:10px;border-radius:50%;background:#c9d2cc;flex:none}}li.active{{color:#17211b;font-weight:750}}li.active .dot{{background:#16883f;box-shadow:0 0 0 5px #e5f3e9}}li.done{{color:#315b40}}li.done .dot{{background:#16883f}}li.failed{{color:#a11}}li.failed .dot{{background:#a11}}.status{{margin-top:18px;padding:12px 14px;border-radius:10px;background:#f2f5f3;font-size:.86rem}}.error{{margin-top:14px;padding:12px 14px;border-radius:10px;background:#fff1f2;color:#991b1b}}.actions{{display:flex;gap:10px;margin-top:18px}}a,button{{min-height:42px;padding:9px 13px;border-radius:9px;border:1px solid #d7dfd9;background:#fff;color:#17211b;text-decoration:none;font:inherit;font-weight:700;cursor:pointer}}[hidden]{{display:none!important}}
</style></head>
<body data-project="{safe_slug}" data-csrf="{safe_csrf}"><main><span class="eyebrow">Base do projeto</span><h1>Preparando o terminal no Komodo</h1><p>Você pode acompanhar esta etapa aqui. A CloudIFF abrirá o terminal automaticamente quando a base estiver pronta.</p><progress></progress><ol id="steps"><li class="active"><span class="dot"></span><span>Validando acesso ao projeto</span></li><li><span class="dot"></span><span>Preparando a imagem-base</span></li><li><span class="dot"></span><span>Iniciando o container editável</span></li><li><span class="dot"></span><span>Preparando o terminal no Komodo</span></li></ol><div class="status" id="status" role="status">Iniciando…</div><div class="error" id="error" hidden></div><div class="actions" id="actions" hidden><button type="button" id="retry">Tentar novamente</button><a href="/cloudiff/portal/?tab=projetos">Voltar aos projetos</a></div></main>
<script>
(()=>{{const slug=document.body.dataset.project,csrf=document.body.dataset.csrf,steps=[...document.querySelectorAll('#steps li')],status=document.getElementById('status'),error=document.getElementById('error'),actions=document.getElementById('actions'),retry=document.getElementById('retry');let active=0,start=Date.now(),timer=0;function show(index){{active=Math.min(index,steps.length-1);steps.forEach((step,i)=>{{step.classList.toggle('active',i===active);step.classList.remove('done','failed')}})}}function tick(){{const seconds=Math.floor((Date.now()-start)/1000);status.textContent='Processando há '+seconds+'s. Esta etapa pode demorar na primeira preparação da imagem.';if(seconds>=3&&active<1)show(1);if(seconds>=8&&active<2)show(2);if(seconds>=14&&active<3)show(3)}}async function run(){{clearInterval(timer);active=0;start=Date.now();show(0);error.hidden=true;actions.hidden=true;tick();timer=setInterval(tick,1000);const body=new URLSearchParams({{csrf_token:csrf,op:'prepare_base_workspace',slug}});try{{const response=await fetch('/cloudiff/portal/action/publication',{{method:'POST',credentials:'same-origin',headers:{{'Content-Type':'application/x-www-form-urlencoded'}},body}});const data=await response.json().catch(()=>({{ok:false,error:'Resposta inválida da CloudIFF.'}}));if(!response.ok||!data.ok)throw new Error(data.error||('HTTP '+response.status));clearInterval(timer);steps.forEach(step=>{{step.classList.remove('active','failed');step.classList.add('done')}});status.textContent='Terminal pronto. Autenticando no Komodo…';setTimeout(()=>location.replace('https://komodoiff.duckdns.org/auth/oidc/login?redirect='+encodeURIComponent(data.terminalUrl)),900)}}catch(err){{clearInterval(timer);steps[active].classList.remove('active');steps[active].classList.add('failed');status.textContent='A preparação não foi concluída.';error.hidden=false;error.textContent=String(err.message||err);actions.hidden=false}}}}retry.onclick=run;run()}})();
</script></body></html>'''
        ...
        if parsed.path.rstrip("/") in ["/cloudiff/portal/action/publication","/cloudif/portal/action/publication","/action/publication"]:
            ...
            try:
                if op=="publish_version":
                    result=publications.enqueue_homologation(slug,user)
                    msg=f"Fluxo atualizado: candidato {result.get('stageCode','H')} enfileirado para Homologação. A Produção não será alterada."
                elif op=="open_base_workspace":
                    try: log_action(user.get("username") or "portal", "publication_base_workspace_prepare_redirect", slug, 0, "dedicated_get_route", "")
                    except Exception: pass
                    return _cloudif_pub_redirect(self,"/cloudiff/portal/publication/base/"+_cloudif_pub_urlparse.quote(slug,safe=""))
                elif op=="prepare_base_workspace":
                    try:
                        result=publications.ensure_base_workspace(slug,user)
                        try: log_action(user.get("username") or "portal", "publication_base_workspace", slug, 0, json.dumps({k:v for k,v in result.items() if k!='terminalUrl'},ensure_ascii=False), "")
                        except Exception: pass
                        return _cloudif_pub_send_json(self,result,200)
                    except Exception as prepare_error:
                        try: log_action(user.get("username") or "portal", "publication_base_workspace", slug, 1, "", str(prepare_error))
                        except Exception: pass
                        return _cloudif_pub_send_json(self,{'ok':False,'error':str(prepare_error)},422)
                elif op=="set_alias":
                    ...
                    raise ValueError("A ativação direta de artefatos dN foi desativada. Use Homologação e Publicação P no wizard.")
...
def _prod_ui_enhance(doc):
    ...
    def _prod_ui_label_close(match):
        attrs=match.group(1) or ''
        if _prod_ui_re.search(r'\baria-label\s*=',attrs,flags=_prod_ui_re.I):
            return match.group(0)
        return '<button'+attrs+' aria-label="Fechar">×</button>'
    doc=_prod_ui_re.sub(r'<button([^>]*)>×</button>',_prod_ui_label_close,doc,flags=_prod_ui_re.I)
    return doc
...
.publication-information{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin:0 0 20px}.publication-info-card{display:flex;flex-direction:column;gap:6px;min-width:0;padding:16px;border:1px solid var(--cif-border);border-radius:12px;background:var(--cif-surface)}.publication-info-card>span{font-size:.68rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--cif-muted)}.publication-info-card>strong,.publication-info-card>a{font-size:.9rem;line-height:1.35;overflow-wrap:anywhere}.publication-info-card>small{margin-top:auto;color:var(--cif-muted);font-size:.72rem}.publication-runtime-card{align-items:flex-start}.publication-runtime-text{margin-top:6px;font-weight:800;color:var(--cif-accent,#16883f);text-decoration:underline;text-underline-offset:3px}.publication-runtime-text:hover{text-decoration-thickness:2px}.publication-runtime-card:hover{border-color:var(--cif-accent,#16883f)}.runtime-modal-open{overflow:hidden}.runtime-modal-backdrop{position:fixed;inset:0;z-index:1700;display:grid;place-items:center;padding:24px;background:rgba(8,18,12,.62)}.runtime-modal-backdrop[hidden]{display:none}.runtime-modal{display:grid;grid-template-rows:auto minmax(0,1fr) auto;width:min(940px,100%);max-height:min(86vh,820px);overflow:hidden;border-radius:18px;background:var(--cif-surface,#fff);box-shadow:0 28px 90px rgba(0,0,0,.34)}.runtime-modal>header,.runtime-modal>footer{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:20px 22px;border-bottom:1px solid var(--cif-border)}.runtime-modal>footer{justify-content:flex-end;border-top:1px solid var(--cif-border);border-bottom:0}.runtime-modal header span{font-size:.68rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--cif-muted)}.runtime-modal header h2{margin:4px 0}.runtime-modal header p{margin:0;color:var(--cif-muted)}.runtime-modal-close{border:0;background:transparent;font-size:1.8rem;line-height:1;cursor:pointer;color:inherit}.runtime-modal-body{overflow:auto;padding:20px 22px}.runtime-modal-meta{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px}.runtime-modal-meta>div{display:grid;gap:4px;padding:12px;border:1px solid var(--cif-border);border-radius:10px}.runtime-modal-meta span{font-size:.68rem;text-transform:uppercase;letter-spacing:.06em;color:var(--cif-muted)}.runtime-modal-body pre{margin:0;padding:18px;border-radius:12px;background:#071a10;color:#d9f7df;overflow:auto;white-space:pre-wrap;word-break:break-word;font:12px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace}.runtime-modal-loading,.runtime-modal-error{padding:32px;text-align:center}.runtime-modal-error{display:grid;gap:6px;color:#a11}@media(max-width:700px){.runtime-modal-backdrop{padding:10px}.runtime-modal{max-height:94vh}.runtime-modal-meta{grid-template-columns:1fr}.runtime-modal>header,.runtime-modal>footer,.runtime-modal-body{padding:16px}}.publication-info-primary{grid-column:span 2}.publication-info-wide{grid-column:span 2}.publication-manager-resource{display:grid;gap:18px}.publication-alias,.publication-active-card,.publication-versions{margin:0!important}.publication-active-card{padding:18px!important}.publication-manager-resource>.cm-actions{margin:0;display:flex;align-items:center;gap:10px}.publication-site-actions{justify-content:flex-start}.publication-new-version{display:flex;justify-content:flex-end;margin-top:14px;padding-top:14px;border-top:1px solid var(--cif-border)}.runtime-info-page{display:grid;gap:18px}.runtime-info-head{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}.runtime-info-head span{font-size:.7rem;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--cif-muted)}.runtime-info-head h2{margin:.25rem 0}.runtime-info-head p{margin:0;color:var(--cif-muted)}.runtime-info-page pre{margin:0;padding:20px;border-radius:14px;background:#0b1c12;color:#d8f7df;overflow:auto;max-height:70vh;white-space:pre-wrap;word-break:break-word}.runtime-info-error,.terminal-unavailable{max-width:720px;margin:7vh auto}.project-service-grid.publication-information{grid-template-columns:repeat(3,minmax(0,1fr))}.project-service-grid.publication-information>article{display:grid;gap:5px;padding:14px;border:1px solid var(--cif-border);border-radius:12px;min-width:0;background:var(--cif-surface)}
...
@media(max-width:900px){.publication-information{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.publication-information{grid-template-columns:1fr}.publication-info-primary,.publication-info-wide{grid-column:auto}.project-service-grid.publication-information{grid-template-columns:1fr}.technology-wizard-steps{grid-template-columns:1fr 1fr}}
...
#cloudif-environment-wizard{position:fixed!important;inset:0!important;z-index:1600!important;display:none;place-items:center;padding:28px;background:rgba(8,15,11,.64);backdrop-filter:blur(5px);overflow:auto}#cloudif-environment-wizard>.env-wizard-box{width:min(900px,100%);max-height:94vh;overflow:auto;margin:auto!important;padding:0;border:1px solid var(--c-border,var(--cif-border));border-radius:20px;background:var(--c-surface,var(--cif-surface));box-shadow:0 30px 90px rgba(0,0,0,.30)}#cloudif-environment-wizard .env-wizard-head{position:sticky;top:0;z-index:5;display:flex;justify-content:space-between;gap:20px;align-items:flex-start;padding:24px 28px 18px;border-bottom:1px solid var(--c-border,var(--cif-border));background:var(--c-surface,var(--cif-surface))}#cloudif-environment-wizard .env-wizard-head h3{margin:0;font-size:1.45rem;letter-spacing:-.02em}#cloudif-environment-wizard .env-wizard-head p{max-width:660px;margin:7px 0 0;color:var(--c-muted,var(--cif-muted));line-height:1.55}#cloudif-environment-wizard .env-wizard-close{display:grid;place-items:center;width:38px;height:38px;border:1px solid var(--c-border,var(--cif-border));border-radius:50%;background:transparent;font-size:1.45rem;line-height:1;cursor:pointer}#cloudif-environment-wizard .env-wizard-shell{padding:0 28px 28px}#cloudif-environment-wizard .env-wizard-section{margin-top:18px;padding:20px;border:1px solid var(--c-border,var(--cif-border));border-radius:14px;background:var(--c-surface,var(--cif-surface))}#cloudif-environment-wizard .env-wizard-section__head{margin-bottom:14px}#cloudif-environment-wizard .env-wizard-section__head h4{margin:0 0 4px;font-size:1rem}#cloudif-environment-wizard .env-wizard-section__head p{margin:0;color:var(--c-muted,var(--cif-muted));font-size:.8rem;line-height:1.5}#cloudif-environment-wizard .env-wizard-footer{position:sticky;bottom:0;z-index:4;display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;margin:20px -28px -28px;padding:16px 28px;border-top:1px solid var(--c-border,var(--cif-border));background:var(--c-surface,var(--cif-surface))}@media(max-width:560px){#cloudif-environment-wizard{padding:8px}#cloudif-environment-wizard>.env-wizard-box{max-height:97vh;border-radius:14px}#cloudif-environment-wizard .env-wizard-head{padding:18px}#cloudif-environment-wizard .env-wizard-shell{padding:0 18px 18px}#cloudif-environment-wizard .env-wizard-footer{margin:20px -18px -18px;padding:14px 18px;display:grid;grid-template-columns:1fr}#cloudif-environment-wizard .env-wizard-footer .btn{width:100%!important}}.publication-configuration{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.publication-configuration>article{display:grid;align-content:start;gap:8px;padding:16px;border:1px solid var(--cif-border);border-radius:12px;background:var(--cif-surface)}.publication-configuration>article>span{font-size:.7rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--cif-muted)}.publication-configuration p{margin:0;color:var(--cif-muted);font-size:.8rem;line-height:1.5}.publication-configuration form{margin-top:4px}.env-wizard-box{width:min(900px,100%)!important}.env-wizard-shell{display:grid;gap:18px}.env-wizard-toolbar{display:flex;align-items:end;justify-content:space-between;gap:14px}.env-wizard-toolbar .pm-field{min-width:210px}.env-revision{font-size:.78rem;color:var(--c-muted,#64748b)}.env-list{display:grid;gap:10px}.env-row{display:grid;grid-template-columns:minmax(180px,.85fr) minmax(240px,1.35fr) auto;gap:10px;align-items:end;padding:14px;border:1px solid var(--c-border,#dce3ed);border-radius:12px;background:var(--c-surface,#fff)}.env-row .pm-field{margin:0}.env-row code{font-size:.82rem}.env-row.secret{align-items:center;background:var(--c-surface-2,#f1f5f9)}.env-type{font-size:.72rem;color:var(--c-muted,#64748b)}.env-empty{padding:28px;border:1px dashed var(--c-border-strong,#c7d2e0);border-radius:12px;text-align:center;color:var(--c-muted,#64748b);background:var(--c-surface-2,#f1f5f9)}.env-status{padding:12px 14px;border-radius:10px;background:var(--c-info-soft,#eff6ff);color:var(--c-info,#1d4ed8);font-size:.82rem}.env-status.bad{background:var(--c-danger-soft,#fef2f2);color:var(--c-danger,#b91c1c)}.env-status.pending{background:var(--c-warning-soft,#fffbeb);color:var(--c-warning,#b45309)}#cloudif-environment-wizard .env-wizard-close{background:transparent!important;color:var(--c-text-strong,#0f172a)!important;border-color:var(--c-border,#dce3ed)!important;box-shadow:none!important}#cloudif-environment-wizard .env-wizard-footer .btn{min-width:0!important}#cloudif-environment-wizard .env-wizard-footer [data-env-save]{background:var(--iff,#168821)!important;color:#fff!important;border-color:var(--iff,#168821)!important}#cloudif-environment-wizard .env-wizard-footer [data-env-add],#cloudif-environment-wizard .env-wizard-footer [data-env-cancel]{background:var(--c-surface-2,#f1f5f9)!important;color:var(--c-text-strong,#0f172a)!important;border-color:var(--c-border,#dce3ed)!important}html[data-theme="dark"] #cloudif-environment-wizard{--c-bg:var(--cif-bg);--c-surface:var(--cif-surface);--c-surface-2:#0b1510;--c-surface-3:#173024;--c-text:var(--cif-text);--c-text-strong:var(--cif-text);--c-muted:var(--cif-muted);--c-faint:var(--cif-muted);--c-border:var(--cif-border);--c-border-strong:#345545;--surface:var(--cif-surface);--paper:var(--cif-bg);--ink:var(--cif-text);--rule:var(--cif-border)}html[data-theme="dark"] #cloudif-environment-wizard .wizard-box,html[data-theme="dark"] #cloudif-environment-wizard .env-wizard-head,html[data-theme="dark"] #cloudif-environment-wizard .env-wizard-shell,html[data-theme="dark"] #cloudif-environment-wizard .env-wizard-section,html[data-theme="dark"] #cloudif-environment-wizard .env-wizard-footer{background:var(--cif-surface)!important;color:var(--cif-text)!important;border-color:var(--cif-border)!important}html[data-theme="dark"] #cloudif-environment-wizard .env-row,html[data-theme="dark"] #cloudif-environment-wizard .env-empty{background:#0b1510!important;color:var(--cif-text)!important;border-color:#345545!important}html[data-theme="dark"] #cloudif-environment-wizard .env-wizard-close{background:transparent!important;color:var(--cif-text)!important;border-color:#345545!important}html[data-theme="dark"] #cloudif-environment-wizard .env-wizard-footer [data-env-add],html[data-theme="dark"] #cloudif-environment-wizard .env-wizard-footer [data-env-cancel]{background:#0b1510!important;color:var(--cif-text)!important;border-color:#345545!important}.project-tabs button{background:var(--cif-surface)!important;color:var(--cif-text)!important;border-color:var(--cif-border)!important;box-shadow:none!important}.project-tabs button:hover{background:color-mix(in srgb,var(--cif-surface) 88%,#176b35)!important;color:var(--cif-text)!important}.project-tabs button[aria-selected="true"]{background:#176b35!important;color:#fff!important;border-color:#176b35!important}html[data-theme="dark"] .project-tabs button{background:#0b1510!important;color:var(--cif-text)!important;border-color:#345545!important}html[data-theme="dark"] .project-tabs button:hover{background:#173024!important}html[data-theme="dark"] .project-tabs button[aria-selected="true"]{background:#176b35!important;color:#fff!important;border-color:#2b8a4e!important}@media(max-width:700px){.env-wizard-toolbar{display:grid;align-items:stretch}.env-wizard-toolbar .pm-field{min-width:0}.env-row{grid-template-columns:1fr}.env-row>.btn{width:100%!important}}
</style>
"""
...
(function(){
 ...
 const cloudifKomodoOrigin='https://komodoiff.duckdns.org';
 function cloudifKomodoLoginUrl(value){const target=new URL(String(value||cloudifKomodoOrigin+'/'),location.origin);if(target.protocol!=='https:'||target.hostname!=='komodoiff.duckdns.org')throw new Error('Destino Komodo inválido.');if(target.pathname==='/auth/oidc/login')return target.href;const login=new URL('/auth/oidc/login',cloudifKomodoOrigin);login.searchParams.set('redirect',target.href);return login.href}
 window.cloudifKomodoLoginUrl=cloudifKomodoLoginUrl;
 function cloudifRewriteKomodoAnchor(anchor){try{const target=new URL(anchor.getAttribute('href')||'',location.href);if(target.hostname==='komodoiff.duckdns.org'&&target.pathname!=='/auth/oidc/login')anchor.href=cloudifKomodoLoginUrl(target.href)}catch(_){}}
 queueMicrotask(()=>document.querySelectorAll('a[href*="komodoiff.duckdns.org"]').forEach(cloudifRewriteKomodoAnchor));
 document.addEventListener('click',event=>{const anchor=event.target.closest&&event.target.closest('a[href]');if(anchor)cloudifRewriteKomodoAnchor(anchor)},true);
 ...
 const envLayer=document.createElement('div');envLayer.className='cloudif-wizard';envLayer.id='cloudif-environment-wizard';envLayer.setAttribute('role','dialog');envLayer.setAttribute('aria-modal','true');envLayer.setAttribute('aria-labelledby','env-wizard-title');envLayer.innerHTML='<section class="wizard-box env-wizard-box"><header class="wizard-head env-wizard-head"><div><span class="env-type">Configuração versionada</span><h3 id="env-wizard-title">Variáveis de ambiente</h3><p>Gerencie as variáveis do projeto sem expor valores secretos.</p><p class="env-type" data-env-project></p></div><button type="button" class="wizard-close env-wizard-close" aria-label="Fechar">×</button></header><div class="pm-new-shell env-wizard-shell"><div class="env-wizard-toolbar"><label class="pm-field"><span>Ambiente</span><select data-env-environment><option value="preview">Preview</option><option value="homologation">Homologação</option><option value="production" selected>Produção</option></select><small>Escolha em qual ambiente a configuração será aplicada.</small></label><span class="env-revision" data-env-revision></span></div><section class="pm-new-section env-wizard-section"><div class="pm-new-section__head"><h4>Variáveis</h4><p>Valores públicos podem ser editados aqui. Secrets permanecem protegidos.</p></div><div class="env-list" data-env-list></div><div class="env-status" data-env-status hidden></div></section><footer class="pm-new-footer env-wizard-footer"><button type="button" class="btn light" data-env-cancel>Cancelar</button><button type="button" class="btn light" data-env-add>+ Nova variável</button><button type="button" class="btn" data-env-save>Salvar alterações</button></footer></div></section>';const envMount=document.querySelector('.legacy-content')||document.body;envMount.appendChild(envLayer);
 const envModel={slug:'',environment:'production',revision:0,rows:[],original:[]};
 const envEsc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 const envList=envLayer.querySelector('[data-env-list]'),envStatus=envLayer.querySelector('[data-env-status]'),envRevision=envLayer.querySelector('[data-env-revision]'),envSelect=envLayer.querySelector('[data-env-environment]');
 function envMessage(text,kind=''){envStatus.hidden=!text;envStatus.textContent=text||'';envStatus.className='env-status'+(kind?' '+kind:'')}
 function envClose(){envLayer.style.display='none';document.body.classList.remove('cloudif-modal-open');if(document.getElementById('cloudif-publication-environments')?.style.display==='grid')document.body.classList.add('cloudif-modal-open');envMessage('')}
 function envPublicRow(item={}){return {name:String(item.name||'').toUpperCase(),value:item.value==null?'':String(item.value),service:String(item.service||''),secret:false,new:Boolean(item.new)}}
 function envDraw(){let html=envModel.rows.map((x,index)=>x.secret?'<div class="env-row secret"><div><code>'+envEsc(x.name)+'</code><div class="env-type">Secret protegido · '+envEsc(x.service||'Projeto')+'</div></div><strong>••••••••</strong><span class="env-type">Use o fluxo de Secrets para substituir ou revogar.</span></div>':'<div class="env-row" data-env-public-row data-model-index="'+index+'" data-env-service="'+envEsc(x.service||'')+'"><label class="pm-field"><span>Nome</span><input aria-label="Nome da variável" data-env-name value="'+envEsc(x.name)+'" placeholder="NOME_DA_VARIAVEL"><small>'+envEsc(x.service?'Serviço '+x.service:'Projeto')+'</small></label><label class="pm-field"><span>Valor</span><input aria-label="Valor" data-env-value value="'+envEsc(x.value)+'" placeholder="valor"></label><button type="button" class="btn light" data-env-delete>Excluir</button></div>').join('');envList.innerHTML=html||'<div class="env-empty">Nenhuma variável configurada neste ambiente.</div>';envList.querySelectorAll('[data-env-public-row]').forEach(row=>{row.querySelector('[data-env-delete]').onclick=()=>{envModel.rows.splice(Number(row.dataset.modelIndex),1);envDraw()}});envRevision.textContent='Revisão '+envModel.revision+' · valores secretos nunca são carregados'}
 async function envFetch(url,options={}){const response=await fetch(url,{credentials:'same-origin',...options});const data=await response.json().catch(()=>({ok:false,error:{code:'invalid_response'}}));if(!response.ok||!data.ok){const err=(data.error&&((data.error.message||data.error.code)))||data.error||('HTTP '+response.status);throw new Error(String(err))}return data}
 async function envLoad(){envMessage('Carregando…');envList.innerHTML='';try{const data=await envFetch('/cloudiff/portal/api/projects/'+encodeURIComponent(envModel.slug)+'/environment?environment='+encodeURIComponent(envModel.environment)+'&includePublicValues=true');envModel.revision=Number(data.revision||0);envModel.rows=(data.entries||[]).map(x=>x.secret?{name:x.name,service:x.service||'',secret:true}:envPublicRow(x));envModel.original=envModel.rows.filter(x=>!x.secret).map(x=>({...x}));envMessage('');envDraw()}catch(error){envMessage('Não foi possível carregar as variáveis: '+error.message,'bad')}}
 function envCollect(){const current=[];envList.querySelectorAll('[data-env-public-row]').forEach(row=>{const name=(row.querySelector('[data-env-name]').value||'').trim().toUpperCase(),value=row.querySelector('[data-env-value]').value,service=row.dataset.envService||'';if(name)current.push({name,value,service})});const before=new Map(envModel.original.map(x=>[x.service+'|'+x.name,x])),after=new Map(current.map(x=>[x.service+'|'+x.name,x])),changes=[];for(const [key,item] of before){if(!after.has(key))changes.push({operation:'delete',name:item.name,service:item.service})}for(const [key,item] of after){const old=before.get(key);if(!old||old.value!==item.value)changes.push({operation:'upsert',name:item.name,service:item.service,value:item.value,definition:{secret:false,runtime:true,buildTime:false,restartRequired:true}})}return changes}
 async function envPost(operation,payload){const csrf=(document.querySelector('input[name="csrf_token"]')||{}).value||'';return envFetch('/cloudiff/portal/api/projects/'+encodeURIComponent(envModel.slug)+'/environment/'+operation,{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify(payload)})}
 async function envExecute(planDigest,approvalId){envMessage('Aplicando a revisão aprovada…','pending');const result=await envPost('change/execute',{planDigest,approvalId});envMessage('Revisão '+result.revision+' salva. Reconciliação '+((result.reconcile||{}).status||'enfileirada')+'.');await envLoad()}
 async function envWaitApproval(planDigest,approval){envMessage('Aguardando aprovação humana. Você pode aprovar na aba Aprovações; este wizard continuará acompanhando.','pending');const deadline=Date.now()+10*60*1000;while(envLayer.style.display!=='none'&&Date.now()<deadline){await new Promise(r=>setTimeout(r,2500));const status=await envFetch('/cloudiff/portal/api/projects/'+encodeURIComponent(envModel.slug)+'/environment/approval/status?approvalId='+encodeURIComponent(approval.approvalId));if(status.status==='approved')return envExecute(planDigest,approval.approvalId);if(['rejected','expired'].includes(status.status))throw new Error('A aprovação foi '+(status.status==='rejected'?'rejeitada':'expirada')+'.')}if(envLayer.style.display!=='none')throw new Error('Tempo de acompanhamento da aprovação encerrado. Abra o wizard novamente para continuar.')}
 async function envSave(){const save=envLayer.querySelector('[data-env-save]');save.disabled=true;try{const changes=envCollect();if(!changes.length){envMessage('Nenhuma alteração para salvar.');return}envMessage('Validando e criando plano…','pending');const plan=await envPost('change/plan',{environment:envModel.environment,expectedRevision:envModel.revision,changes});const approval=await envPost('approval/request',{planDigest:plan.planDigest,reason:'Alteração de variáveis pelo wizard do projeto'});if(approval.status==='approved')await envExecute(plan.planDigest,approval.approvalId);else await envWaitApproval(plan.planDigest,approval)}catch(error){envMessage(error.message||'Falha ao salvar variáveis.','bad')}finally{save.disabled=false}}
 envLayer.querySelector('.env-wizard-close').onclick=envClose;envLayer.querySelector('[data-env-cancel]').onclick=envClose;envLayer.querySelector('[data-env-add]').onclick=()=>{envModel.rows.push({name:'',value:'',service:'',secret:false,new:true});envDraw()};envLayer.querySelector('[data-env-save]').onclick=envSave;envSelect.onchange=()=>{envModel.environment=envSelect.value;envLoad()};envLayer.addEventListener('click',event=>{if(event.target===envLayer)envClose()});
 document.addEventListener('click',event=>{const button=event.target.closest('[data-env-wizard]');if(!button)return;const requested=['preview','homologation','production'].includes(button.dataset.envEnvironment)?button.dataset.envEnvironment:'production';envModel.slug=button.dataset.projectSlug||'';envModel.environment=requested;envSelect.value=requested;envLayer.querySelector('[data-env-project]').textContent=envModel.slug;envLayer.style.display='grid';document.body.classList.add('cloudif-modal-open');envLoad();setTimeout(()=>envLayer.querySelector('select,input,button')?.focus(),0)});
 const envLabels={preview:'Preview',homologation:'Homologação',production:'Produção'};
 const envStatusLabels={active:'Ativo',validated:'Validado',expired:'Expirado',not_created:'Não criado',not_deployed:'Não publicado',not_published:'Não publicado',running:'Online',published:'Publicado',blocked:'Bloqueado',failed:'Falhou',removed:'Removido',unknown:'Desconhecido'};
 const publicationTools=['overview','site','php','node','terminal','variables'];
 const publicationEnvironmentLayer=document.createElement('div');
 publicationEnvironmentLayer.className='cloudif-wizard publication-environments-wizard';
 publicationEnvironmentLayer.id='cloudif-publication-environments';
 publicationEnvironmentLayer.setAttribute('role','dialog');
 publicationEnvironmentLayer.setAttribute('aria-modal','true');
 publicationEnvironmentLayer.setAttribute('aria-labelledby','publication-environments-title');
 publicationEnvironmentLayer.innerHTML='<section class="wizard-box publication-environments-box"><header class="wizard-head publication-environments-head"><div><span>Publicação</span><h3 id="publication-environments-title">Ambiente de publicação</h3><p class="publication-environments-project" data-publication-environments-project></p></div><button type="button" class="wizard-close publication-environments-close" aria-label="Fechar">×</button></header><nav class="publication-environments-tabs" role="tablist" aria-label="Ambientes"><button type="button" role="tab" data-publication-env-tab="preview">Preview</button><button type="button" role="tab" data-publication-env-tab="homologation">Homologação</button><button type="button" role="tab" data-publication-env-tab="production">Produção</button></nav><nav class="publication-workspace-tools" aria-label="Ferramentas do ambiente"><button type="button" data-publication-tool="overview">Visão geral</button><button type="button" data-publication-tool="site">Site</button><button type="button" data-publication-tool="php">PHP</button><button type="button" data-publication-tool="node">Node.js</button><button type="button" data-publication-tool="terminal">Terminal</button><button type="button" data-publication-tool="variables">Variáveis</button></nav><div class="publication-environments-body" data-publication-environments-body><div class="publication-environment-loading">Carregando ambiente…</div></div><footer class="publication-environments-footer"><button type="button" class="btn light" data-publication-environments-refresh>Atualizar</button><button type="button" class="btn light" data-publication-environments-close>Fechar</button></footer></section>';
 const publicationEnvironmentMount=document.querySelector('.legacy-content')||document.body;
 publicationEnvironmentMount.appendChild(publicationEnvironmentLayer);
 const publicationEnvironmentModel={slug:'',active:'production',tool:'overview',data:null,release:null,csrf:'',toolRequest:0,loadRequest:0,opener:null};
 const publicationEnvironmentBody=publicationEnvironmentLayer.querySelector('[data-publication-environments-body]');
 const publicationEnvironmentProject=publicationEnvironmentLayer.querySelector('[data-publication-environments-project]');
 function publicationEnvironmentLoading(text='Carregando ambiente…'){publicationEnvironmentBody.innerHTML='<div class="publication-environment-loading">'+envEsc(text)+'</div>'}
 function publicationEnvironmentClose(){publicationEnvironmentLayer.style.display='none';publicationEnvironmentModel.toolRequest++;publicationEnvironmentModel.loadRequest++;document.body.classList.remove('cloudif-modal-open');const opener=publicationEnvironmentModel.opener;publicationEnvironmentModel.opener=null;if(opener&&document.contains(opener))opener.focus()}
 function publicationEnvironmentUpdateTabs(){publicationEnvironmentLayer.querySelectorAll('[data-publication-env-tab]').forEach(button=>{const selected=button.dataset.publicationEnvTab===publicationEnvironmentModel.active;button.setAttribute('aria-selected',String(selected));button.classList.toggle('is-active',selected)})}
 function publicationEnvironmentUpdateTools(){publicationEnvironmentLayer.querySelectorAll('[data-publication-tool]').forEach(button=>{const selected=button.dataset.publicationTool===publicationEnvironmentModel.tool;button.setAttribute('aria-pressed',String(selected));button.classList.toggle('is-active',selected)})}
 function publicationEnvironmentSelect(name){publicationEnvironmentModel.active=['preview','homologation','production'].includes(name)?name:'production';publicationEnvironmentUpdateTabs();publicationEnvironmentRender()}
 function publicationEnvironmentToolSelect(name){publicationEnvironmentModel.tool=publicationTools.includes(name)?name:'overview';publicationEnvironmentUpdateTools();publicationEnvironmentRender()}
 function publicationSafeSiteUrl(value){try{const target=new URL(String(value||''),location.origin);const allowed=target.origin===location.origin||target.hostname.endsWith('.cloudiff.duckdns.org');return allowed&&target.protocol==='https:'?target.href:(target.origin===location.origin?target.href:'')}catch(_){return ''}}
 function publicationStageContext(){
   const release=publicationEnvironmentModel.release||{},environment=publicationEnvironmentModel.active,overview=publicationEnvironmentModel.data||{},fallback=(overview.environments||[]).find(x=>x.name===environment)||{},publicNumber=Number(release.publicNumber||0);
   ...
   if(!active)return {environment,label:'Produção',stageCode:'P',url:publicationSafeSiteUrl(fallback.url||''),container:'',status:'Sem publicação',exists:Boolean(fallback.url)};
   const publication=Number(active.publication_number||0),deploy=Number(active.deploy_number||publication||0),isLegacy=!('candidate_number' in active),container=publicNumber?(isLegacy&&deploy?'cloudif-p'+publicNumber+'-d'+deploy+'-web':publication?'cloudif-p'+publicNumber+'-p'+publication+'-publication-web':''):'';
   return {environment,label:'Produção',stageCode:active.stage_code||('P'+publication),url:publicationSafeSiteUrl(active.stableUrl||active.url||fallback.url||''),container,status:String(active.status||'Publicado'),exists:true};
 }
 function publicationEnvironmentOverview(){
   const data=publicationEnvironmentModel.data,item=(data.environments||[]).find(x=>x.name===publicationEnvironmentModel.active)||{},ctx=publicationStageContext(),name=item.name||publicationEnvironmentModel.active,label=envLabels[name]||name,status=envStatusLabels[item.status]||item.status||ctx.status||'—',cfg=item.configuration||{},runtime=item.runtime||{},artifact=String(item.artifact||''),artifactLabel=item.deployNumber?'d'+item.deployNumber:(artifact?artifact.slice(0,12):'—'),missing=Number(cfg.missingRequired||0),detail=cfg.valid===true?'Configuração válida':(missing?missing+' obrigatória(s) pendente(s)':'Configuração ainda não validada'),expires=item.expiresAt?new Date(Number(item.expiresAt)*1000).toLocaleString('pt-BR'):'',url=ctx.url||publicationSafeSiteUrl(item.url||''),isOk=['active','running','published'].includes(item.status)||/online|publicado/i.test(ctx.status||''),open=url?'<button type="button" class="btn" data-publication-tool-jump="site">Visualizar '+envEsc(label)+'</button>':'<span class="publication-environment-hint">Ainda não há URL pública para este ambiente.</span>',variables=data.canWrite?'<button type="button" class="btn light" data-publication-tool-jump="variables">Ver variáveis</button>':'';
   publicationEnvironmentBody.innerHTML='<article class="publication-environment-detail"><div class="publication-environment-status"><div><span>'+envEsc(label)+'</span><h4>'+envEsc(status)+'</h4></div><span class="environment-state '+(isOk?'is-ok':'')+'">'+envEsc(status)+'</span></div><dl><div><dt>Configuração</dt><dd>r'+Number(cfg.revision||0)+'</dd></div><div><dt>Validação</dt><dd>'+envEsc(detail)+'</dd></div><div><dt>Artefato</dt><dd><code>'+envEsc(artifactLabel)+'</code></dd></div>'+(ctx.stageCode?'<div><dt>Estágio</dt><dd><code>'+envEsc(ctx.stageCode)+'</code></dd></div>':'')+(runtime.status?'<div><dt>Runtime</dt><dd>'+envEsc(envStatusLabels[runtime.status]||runtime.status)+'</dd></div>':'')+(expires?'<div><dt>Expira</dt><dd>'+envEsc(expires)+'</dd></div>':'')+'</dl><div class="publication-environment-actions">'+open+variables+'</div></article>';
 }
 function publicationSiteRender(){
   const ctx=publicationStageContext();
   if(!ctx.url){publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>'+envEsc(ctx.label)+' ainda não possui URL disponível.</strong><span>Prepare ou publique este estágio antes de abrir o preview do site.</span></div>';return}
   publicationEnvironmentBody.innerHTML='<section class="stage-site-preview"><header><div><span>'+envEsc(ctx.stageCode)+' · '+envEsc(ctx.label)+'</span><strong>Preview do site</strong><small>'+envEsc(ctx.url)+'</small></div><a class="btn light" href="'+envEsc(ctx.url)+'" target="_blank" rel="noopener">Abrir em nova aba</a></header><div class="stage-site-preview__frame"><iframe src="'+envEsc(ctx.url)+'" title="Site '+envEsc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" sandbox="allow-forms allow-modals allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts allow-downloads"></iframe></div><footer>O preview usa a URL real do estágio. Se a aplicação definir uma política própria que impeça incorporação, use “Abrir em nova aba”.</footer></section>';
 }
 async function publicationRuntimeLoad(kind,requestId){
   const label=kind==='php'?'PHP':'Node.js';
   publicationEnvironmentLoading('Consultando '+label+'…');
   try{
     const response=await fetch('/cloudiff/portal/api/project-runtime-info?slug='+encodeURIComponent(publicationEnvironmentModel.slug)+'&kind='+encodeURIComponent(kind),{credentials:'same-origin',headers:{Accept:'application/json','Cache-Control':'no-store'}}),data=await response.json();
     if(requestId!==publicationEnvironmentModel.toolRequest)return;
     if(!response.ok||!data.ok)throw new Error(data.error||'runtime_info_failed');
     const title=kind==='php'?'Informações do PHP':'Informações do Node.js',description=kind==='php'?'Versão, configuração e extensões carregadas.':'Runtime, npm e dependências declaradas da API.',standalone='/cloudiff/portal/action/project-runtime-info?slug='+encodeURIComponent(publicationEnvironmentModel.slug)+'&kind='+encodeURIComponent(kind);
     publicationEnvironmentBody.innerHTML='<section class="publication-runtime-preview"><header><div><span>Diagnóstico autenticado</span><h4>'+envEsc(title)+'</h4><p>'+envEsc(description)+'</p></div><a class="publication-runtime-newtab" href="'+envEsc(standalone)+'" target="_blank" rel="noopener">Abrir em nova aba</a></header><div class="publication-runtime-preview-meta"><div><span>Projeto</span><strong>'+envEsc(publicationEnvironmentModel.slug)+'</strong></div><div><span>Container</span><strong>'+envEsc(data.container||'—')+'</strong></div></div><pre>'+envEsc(data.output||'')+'</pre></section>';
   }catch(error){if(requestId!==publicationEnvironmentModel.toolRequest)return;publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>Não foi possível consultar '+envEsc(label)+'.</strong><span>Tente atualizar o diagnóstico.</span></div>'}
 }
 function publicationTerminalRender(requestId){
   const ctx=publicationStageContext(),release=publicationEnvironmentModel.release||{},allowed=ctx.environment==='production'?Boolean(release.canPublish):Boolean((publicationEnvironmentModel.data||{}).canWrite);
   if(!ctx.exists){publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>'+envEsc(ctx.label)+' ainda não possui container correspondente.</strong><span>Prepare este estágio antes de abrir o terminal.</span></div>';return}
   if(!allowed){publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>Terminal não autorizado.</strong><span>'+(ctx.environment==='production'?'Produção exige o responsável pelo projeto ou administrador.':'Você possui acesso de leitura, mas não permissão para abrir o terminal.')+'</span></div>';return}
   publicationTerminalOpen(null,requestId);
 }
 async function publicationTerminalOpen(button=null,requestId=publicationEnvironmentModel.toolRequest){
   if(button&&button.disabled)return;if(button)button.disabled=true;const ctx=publicationStageContext();publicationEnvironmentLoading('Preparando terminal de '+ctx.label+'…');
   try{
     const response=await fetch('/cloudiff/portal/api/projects/'+encodeURIComponent(publicationEnvironmentModel.slug)+'/release-flow/stage/terminal',{method:'POST',credentials:'same-origin',headers:{Accept:'application/json','Content-Type':'application/json','X-CSRF-Token':publicationEnvironmentModel.csrf},body:JSON.stringify({environment:publicationEnvironmentModel.active})}),data=await response.json().catch(()=>({ok:false,error:{message:'Resposta inválida da plataforma.'}}));
     if(requestId!==publicationEnvironmentModel.toolRequest)return;
     if(!response.ok||!data.ok)throw new Error((data.error&&((data.error.message||data.error.code)))||data.error||'Terminal indisponível.');
     const target=new URL(String(data.terminalUrl||''),location.origin);if(target.protocol!=='https:'||target.hostname!=='komodoiff.duckdns.org'||!target.pathname.includes('/terminal/'))throw new Error('A plataforma retornou um terminal inválido.');
     const embed=new URL(target.href);embed.searchParams.set('_cloudiff_embed',String(Date.now()));const embedLogin=cloudifKomodoLoginUrl(embed.href),externalLogin=cloudifKomodoLoginUrl(target.href);
     publicationEnvironmentBody.innerHTML='<section class="stage-site-preview stage-terminal-embed"><header><div><span>'+envEsc(data.stageCode||ctx.stageCode)+' · '+envEsc(ctx.label)+'</span><strong>Terminal do container</strong><small>'+envEsc(data.container||ctx.container||'')+'</small></div><a class="btn light" href="'+envEsc(externalLogin)+'" target="_blank" rel="noopener">Abrir no Komodo</a></header><div class="stage-site-preview__frame stage-terminal-embed__frame"><iframe src="'+envEsc(embedLogin)+'" title="Terminal '+envEsc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" allow="clipboard-read; clipboard-write; fullscreen" allowfullscreen></iframe></div><footer>SSO OIDC iniciado diretamente e terminal carregado no container deste estágio. “Abrir no Komodo” preserva o mesmo destino.</footer></section>';
   }catch(error){if(requestId!==publicationEnvironmentModel.toolRequest)return;publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>Terminal não aberto.</strong><span>'+envEsc(error.message||'Não foi possível preparar o terminal.')+'</span><button type="button" class="btn light" data-publication-terminal-open>Tentar novamente</button></div>'}finally{if(button)button.disabled=false}
 }
 async function publicationVariablesLoad(requestId){
   publicationEnvironmentLoading('Carregando variáveis…');
   try{
     const response=await fetch('/cloudiff/portal/api/projects/'+encodeURIComponent(publicationEnvironmentModel.slug)+'/environment?environment='+encodeURIComponent(publicationEnvironmentModel.active)+'&includePublicValues=true',{credentials:'same-origin',headers:{Accept:'application/json','Cache-Control':'no-store'}}),data=await response.json();
     if(requestId!==publicationEnvironmentModel.toolRequest)return;
     if(!response.ok||!data.ok)throw new Error((data.error&&((data.error.message||data.error.code)))||data.error||'environment_failed');
     const rows=(data.entries||[]).map(item=>'<div class="publication-variable-row"><div><code>'+envEsc(item.name||'')+'</code><span>'+envEsc(item.service||'Projeto')+'</span></div><strong>'+(item.secret?'••••••••':envEsc(item.value==null?'':item.value))+'</strong><small>'+(item.secret?'Secret protegido':'Valor público')+'</small></div>').join('');
     const canWrite=Boolean((publicationEnvironmentModel.data||{}).canWrite),edit=canWrite?'<button type="button" class="btn" data-env-wizard data-project-slug="'+envEsc(publicationEnvironmentModel.slug)+'" data-env-environment="'+envEsc(publicationEnvironmentModel.active)+'">Editar variáveis</button>':'';
     publicationEnvironmentBody.innerHTML='<section class="publication-variables-preview"><header><div><span>Configuração</span><h4>Variáveis de '+envEsc(envLabels[publicationEnvironmentModel.active]||publicationEnvironmentModel.active)+'</h4><p>Secrets permanecem ocultos. Valores públicos podem ser revisados aqui.</p></div><span class="publication-variable-revision">r'+Number(data.revision||0)+'</span></header><div class="publication-variable-list">'+(rows||'<div class="publication-variable-empty">Nenhuma variável configurada neste ambiente.</div>')+'</div><footer>'+edit+'</footer></section>';
   }catch(error){if(requestId!==publicationEnvironmentModel.toolRequest)return;publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>Não foi possível carregar as variáveis.</strong><span>Tente atualizar o ambiente.</span></div>'}
 }
 function publicationEnvironmentRender(){
   const requestId=++publicationEnvironmentModel.toolRequest;
   if(!publicationEnvironmentModel.data){publicationEnvironmentLoading();return}
   if(publicationEnvironmentModel.tool==='overview')return publicationEnvironmentOverview();
   if(publicationEnvironmentModel.tool==='site')return publicationSiteRender();
   if(publicationEnvironmentModel.tool==='php'||publicationEnvironmentModel.tool==='node')return publicationRuntimeLoad(publicationEnvironmentModel.tool,requestId);
   if(publicationEnvironmentModel.tool==='terminal')return publicationTerminalRender(requestId);
   if(publicationEnvironmentModel.tool==='variables')return publicationVariablesLoad(requestId);
 }
 async function publicationEnvironmentLoad(){
   const requestId=++publicationEnvironmentModel.loadRequest,slug=publicationEnvironmentModel.slug;
   publicationEnvironmentModel.data=null;publicationEnvironmentModel.release=null;publicationEnvironmentModel.csrf='';
   publicationEnvironmentRender();
   try{
     const response=await fetch('/cloudiff/portal/api/projects/'+encodeURIComponent(slug)+'/environments-overview',{credentials:'same-origin',headers:{Accept:'application/json','Cache-Control':'no-store'}}),data=await response.json();
     if(requestId!==publicationEnvironmentModel.loadRequest||slug!==publicationEnvironmentModel.slug)return;
     if(!response.ok||!data.ok)throw new Error((data.error||{}).message||'environment_overview_failed');
     publicationEnvironmentModel.data=data;
     try{const releaseResponse=await fetch('/cloudiff/portal/api/projects/'+encodeURIComponent(slug)+'/release-flow',{credentials:'same-origin',headers:{Accept:'application/json','Cache-Control':'no-store'}}),release=await releaseResponse.json();if(requestId===publicationEnvironmentModel.loadRequest&&releaseResponse.ok&&release.ok){publicationEnvironmentModel.release=release;publicationEnvironmentModel.csrf=String(release.csrfToken||'')}}catch(_){}
     publicationEnvironmentRender();
   }catch(error){if(requestId!==publicationEnvironmentModel.loadRequest||slug!==publicationEnvironmentModel.slug)return;publicationEnvironmentBody.innerHTML='<div class="publication-environment-error"><strong>Não foi possível consultar os ambientes.</strong><span>Tente novamente em alguns instantes.</span></div>'}
 }
 function publicationEnvironmentOpen(button){
   publicationEnvironmentModel.slug=button.dataset.projectSlug||'';
   publicationEnvironmentModel.active=['preview','homologation','production'].includes(button.dataset.environment)?button.dataset.environment:'production';
   publicationEnvironmentModel.tool=publicationTools.includes(button.dataset.publicationTool)?button.dataset.publicationTool:'overview';
   publicationEnvironmentModel.opener=button;
   publicationEnvironmentProject.textContent=publicationEnvironmentModel.slug;
   publicationEnvironmentUpdateTabs();publicationEnvironmentUpdateTools();
   publicationEnvironmentLayer.style.display='grid';document.body.classList.add('cloudif-modal-open');
   publicationEnvironmentLoad();
   setTimeout(()=>publicationEnvironmentLayer.querySelector('.publication-workspace-tools .is-active,[aria-selected="true"]')?.focus(),0);
 }
 publicationEnvironmentLayer.querySelector('.publication-environments-close').onclick=publicationEnvironmentClose;
 publicationEnvironmentLayer.querySelector('[data-publication-environments-close]').onclick=publicationEnvironmentClose;
 publicationEnvironmentLayer.querySelector('[data-publication-environments-refresh]').onclick=publicationEnvironmentLoad;
 publicationEnvironmentLayer.querySelectorAll('[data-publication-env-tab]').forEach(button=>button.onclick=()=>publicationEnvironmentSelect(button.dataset.publicationEnvTab));
 publicationEnvironmentLayer.querySelectorAll('[data-publication-tool]').forEach(button=>button.onclick=()=>publicationEnvironmentToolSelect(button.dataset.publicationTool));
 publicationEnvironmentLayer.addEventListener('click',event=>{if(event.target===publicationEnvironmentLayer)publicationEnvironmentClose();const jump=event.target.closest('[data-publication-tool-jump]');if(jump)publicationEnvironmentToolSelect(jump.dataset.publicationToolJump);const terminal=event.target.closest('[data-publication-terminal-open]');if(terminal)publicationTerminalOpen(terminal)});
 document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;if(envLayer.style.display==='grid'){envClose();return}if(publicationEnvironmentLayer.style.display==='grid')publicationEnvironmentClose()});
 document.addEventListener('click',event=>{const button=event.target.closest('[data-publication-environments]');if(!button)return;event.preventDefault();publicationEnvironmentOpen(button)});
 const publicationDeepLink=new URLSearchParams(location.search),publicationDeepLinkProject=publicationDeepLink.get('project')||'',publicationDeepLinkOpen=publicationDeepLink.get('open')||'';
 if(publicationDeepLink.get('tab')==='publicacao'&&publicationDeepLinkProject&&publicationDeepLinkOpen==='variables'){
   const button=[...document.querySelectorAll('[data-publication-environments][data-publication-tool="variables"]')].find(x=>x.dataset.projectSlug===publicationDeepLinkProject);
   if(button)setTimeout(()=>publicationEnvironmentOpen(button),0);
 }
...
if 'Portal' in globals() and not globals().get('_cpx_get_wrapped'):
    ...
    def _cpx_do_GET(self):
        ...
        if path in ('/cloudif/portal/action/open-container-shell','/action/open-container-shell'):
            ...
            oidc=setting_value('CLOUDIF_KOMODO_OIDC_URL','https://komodoiff.duckdns.org/auth/oidc/login');target=oidc+('&' if '?' in oidc else '?')+_cpx_parse.urlencode({'redirect':target})
...
if 'Portal' in globals() and not globals().get('_rd_wrapped'):
    ...
    def _rd_get(self):
        ...
        if path in ('/cloudiff/portal/api/project-runtime-info','/cloudif/portal/api/project-runtime-info','/api/project-runtime-info'):
...
_WHP_RELEASE_ASSETS=r"""<script id="cloudif-whp-release-script">
(function(){
 const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 const layer=document.createElement('div');layer.id='cloudif-release-flow';layer.setAttribute('role','dialog');layer.setAttribute('aria-modal','true');layer.setAttribute('aria-labelledby','release-wizard-title');layer.innerHTML='<section class="release-wizard"><header><div><h2 id="release-wizard-title">Preview → Homologação → Publicação</h2><p data-release-project></p></div><button type="button" class="release-wizard-close" aria-label="Fechar">×</button></header><nav class="release-tabs" aria-label="Estágios"><button type="button" data-release-tab="preview">W · Preview</button><button type="button" data-release-tab="homologation">H · Homologação</button><button type="button" data-release-tab="publication">P · Publicação</button></nav><nav class="release-tools" aria-label="Ferramentas do estágio"><button type="button" data-release-view="stage">Etapa</button><button type="button" data-release-view="site">Site</button><button type="button" data-release-view="terminal">Terminal</button></nav><main class="release-wizard-body" data-release-body><p>Carregando fluxo…</p></main><footer class="release-wizard-footer"><span data-release-footer>W = Workspace Preview · H = Homologation · P = Publication</span><div><button type="button" class="btn light" data-release-refresh>Atualizar</button> <button type="button" class="btn light" data-release-close>Fechar</button></div></footer></section>';const mount=document.querySelector('.legacy-content')||document.body;mount.appendChild(layer);
 const model={slug:'',tab:'preview',view:'stage',data:null,csrf:'',approval:null,poll:null,busy:false,opener:null};const body=layer.querySelector('[data-release-body]');
 function apiBase(){return '/cloudiff/portal/api/projects/'+encodeURIComponent(model.slug)+'/release-flow'}
 async function call(url,options={}){const response=await fetch(url,{credentials:'same-origin',headers:{Accept:'application/json',...(options.headers||{})},...options}),data=await response.json().catch(()=>({ok:false,error:{message:'Resposta inválida da plataforma.'}}));if(!response.ok||!data.ok){const e=data.error||{};throw new Error(String(e.message||e.code||data.message||'Operação indisponível.'))}return data}
 async function post(op,payload={}){return call(apiBase()+'/'+op,{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':model.csrf},body:JSON.stringify(payload)})}
 function close(){layer.classList.remove('is-open');document.body.classList.remove('cloudif-modal-open');clearTimeout(model.poll);model.poll=null;const opener=model.opener;model.opener=null;if(opener&&document.contains(opener))opener.focus()}
 function tab(name){model.tab=['preview','homologation','publication'].includes(name)?name:'preview';layer.querySelectorAll('[data-release-tab]').forEach(x=>x.classList.toggle('is-active',x.dataset.releaseTab===model.tab));render()}
 function view(name){model.view=['stage','site','terminal'].includes(name)?name:'stage';layer.querySelectorAll('[data-release-view]').forEach(x=>x.classList.toggle('is-active',x.dataset.releaseView===model.view));render()}
 function jobHtml(){const j=model.data&&model.data.job;if(!j||!['queued','running'].includes(j.status))return '';const labels={homologation_candidate:'Preparando Homologação',production_release:'Publicando em Produção'};return '<div class="release-job"><strong>'+esc(labels[j.operation]||'Operação em andamento')+'</strong><span>'+esc(j.message||'Processando…')+'</span><progress></progress></div>'}
 function openLink(url,label){return url?'<a class="btn light" href="'+esc(url)+'" target="_blank" rel="noopener">'+esc(label)+'</a>':''}
 function envButton(environment){return '<button class="btn light" type="button" data-env-wizard data-project-slug="'+esc(model.slug)+'" data-env-environment="'+environment+'">Variáveis</button>'}
 function renderPreview(){const d=model.data,p=d.preview||{},git=p.git||{},configured=!!p.configured,healthy=!!p.healthy,code=p.stageCode||'W1',alert=!configured?'':(!git.remoteAvailable?'<div class="release-alert"><strong>Sincronização Git indisponível</strong><span>'+esc(git.message||'O Preview continua usando a cópia local. Tente novamente em alguns instantes ou procure o administrador.')+'</span></div>':git.status==='conflict'?'<div class="release-alert bad"><strong>Sincronização pendente</strong><span>'+esc(git.message||'Há alterações locais e remotas que precisam ser reconciliadas.')+'</span></div>':'<div class="release-alert ok"><strong>Workspace local disponível</strong><span>'+esc(git.message||'Preview pronto para desenvolvimento.')+'</span></div>');return jobHtml()+'<section class="release-stage"><div class="release-stage-head"><div><span class="release-kicker">'+esc(code)+' · Preview</span><h3>Desenvolvimento contínuo</h3><p>O código do workspace local está montado no container. Alterações aparecem no Preview sem criar uma versão.</p></div><span class="release-badge '+(healthy?'ok':'warn')+'">'+(configured?(healthy?'Online':'Atenção'):'Não preparado')+'</span></div>'+alert+'<div class="release-grid"><div class="release-box"><span>URL</span><strong>'+esc(p.url||'Será criada ao preparar W1')+'</strong></div><div class="release-box"><span>Git</span><strong>'+esc(git.status||'Não verificado')+'</strong><small>'+esc((git.head||'').slice(0,12))+'</small></div></div><div class="release-actions">'+openLink(p.url,'Abrir '+code)+(configured&&healthy&&d.canWrite?'<button class="btn light" type="button" data-release-action="preview-terminal">Abrir terminal do Preview</button>':'')+'<button class="btn" type="button" data-release-action="preview-ensure">'+(configured?'Sincronizar Preview':'Preparar Preview')+'</button>'+envButton('preview')+(d.canPublish?'<button class="btn light" type="button" data-release-action="preview-recreate-production">Recriar da Produção</button><button class="btn light" type="button" data-release-action="preview-recreate-template">Recriar do template</button>':'')+'</div></section>'}
 function diffHtml(c){const diff=c.diff||{},runtime=c.runtimeDiff||{},files=(diff.files||[]).slice(0,80),changes=(runtime.changes||[]).slice(0,80);return '<div class="release-diff"><details><summary>Código · '+Number(diff.fileCount||0)+' arquivo(s) · +'+Number(diff.added||0)+' / -'+Number(diff.removed||0)+'</summary><ul class="release-diff-list">'+(files.map(x=>'<li><code>'+esc(x.path)+'</code><span>+'+Number(x.added||0)+' / -'+Number(x.removed||0)+'</span></li>').join('')||'<li>Nenhuma alteração de código.</li>')+'</ul></details><details><summary>Runtime · '+Number(runtime.changeCount||0)+' alteração(ões)</summary><ul class="release-diff-list">'+(changes.map(x=>'<li><code>'+esc(x.path)+'</code><span>'+esc(x.kind)+'</span></li>').join('')||'<li>Nenhuma alteração relevante de runtime.</li>')+'</ul></details></div>'}
 function renderHomologation(){const d=model.data,c=(d.candidates||[])[0],canH=!!d.canHomologate,canP=!!d.canPublish;if(!c)return jobHtml()+'<section class="release-stage"><div class="release-stage-head"><div><span class="release-kicker">H · Homologação</span><h3>Nenhum candidato</h3><p>Quando o Preview estiver pronto, congele código + runtime em um candidato imutável.</p></div></div><div class="release-actions"><button class="btn" data-release-action="homologation-create">Enviar Preview para homologação</button>'+envButton('homologation')+'</div>'+homologatorsHtml()+'</section>';const labels={awaiting_homologation:'Aguardando homologação',homologated:'Homologado',rejected:'Rejeitado',published:'Publicado'};let actions=openLink(c.url,'Abrir '+c.stage_code)+envButton('homologation');if(c.status==='awaiting_homologation'&&canH){actions+='<button class="btn" data-release-action="homologation-approve" data-candidate="'+c.candidate_number+'">Homologar</button><button class="btn light" data-release-action="homologation-reject" data-candidate="'+c.candidate_number+'">Rejeitar</button>';if(canP)actions+='<button class="btn" data-release-action="homologation-publish" data-candidate="'+c.candidate_number+'">Homologar e publicar</button>'}if(c.status==='homologated'&&canP)actions+='<button class="btn" data-release-action="production-request" data-candidate="'+c.candidate_number+'">Publicar em Produção</button>';if(['rejected','published'].includes(c.status))actions+='<button class="btn" data-release-action="homologation-create">Criar novo candidato</button>';return jobHtml()+'<section class="release-stage"><div class="release-stage-head"><div><span class="release-kicker">'+esc(c.stage_code)+' · Homologação</span><h3>'+esc(labels[c.status]||c.status)+'</h3><p>Este candidato é imutável. O Preview pode continuar recebendo alterações sem mudar o que está sendo homologado.</p></div><span class="release-badge '+(c.status==='homologated'||c.status==='published'?'ok':c.status==='rejected'?'warn':'')+'">'+esc(labels[c.status]||c.status)+'</span></div><div class="release-grid"><div class="release-box"><span>Commit congelado</span><code>'+esc((c.commit_sha||'').slice(0,12))+'</code></div><div class="release-box"><span>Preview de origem</span><strong>W'+Number(c.preview_generation||0)+'</strong></div></div>'+diffHtml(c)+'<div class="release-actions">'+actions+'</div>'+homologatorsHtml()+'</section>'}
 function homologatorsHtml(){const d=model.data;if(!d.canPublish)return '';return '<section class="release-homologators"><label><strong>Homologadores autorizados</strong><input data-release-homologators value="'+esc((d.homologators||[]).join(', '))+'" placeholder="usuario1, usuario2"><small>O responsável pelo projeto sempre pode homologar. Separe outros usuários por vírgula.</small></label><div><button class="btn light" data-release-action="homologators-save">Salvar homologadores</button></div></section>'}
 function activeRelease(){return (model.data.releases||[]).find(x=>Number(x.is_active)===1)||model.data.legacyProduction||null}
 function releaseSafeUrl(value){try{const target=new URL(String(value||''),location.origin),allowed=target.origin===location.origin||target.hostname.endsWith('.cloudiff.duckdns.org');return allowed&&(target.protocol==='https:'||target.origin===location.origin)?target.href:''}catch(_){return ''}}
 function releaseStageContext(){const d=model.data||{},num=Number(d.publicNumber||0);if(model.tab==='preview'){const p=d.preview||{};return {environment:'preview',label:'Preview',stageCode:p.stageCode||'W',url:releaseSafeUrl(p.url||''),container:p.container||'',status:p.configured?(p.healthy?'Online':'Atenção'):'Não preparado',exists:!!p.configured}}if(model.tab==='homologation'){const c=(d.candidates||[])[0];if(!c)return {environment:'homologation',label:'Homologação',stageCode:'H',url:'',container:'',status:'Sem candidato',exists:false};const dep=Number(c.deploy_number||0);return {environment:'homologation',label:'Homologação',stageCode:c.stage_code||('H'+Number(c.candidate_number||0)),url:releaseSafeUrl(c.url||''),container:num&&dep?'cloudif-p'+num+'-d'+dep+'-web':'',status:String(c.status||'Candidato'),exists:true}}const a=activeRelease();if(!a)return {environment:'production',label:'Produção',stageCode:'P',url:'',container:'',status:'Sem publicação',exists:false};const publication=Number(a.publication_number||0),dep=Number(a.deploy_number||publication||0),legacy=!('candidate_number' in a),container=num?(legacy&&dep?'cloudif-p'+num+'-d'+dep+'-web':publication?'cloudif-p'+num+'-p'+publication+'-publication-web':''):'';return {environment:'production',label:'Produção',stageCode:a.stage_code||('P'+publication),url:releaseSafeUrl(a.stableUrl||a.url||''),container,status:String(a.status||'Publicado'),exists:true}}
 function renderReleaseSite(){const c=releaseStageContext();if(!c.url)return '<div class="release-alert"><strong>'+esc(c.label)+' ainda não possui URL disponível.</strong><span>Prepare ou publique este estágio antes de abrir o preview do site.</span></div>';return '<section class="stage-site-preview"><header><div><span>'+esc(c.stageCode)+' · '+esc(c.label)+'</span><strong>Preview do site</strong><small>'+esc(c.url)+'</small></div><a class="btn light" href="'+esc(c.url)+'" target="_blank" rel="noopener">Abrir em nova aba</a></header><div class="stage-site-preview__frame"><iframe src="'+esc(c.url)+'" title="Site '+esc(c.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" sandbox="allow-forms allow-modals allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts allow-downloads"></iframe></div><footer>Exibindo a URL real do estágio. Políticas de segurança definidas pela própria aplicação continuam sendo respeitadas.</footer></section>'}
 function renderReleaseTerminal(){const c=releaseStageContext();if(!c.exists)return '<div class="release-alert"><strong>'+esc(c.label)+' ainda não possui container correspondente.</strong><span>Prepare este estágio antes de abrir o terminal.</span></div>';const allowed=c.environment==='production'?!!(model.data&&model.data.canPublish):!!(model.data&&model.data.canWrite);if(!allowed)return '<div class="release-alert"><strong>Terminal não autorizado.</strong><span>'+(c.environment==='production'?'Produção exige o responsável pelo projeto ou administrador.':'Você possui acesso de leitura, mas não permissão para abrir o terminal.')+'</span></div>';return '<div class="release-job"><strong>Preparando terminal de '+esc(c.label)+'…</strong><progress></progress></div>'}
 function activationFor(candidate){return (model.data.activationRequests||[]).find(x=>Number(x.candidate_number)===Number(candidate&&candidate.candidate_number))||null}
 function renderPublication(){const d=model.data,c=(d.candidates||[]).find(x=>x.status==='homologated')||(d.candidates||[])[0],active=activeRelease(),activation=activationFor(c),live=model.approval;let approval='';if(activation){const status=(live&&live.approvalId===activation.approval_id?live.status:activation.status)||'pending';approval='<div class="release-alert '+(status==='approved'?'ok':'')+'"><strong>Autorização crítica P'+Number(activation.publication_number)+'</strong><span>'+esc(status==='approved'?'A dupla aprovação foi concluída. A publicação pode prosseguir.':'Aguardando a autorização exigida para Produção.')+'</span></div>'}let actions=envButton('production');if(c&&c.status==='homologated'&&d.canPublish){if(!activation)actions+='<button class="btn" data-release-action="production-request" data-candidate="'+c.candidate_number+'">Solicitar publicação</button>';else if(live&&live.status==='approved')actions+='<button class="btn" data-release-action="production-enqueue" data-candidate="'+c.candidate_number+'" data-approval="'+esc(live.approvalId)+'" data-digest="'+esc(live.activationDigest)+'">Publicar P'+Number(live.publicationNumber)+'</button>';else actions+='<a class="btn light" href="/cloudiff/portal/?tab=aprovacoes">Abrir aprovações</a><button class="btn light" data-release-action="approval-refresh">Atualizar autorização</button>'}const history=(d.releases||[]).map(x=>'<div class="release-history-row"><div><strong>'+esc(x.stage_code)+'</strong><br><small>'+esc(x.published_at||x.created_at||'')+'</small></div><div>'+openLink(x.url,'Abrir')+(d.canPublish&&Number(x.is_active)!==1?' <button class="btn light" data-release-action="rollback" data-publication="'+x.publication_number+'">Rollback</button>':'')+'</div></div>').join('');return jobHtml()+'<section class="release-stage"><div class="release-stage-head"><div><span class="release-kicker">P · Publication</span><h3>'+(active?'Produção ativa':'Ainda sem publicação P')+'</h3><p>Produção reutiliza exatamente o artefato homologado; não há rebuild entre H e P.</p></div><span class="release-badge '+(active?'ok':'warn')+'">'+esc(active?(active.stage_code||'P ativa'):'Sem P ativa')+'</span></div>'+(active?'<div class="release-grid"><div class="release-box"><span>URL amigável</span><strong>'+esc(active.stableUrl||active.stable_hostname||'')+'</strong></div><div class="release-box"><span>Artefato</span><code>'+esc((active.artifact_image_id||'legado').slice(0,20))+'</code></div></div><div class="release-actions">'+openLink(active.stableUrl||active.url,'Abrir Produção')+'</div>':'')+approval+'<div class="release-actions">'+actions+'</div>'+(history?'<div class="release-history"><strong>Histórico P</strong>'+history+'</div>':'')+'</section>'}
 function render(){if(!model.data){body.innerHTML='<p>Carregando fluxo…</p>';return}if(model.view==='terminal'){body.innerHTML=renderReleaseTerminal();bindActions();const c=releaseStageContext(),allowed=c.exists&&(c.environment==='production'?!!model.data.canPublish:!!model.data.canWrite);if(allowed&&!model.busy)setTimeout(openStageTerminal,0);return}body.innerHTML=model.view==='site'?renderReleaseSite():(model.tab==='preview'?renderPreview():model.tab==='homologation'?renderHomologation():renderPublication());bindActions()}
 async function load(){clearTimeout(model.poll);try{const data=await call(apiBase());model.data=data;model.csrf=data.csrfToken||model.csrf;model.approval=null;const a=(data.activationRequests||[])[0];if(a&&a.approval_id){try{model.approval=await call(apiBase()+'/approval/status?approvalId='+encodeURIComponent(a.approval_id))}catch(_){}}render();updateSummary(model.slug,data);if(data.job&&['queued','running'].includes(data.job.status))model.poll=setTimeout(load,2200)}catch(error){body.innerHTML='<div class="release-alert bad"><strong>Não foi possível carregar o fluxo.</strong><span>'+esc(error.message)+'</span></div>'}}
 async function act(op,payload={},message='Processando…'){if(model.busy)return;model.busy=true;body.insertAdjacentHTML('afterbegin','<div class="release-job"><strong>'+esc(message)+'</strong><progress></progress></div>');try{const result=await post(op,payload);if(result.approvalId&&result.status!=='approved'){model.tab='publication'}await load()}catch(error){await load();body.insertAdjacentHTML('afterbegin','<div class="release-alert bad"><strong>Não foi possível concluir.</strong><span>'+esc(error.message)+'</span></div>')}finally{model.busy=false}}
 async function requestProduction(candidate){if(model.busy)return;model.busy=true;try{const r=await post('production/approval/request',{candidateNumber:Number(candidate),reason:'Publicar candidato homologado em Produção'});if(r.status==='approved')await post('production/enqueue',{candidateNumber:Number(candidate),approvalId:r.approvalId,activationDigest:r.activationDigest});model.tab='publication';await load()}catch(e){await load();body.insertAdjacentHTML('afterbegin','<div class="release-alert bad"><strong>Publicação não iniciada.</strong><span>'+esc(e.message)+'</span></div>')}finally{model.busy=false}}
 function renderEmbeddedTerminal(result,ctx){const target=new URL(String(result.terminalUrl||''),location.origin);if(target.protocol!=='https:'||target.hostname!=='komodoiff.duckdns.org'||!target.pathname.includes('/terminal/'))throw new Error('A plataforma retornou um terminal inválido.');const embed=new URL(target.href);embed.searchParams.set('_cloudiff_embed',String(Date.now()));const embedLogin=window.cloudifKomodoLoginUrl(embed.href),externalLogin=window.cloudifKomodoLoginUrl(target.href);body.innerHTML='<section class="stage-site-preview stage-terminal-embed"><header><div><span>'+esc(result.stageCode||ctx.stageCode)+' · '+esc(ctx.label)+'</span><strong>Terminal do container</strong><small>'+esc(result.container||ctx.container||'')+'</small></div><a class="btn light" href="'+esc(externalLogin)+'" target="_blank" rel="noopener">Abrir no Komodo</a></header><div class="stage-site-preview__frame stage-terminal-embed__frame"><iframe src="'+esc(embedLogin)+'" title="Terminal '+esc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" allow="clipboard-read; clipboard-write; fullscreen" allowfullscreen></iframe></div><footer>SSO OIDC iniciado diretamente e terminal carregado no container deste estágio. “Abrir no Komodo” preserva o mesmo destino.</footer></section>'}
 async function openStageTerminal(){if(model.busy)return;model.busy=true;const ctx=releaseStageContext();body.innerHTML='<div class="release-job"><strong>Preparando terminal de '+esc(ctx.label)+'…</strong><progress></progress></div>';try{const environment=model.tab==='publication'?'production':model.tab,result=await post('stage/terminal',{environment});renderEmbeddedTerminal(result,ctx)}catch(error){body.innerHTML='<div class="release-alert bad"><strong>Terminal não aberto.</strong><span>'+esc(error.message)+'</span></div>'}finally{model.busy=false}}
 async function openPreviewTerminal(){if(model.busy)return;model.busy=true;const ctx=releaseStageContext();body.innerHTML='<div class="release-job"><strong>Preparando terminal do Preview…</strong><progress></progress></div>';try{const result=await post('preview/terminal',{});renderEmbeddedTerminal(result,ctx)}catch(error){body.innerHTML='<div class="release-alert bad"><strong>Terminal não aberto.</strong><span>'+esc(error.message)+'</span></div>'}finally{model.busy=false}}
 function bindActions(){body.querySelectorAll('[data-env-wizard]').forEach(b=>b.addEventListener('click',()=>close()));body.querySelectorAll('[data-release-action]').forEach(button=>button.onclick=async()=>{const a=button.dataset.releaseAction,c=Number(button.dataset.candidate||0);if(a==='preview-terminal')return openPreviewTerminal();if(a==='preview-ensure')return act('preview/ensure',{},'Preparando e sincronizando o Preview…');if(a==='preview-recreate-production')return confirm('Recriar o runtime do Preview a partir da Produção atual? O workspace Git será preservado.')&&act('preview/recreate',{source:'production'},'Recriando Preview…');if(a==='preview-recreate-template')return confirm('Recriar o runtime do Preview a partir do template inicial? O workspace Git será preservado.')&&act('preview/recreate',{source:'template'},'Recriando Preview…');if(a==='homologation-create')return act('homologation/enqueue',{},'Congelando código e runtime para Homologação…');if(a==='homologation-approve')return act('homologation/approve',{candidateNumber:c},'Registrando homologação…');if(a==='homologation-reject'){const note=prompt('Motivo da rejeição:')||'';if(note.trim())return act('homologation/reject',{candidateNumber:c,note},'Registrando rejeição…');return}if(a==='homologation-publish')return act('homologation/homologate-and-publish',{candidateNumber:c},'Homologando e solicitando Produção…');if(a==='production-request')return requestProduction(c);if(a==='production-enqueue')return act('production/enqueue',{candidateNumber:c,approvalId:button.dataset.approval,activationDigest:button.dataset.digest},'Publicando o artefato homologado…');if(a==='approval-refresh')return load();if(a==='rollback')return confirm('Ativar novamente P'+button.dataset.publication+' em Produção?')&&act('rollback',{publicationNumber:Number(button.dataset.publication)},'Executando rollback…');if(a==='homologators-save'){const input=body.querySelector('[data-release-homologators]'),usernames=(input.value||'').split(',').map(x=>x.trim()).filter(Boolean);return act('homologators',{usernames},'Salvando homologadores…')}})}
 function updateSummary(slug,data){document.querySelectorAll('[data-release-flow-summary="'+CSS.escape(slug)+'"]').forEach(root=>{const w=root.querySelector('[data-release-summary-w]'),h=root.querySelector('[data-release-summary-h]'),p=root.querySelector('[data-release-summary-p]'),preview=data.preview||{},candidate=(data.candidates||[])[0],release=(data.releases||[]).find(x=>Number(x.is_active)===1)||data.legacyProduction;w.textContent=preview.configured?(preview.stageCode+' · '+(preview.healthy?'online':'atenção')):'Não preparado';h.textContent=candidate?(candidate.stage_code+' · '+({awaiting_homologation:'aguardando',homologated:'homologado',rejected:'rejeitado',published:'publicado'}[candidate.status]||candidate.status)):'Sem candidato';p.textContent=release?(release.stage_code||'P ativa'):'Sem publicação'})}
 async function loadSummary(root){const slug=root.dataset.releaseFlowSummary;if(!slug)return;try{updateSummary(slug,await call('/cloudiff/portal/api/projects/'+encodeURIComponent(slug)+'/release-flow'))}catch(_){}}
 function open(button){model.opener=button;model.slug=button.dataset.projectSlug||'';model.tab='preview';model.view='stage';model.data=null;model.approval=null;layer.querySelector('[data-release-project]').textContent=model.slug;layer.classList.add('is-open');document.body.classList.add('cloudif-modal-open');layer.querySelectorAll('[data-release-view]').forEach(x=>x.classList.toggle('is-active',x.dataset.releaseView==='stage'));tab('preview');load();setTimeout(()=>layer.querySelector('[data-release-tab].is-active')?.focus(),0)}
 layer.querySelector('.release-wizard-close').onclick=close;layer.querySelector('[data-release-close]').onclick=close;layer.querySelector('[data-release-refresh]').onclick=load;layer.querySelectorAll('[data-release-tab]').forEach(b=>b.onclick=()=>tab(b.dataset.releaseTab));layer.querySelectorAll('[data-release-view]').forEach(b=>b.onclick=()=>view(b.dataset.releaseView));layer.addEventListener('click',e=>{if(e.target===layer)close()});document.addEventListener('keydown',e=>{if(e.key==='Escape'&&layer.classList.contains('is-open'))close()});document.addEventListener('click',e=>{const b=e.target.closest('[data-release-flow-open]');if(b)open(b)});
 const releaseDeepLink=new URLSearchParams(location.search),releaseDeepLinkProject=releaseDeepLink.get('project')||'';
 if(releaseDeepLink.get('tab')==='publicacao'&&releaseDeepLinkProject&&releaseDeepLink.get('open')==='release'){
   const button=[...document.querySelectorAll('[data-release-flow-open]')].find(x=>x.dataset.projectSlug===releaseDeepLinkProject);
   if(button)setTimeout(()=>open(button),0);
 }
 const roots=[...document.querySelectorAll('[data-release-flow-summary]')];if('IntersectionObserver'in window){const io=new IntersectionObserver(entries=>entries.forEach(x=>{if(x.isIntersecting){loadSummary(x.target);io.unobserve(x.target)}}),{rootMargin:'150px'});roots.forEach(x=>io.observe(x))}else roots.forEach(loadSummary);
})();
</script>"""
if 'page' in globals() and not globals().get('_whp_release_wrapped'):
    _whp_release_prev_page=page
    def page(user,tab,body):
        doc=_whp_release_prev_page(user,tab,body)
        return doc.replace('</body>',_WHP_RELEASE_ASSETS+'</body>',1)
    _whp_release_wrapped=True
# CloudIF W/H/P release wizard END
...
if 'page' in globals() and not globals().get('_admin_resources139_wrapped'):
    ...
    def _admin_resources139_panel(user):
        ...
        return ('<section class="cm-card" id="admin-recursos-globais"><div class="cm-resource-title"><div><h2>Administração global</h2>'
          ...
          '<div class="cm-actions admin-tool-shortcuts"><a class="btn" target="_blank" rel="noopener" href="https://komodoiff.duckdns.org/auth/oidc/login?redirect=https%3A%2F%2Fkomodoiff.duckdns.org%2Fservers"><span aria-hidden="true">SV</span> Servidores</a>'
          '<a class="btn light" target="_blank" rel="noopener" href="https://komodoiff.duckdns.org/auth/oidc/login?redirect=https%3A%2F%2Fkomodoiff.duckdns.org%2Fcontainers"><span aria-hidden="true">CT</span> Containers</a>'
...
.project-final__grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;padding:16px}.project-final__section{display:grid;align-content:start;gap:9px;min-width:0;padding:16px;border:1px solid var(--c-border,#dce3ed);border-radius:12px;background:var(--c-surface,#fff)}.project-final__section h3{margin:0;font-size:.92rem}.project-final__section p{margin:0}.project-final__meta{color:var(--c-muted,#64748b);font-size:.78rem;overflow-wrap:anywhere}.project-final__actions{display:flex;gap:8px;flex-wrap:wrap}.project-final__actions form{display:flex;gap:8px;flex-wrap:wrap;margin:0}.project-final__actions .btn{margin:0!important;width:auto!important}.project-final__publication .project-final__actions .btn{min-height:38px!important;padding:8px 11px!important;border-radius:9px!important;font-size:.78rem!important;font-weight:800!important}.project-final__publication .project-final__actions form .btn{background:var(--iff,#168821)!important;color:#fff!important;border-color:var(--iff,#168821)!important}.project-final__publication .project-final__actions>[data-env-wizard],.project-final__publication .project-final__actions>a.btn{background:var(--c-surface-2,#f1f5f9)!important;color:var(--c-text-strong,#0f172a)!important;border-color:var(--c-border,#dce3ed)!important}html[data-theme="dark"] .project-final__publication{--c-surface:var(--cif-surface);--c-surface-2:#0b1510;--c-text:var(--cif-text);--c-text-strong:var(--cif-text);--c-muted:var(--cif-muted);--c-border:var(--cif-border);background:var(--cif-surface)!important;border-color:var(--cif-border)!important;color:var(--cif-text)!important}html[data-theme="dark"] .project-final__publication .project-final__actions>[data-env-wizard],html[data-theme="dark"] .project-final__publication .project-final__actions>a.btn{background:#0b1510!important;color:var(--cif-text)!important;border-color:#345545!important}.project-final__status{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.project-final__status strong{font-size:.86rem}.project-management-final .cloudif-wizard{display:none}
.cloudif-wizard{position:fixed!important;inset:0!important;z-index:1400!important;display:none;place-items:center;padding:28px;background:rgba(8,15,11,.64);backdrop-filter:blur(5px);overflow:auto}
.cloudif-wizard>.wizard-box,.cloudif-wizard>.card{width:min(1040px,100%);max-height:94vh;overflow:auto;margin:auto!important;padding:0;border:1px solid var(--c-border,#dce3ed);border-radius:20px;background:var(--c-surface,#fff);box-shadow:0 30px 90px rgba(0,0,0,.30)}
.cloudif-wizard .wizard-head{position:sticky;top:0;z-index:5;display:flex;justify-content:space-between;gap:20px;align-items:flex-start;padding:24px 28px 18px;border-bottom:1px solid var(--c-border,#dce3ed);background:var(--c-surface,#fff)}
.cloudif-wizard .wizard-head h3{margin:0;font-size:1.45rem;letter-spacing:-.02em}.cloudif-wizard .wizard-head p{max-width:660px;margin:7px 0 0;color:var(--c-muted,#64748b);line-height:1.55}
.cloudif-wizard .wizard-close{display:grid;place-items:center;width:38px;height:38px;border:1px solid var(--c-border,#dce3ed);border-radius:50%;background:transparent;font-size:1.45rem;line-height:1;cursor:pointer}.cloudif-wizard .wizard-close:hover{background:var(--c-surface-2,#f1f5f9)}
.pm-new-shell{--pm-accent:var(--iff,#168821);--pm-accent-dark:var(--iff-dark,#0b6418);--pm-accent-soft:var(--iff-wash,#eaf4ec);--pm-text:var(--ink,#0f1f14);color:var(--pm-text);background:var(--surface,#fff);padding:0 28px 24px}
.pm-new-step{display:none}.pm-new-step.is-active{display:grid;gap:14px}.pm-new-steps li{cursor:pointer;transition:.18s ease}.pm-new-steps li.is-active{color:var(--on-iff,#fff);background:var(--pm-accent)}.pm-new-steps li.is-active:before{color:var(--pm-accent-dark);background:var(--surface,#fff)}.pm-new-steps li.is-complete{color:var(--pm-accent-dark);background:var(--pm-accent-soft)}
.pm-new-progress{height:4px;margin:-10px 0 20px;border-radius:999px;background:var(--c-surface-2,#f1f5f9);overflow:hidden}.pm-new-progress>span{display:block;width:25%;height:100%;border-radius:inherit;background:var(--pm-accent);transition:width .2s ease}
.pm-new-nav{display:flex;align-items:center;justify-content:space-between;gap:10px}.pm-new-nav__right{display:flex;gap:10px}.pm-new-nav .btn.primary{background:var(--pm-accent)!important;border-color:var(--pm-accent)!important;color:var(--on-iff,#fff)!important}.pm-new-nav .btn.primary:hover{background:var(--pm-accent-dark)!important}
.pm-tenant-create[hidden],.pm-tenant-select[hidden]{display:none!important}.pm-tenant-preview{display:grid;gap:5px;padding:12px;border:1px solid var(--rule,#dce3da);border-radius:10px;background:var(--pm-accent-soft)}.pm-tenant-preview span{font-size:.7rem;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:var(--pm-accent-dark)}.pm-tenant-preview code{font-size:.88rem;overflow-wrap:anywhere;color:var(--ink,#0f1f14)}
.pm-new-steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;padding:0;margin:18px 0 22px;list-style:none}.pm-new-steps li{display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:10px;background:var(--c-surface-2,#f1f5f9);color:var(--c-muted,#64748b);font-size:.76rem;font-weight:800}.pm-new-steps li:before{display:grid;place-items:center;width:22px;height:22px;border-radius:50%;background:var(--c-surface,#fff);content:attr(data-step);font-size:.7rem}.pm-new-steps li:first-child{color:var(--c-text-strong,#0f172a)}
.pm-new-layout{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(260px,.75fr);gap:18px;align-items:start}.pm-new-main{display:grid;gap:14px}.pm-new-summary{position:sticky;top:104px;display:grid;gap:14px;padding:18px;border:1px solid var(--rule,#dce3da);border-radius:14px;background:var(--paper,#f6f7f3);color:var(--pm-text)}.pm-new-summary h4{margin:0}.pm-new-summary p{margin:0;color:var(--c-muted,#64748b);font-size:.82rem;line-height:1.5}.pm-new-summary dl{display:grid;gap:11px;margin:0}.pm-new-summary dl>div{display:grid;gap:3px;padding-bottom:10px;border-bottom:1px solid var(--c-border,#dce3ed)}.pm-new-summary dl>div:last-child{padding-bottom:0;border-bottom:0}.pm-new-summary dt{font-size:.68rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--c-muted,#64748b)}.pm-new-summary dd{margin:0;font-size:.86rem;font-weight:750;overflow-wrap:anywhere}
.pm-new-section{display:grid;gap:14px;padding:19px;border:1px solid var(--rule,#dce3da);border-radius:14px;background:var(--surface,#fff);color:var(--pm-text)}.pm-new-section__head{display:grid;gap:4px}.pm-new-section__head h4{margin:0;font-size:1rem}.pm-new-section__head p{margin:0;color:var(--c-muted,#64748b);font-size:.8rem;line-height:1.45}.pm-field{display:grid;gap:7px}.pm-field>span{font-size:.78rem;font-weight:800}.pm-field small{color:var(--c-muted,#64748b);line-height:1.45}.pm-field input,.pm-field textarea,.pm-field select{width:100%;min-height:44px;padding:10px 12px;border:1px solid var(--rule,#dce3da);border-radius:10px;background:var(--surface,#fff);color:var(--ink,#0f1f14);font:inherit}.pm-field textarea{min-height:88px;resize:vertical}.pm-field input:focus,.pm-field textarea:focus,.pm-field select:focus{outline:2px solid rgba(22,136,33,.18);border-color:var(--pm-accent)}
.pm-choice-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.pm-choice{position:relative;display:grid;gap:5px;padding:13px;border:1px solid var(--rule,#dce3da);border-radius:12px;background:var(--surface,#fff);color:var(--ink,#0f1f14);cursor:pointer}.pm-choice input{position:absolute;opacity:0;pointer-events:none}.pm-choice strong{font-size:.84rem}.pm-choice small{color:var(--c-muted,#64748b);line-height:1.35}.pm-choice:has(input:checked){border-color:var(--pm-accent);box-shadow:0 0 0 2px rgba(22,136,33,.14);background:var(--pm-accent-soft)}
.pm-runtime-options{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.pm-runtime-options .pm-choice{min-height:82px}.pm-runtime-options .pm-choice strong{font-size:.88rem}.pm-tenant-select[hidden]{display:none!important}
.pm-provision-list{display:grid;gap:9px;margin:0;padding:0;list-style:none}.pm-provision-list li{display:flex;gap:9px;align-items:flex-start;color:var(--c-muted,#64748b);font-size:.8rem;line-height:1.45}.pm-provision-list li:before{content:'✓';font-weight:900;color:var(--iff-dark,#0b6418)}
.pm-new-footer{position:sticky;bottom:0;z-index:4;display:flex;justify-content:flex-end;gap:10px;margin:22px -28px -24px;padding:16px 28px;border-top:1px solid var(--c-border,#dce3ed);background:var(--c-surface,#fff)}.pm-new-footer .btn{min-height:42px}.pm-new-footer .btn:first-child{order:1}.pm-new-footer .btn:last-child{order:0}
#pm197_new .pm-new-step:not(.is-active){display:none!important}
...
.pm-live{display:grid;gap:16px;padding:24px 28px 28px}.pm-live[hidden]{display:none!important}.pm-live-status{display:flex;align-items:center;gap:12px;padding:14px;border:1px solid var(--rule,#dce3da);border-radius:12px;background:var(--rule-soft,#edf1eb);color:var(--ink,#0f1f14)}.pm-live-dots{display:inline-flex;gap:4px}.pm-live-dots i{width:6px;height:6px;border-radius:50%;background:currentColor;animation:pmLivePulse 1.1s infinite ease-in-out}.pm-live-dots i:nth-child(2){animation-delay:.18s}.pm-live-dots i:nth-child(3){animation-delay:.36s}@keyframes pmLivePulse{0%,80%,100%{opacity:.25;transform:translateY(0)}40%{opacity:1;transform:translateY(-3px)}}.pm-live-progress{display:grid;gap:7px}.pm-live-progress progress{width:100%;height:12px;accent-color:var(--focus,#1b5fbf)}.pm-live-steps{display:grid;gap:9px;margin:0;padding:0;list-style:none}.pm-live-step{display:grid;grid-template-columns:26px 1fr auto;gap:12px;align-items:center;padding:12px;border:1px solid var(--rule,#dce3da);border-radius:11px;background:var(--surface,#fff);color:var(--ink,#0f1f14)}.pm-live-step.pending{opacity:.58}.pm-live-step.running{background:var(--rule-soft,#edf1eb);border-color:var(--focus,#1b5fbf)}.pm-live-step.done{background:var(--iff-wash,#eaf4ec);border-color:var(--iff,#168821)}.pm-live-step.failed{background:var(--halt-wash,#faebec);border-color:var(--halt,#9c1c24)}.pm-live-step-icon{width:24px;height:24px;display:grid;place-items:center;border-radius:50%;background:var(--rule-soft,#edf1eb);color:var(--ink,#0f1f14);font-size:.72rem;font-weight:900}.pm-live-step.running .pm-live-step-icon{background:var(--focus,#1b5fbf);color:var(--surface,#fff)}.pm-live-step.done .pm-live-step-icon{background:var(--iff,#168821);color:var(--on-iff,#fff)}.pm-live-step.failed .pm-live-step-icon{background:var(--halt,#9c1c24);color:var(--surface,#fff)}.pm-live-step small{display:block;color:var(--ink-2,#42544a);margin-top:2px}.pm-live-terminal{padding:15px;border-radius:11px}.pm-live-terminal.ok{background:var(--iff-wash,#eaf4ec);border:1px solid var(--iff,#168821)}.pm-live-terminal.bad{background:var(--halt-wash,#faebec);border:1px solid var(--halt,#9c1c24)}.pm-live-actions{display:flex;justify-content:flex-end;gap:10px}
...
def _pm197_render(user,selected=''):
    ...
    for p in rows:
        ...
        provision_label={'queued':'Na fila','running':'Em execução','succeeded':'Pronto','failed':'Atenção','not_started':'Não iniciado'}.get(provision_state['status'],provision_state['status'])
        repo_value='Vinculado' if forge_target else 'Não vinculado'
        runtime_value=runtime_state.get('status') or 'Não verificado'
        markup=f'''<article class="project-workspace-detail" data-project-slug="{h(slug)}" data-project-owner="{h(owner)}" data-provision-status="{h(provision_state['status'])}" data-provision-recoverable="{'1' if provision_state.get('recoverable') else '0'}">
<header class="project-workspace-detail__head"><div><span class="project-workspace-kicker">Projeto ativo</span><h2>{h(p['name'] or slug)}</h2><p><code>{h(slug)}</code> · {h(owner)}</p></div><div class="project-workspace-status"><span class="pill {'ok' if runtime_state.get('healthy') else 'muted'}">{h(runtime_value)}</span><span class="pill {'ok' if provision_state['status']=='succeeded' else 'muted'}">{h(provision_label)}</span></div></header>
<dl class="project-overview-strip"><div><dt>Responsável</dt><dd>{h(owner)}</dd></div><div><dt>Banco</dt><dd>{h(p['tenant'] or 'Sem tenant')}</dd></div><div><dt>Repositório</dt><dd>{h(repo_value)}</dd></div><div><dt>Runtime</dt><dd>{h(runtime_state.get('label') or 'Não identificado')}</dd></div></dl>
<div class="project-workspace-columns">
<section class="project-workspace-section"><div class="project-workspace-section__head"><div><span>Recursos</span><h3>Acessos do projeto</h3></div></div><div class="project-resource-list">
<div class="project-resource-row"><span>Banco / Studio</span><strong>{h(p['tenant'] or 'Não vinculado')}</strong><div>{bank_action}</div></div>
<div class="project-resource-row"><span>Repositório Forge</span><strong>{h(forge_target or 'Não vinculado')}</strong><div>{repo_action}</div></div>
<div class="project-resource-row"><span>Terminal</span><strong>{'Stack '+h(runtime_project.get('stack_id')) if runtime_project.get('stack_id') else 'Não disponível'}</strong><div>{terminal_action}</div></div>
</div></section>
<section class="project-workspace-section"><div class="project-workspace-section__head"><div><span>Operações</span><h3>Próximas ações</h3></div></div><div class="project-action-stack"><a class="btn" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}&amp;open=release">Publicar</a><a class="btn light" href="/cloudiff/portal/?tab=git&amp;project={urllib.parse.quote(slug,safe='')}">Código</a><form method="post" action="{url('/action/project_action')}"><input type="hidden" name="csrf_token" value="{h(csrf_token)}"><input type="hidden" name="slug" value="{h(slug)}"><button class="btn gray" name="op" value="check">Checar estado</button></form>{resume_action}<button class="btn light" type="button" onclick="cloudifShowWizard('{acl_id}')">Permissões</button>{delete_action}</div></section>
</div>
<details class="project-workspace-more"><summary>Detalhes técnicos</summary><div class="project-workspace-more__body"><section><h3>Descrição</h3><p>{h(p['description'] or 'Sem descrição.')}</p></section><section><h3>Publicação</h3><p>Preview → Homologação → Produção. A produção reutiliza o artefato homologado.</p><div class="project-final__actions"><a class="btn light" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}&amp;open=variables">Variáveis</a><a class="btn light" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}">Histórico de publicação</a></div></section><section><h3>Provisionamento</h3><p><strong>{h(provision_label)}</strong></p><p class="project-final__meta">Etapa: {h(provision_state['step'] or '—')} · Atualizado: {h(provision_state['updated_at'] or '—')}</p>{('<p class="project-final__meta">'+h(provision_state['error'])+'</p>') if provision_state['error'] else ''}{('<p class="project-final__meta"><strong>A infraestrutura está pronta.</strong> A retomada continuará da última etapa segura sem recriar projeto, repositório ou banco.</p>') if provision_state.get('recoverable') else ''}</section></div></details>
</article>'''
        project_items.append({'slug':slug,'name':str(p['name'] or slug),'owner':owner,'markup':markup,'status':provision_state['status'],'tenant':str(p['tenant'] or '')})
        ...
        acl_modal=acl_modal.replace('<div class="wizard-head">','<div class="wizard-head">',1)
        acl_modal=acl_modal.replace('<form method="post" action="/cloudiff/portal/action/project_acl" onsubmit=', '<p class="small">A ACL do projeto é a fonte central. Após salvar, o reconciliador aplica o acesso no Portal, Forgejo, tenant e publicação e registra falhas pendentes.</p><form method="post" action="/cloudiff/portal/action/project_acl" onsubmit=',1)
    ...
    valid_slugs={item['slug'] for item in project_items}
    selected=str(selected or '').strip()
    if selected not in valid_slugs:
        own=next((item for item in project_items if item['owner']==user['username']),None)
        selected=(own or (project_items[0] if project_items else {})).get('slug','')
    active=next((item for item in project_items if item['slug']==selected),None)
    status_names={'succeeded':'Pronto','running':'Executando','queued':'Na fila','failed':'Atenção','not_started':'Novo'}
    project_rows=''.join(
        f'<a class="project-row'+(' is-active' if item['slug']==selected else '')+'" href="/cloudiff/portal/?tab=projetos&amp;project='+urllib.parse.quote(item['slug'],safe='')+'" data-project-select data-project-slug="'+h(item['slug'])+'" data-project-name="'+h(item['name'])+'"'+(' aria-current="true"' if item['slug']==selected else '')+'><span><strong>'+h(item['name'])+'</strong><small>'+h(item['owner'])+' · '+h(item['slug'])+'</small></span><em>'+h(status_names.get(item['status'],item['status'] or '—'))+'</em></a>'
        for item in project_items
    )
    workspace_html=f'''<section id="cloudif-project-list" class="project-workspace" data-active-project="{h(selected)}" data-active-project-name="{h(active['name'] if active else '')}"><header class="project-workspace-head"><div><span class="project-workspace-kicker">Workspace</span><h2>Projetos</h2><p>Selecione um projeto para trabalhar. Os detalhes aparecem somente para o contexto ativo.</p></div><button class="btn" type="button" onclick="cloudifShowWizard('pm197_new')">Novo projeto</button></header><div class="project-workspace-layout"><aside class="project-index" aria-label="Seletor de projetos"><label class="project-filter"><span class="sr-only">Buscar projeto</span><input type="search" placeholder="Buscar projeto…" autocomplete="off" data-project-filter></label><div class="project-index-meta"><span>{len(project_items)} visível{'' if len(project_items)==1 else 'is'}</span><button type="button" class="project-filter-clear" data-project-filter-clear hidden>Limpar</button></div><nav class="project-list" aria-label="Projetos visíveis">{project_rows or '<p class="project-empty">Nenhum projeto visível.</p>'}</nav></aside><div class="project-workspace-main">{active['markup'] if active else '<section class="project-workspace-empty"><h3>Nenhum projeto selecionado</h3><p>Crie um projeto ou ajuste suas permissões.</p></section>'}</div></div></section>'''
    return f'''{_PM197_CSS}<script>function cloudifShowWizard(id){{document.querySelectorAll('.cloudif-wizard').forEach(x=>x.style.display='none');const target=document.getElementById(id);if(target){{target._cloudifOpener=document.activeElement;target.style.display='grid';document.body.classList.add('cloudif-modal-open');const focus=target.querySelector('input,select,textarea,button');if(focus)setTimeout(()=>focus.focus(),0)}}}}function cloudifCancelWizard(){{let opener=null;document.querySelectorAll('.cloudif-wizard').forEach(x=>{{if(x.style.display!=='none'&&x._cloudifOpener)opener=x._cloudifOpener;x.style.display='none';x._cloudifOpener=null}});document.body.classList.remove('cloudif-modal-open');if(opener&&document.contains(opener))setTimeout(()=>opener.focus(),0)}}function cloudifHideWizard(id){{const target=document.getElementById(id),opener=target&&target._cloudifOpener;if(target){{target.style.display='none';target._cloudifOpener=null}}document.body.classList.remove('cloudif-modal-open');if(opener&&document.contains(opener))setTimeout(()=>opener.focus(),0)}}document.addEventListener('click',e=>{{const modal=e.target.closest('.cloudif-wizard');if(modal&&e.target===modal)cloudifHideWizard(modal.id)}});document.addEventListener('keydown',e=>{{if(e.key==='Escape')cloudifCancelWizard()}});document.addEventListener('toggle',e=>{{const d=e.target;if(d.matches&&d.matches('.project-final[open]'))document.querySelectorAll('.project-final[open]').forEach(x=>{{if(x!==d)x.open=false}})}},true);(function(){{
 ...
 const username=form.dataset.username||'usuario';const labels={{link:'Tenant existente',create:'Novo tenant dedicado',skip:'Sem banco'}};const runtimeLabels={{node20:'Apache · Node.js 20',node22:'Apache · Node.js 22',node24:'Apache · Node.js 24'}};
 ...
 function updateTenant(){{const mode=form.querySelector('[name=db_mode]:checked');const selected=mode?mode.value:'link';const existing=form.querySelector('[name=tenant_existing]');const suffix=form.querySelector('[name=tenant_suffix]');const duration=form.querySelector('[name=tenant_keepalive_hours]');const hidden=form.querySelector('[name=tenant]');const existingWrap=form.querySelector('.pm-tenant-select');const createWrap=form.querySelector('.pm-tenant-create');let tenant='';if(selected==='link')tenant=existing?existing.value:'';if(selected==='create'){{let base=slugify(suffix&&suffix.value?suffix.value:(form.elements.name.value||''));if(!base)base=stamp();tenant=username+'-'+base}}if(existingWrap)existingWrap.hidden=selected!=='link';if(createWrap)createWrap.hidden=selected!=='create';if(hidden)hidden.value=tenant;const preview=form.querySelector('[data-pm-tenant-preview]');if(preview)preview.textContent=tenant||'Nenhum tenant';const durationText=duration?duration.value+' hora(s) ligado após concluir o provisionamento':'6 horas';setSummary('db',selected==='create'?tenant+' · '+durationText:(labels[selected]||'Tenant existente'))}}
 function updateSummary(){{const runtime=form.querySelector('[name=runtime_template]');setSummary('name',form.elements.name.value.trim()||'Nome ainda não informado');const php=form.querySelector('[name=php_version]');setSummary('runtime',(runtimeLabels[runtime&&runtime.value]||runtimeLabels['node22'])+' · PHP '+(php?php.value:'8.3'));updateTenant()}}
 ...
 form.addEventListener('input',updateSummary);form.addEventListener('change',updateSummary);if(previous)previous.addEventListener('click',()=>show(current-1));if(next)next.addEventListener('click',()=>{{if(validateStep())show(current+1)}});stepItems.forEach((item,i)=>item.addEventListener('click',()=>{{if(i<=current||validateStep())show(i)}}));const live=document.getElementById('pm197_provision_live'),shell=modal.querySelector('.pm-new-shell'),headTitle=document.getElementById('pm197_new_title'),headText=modal.querySelector('.wizard-head p'),closeButton=modal.querySelector('.wizard-close');const liveLabels=['Registro do projeto','Repositório Forgejo','Stack e containers','Banco e tenant','Identidade e permissões','Configuração de backup','Template da aplicação','Publicação inicial','Conclusão'];let provisioning=false,lastProvision=null;
 function liveDots(){{return '<span class="pm-live-dots"><i></i><i></i><i></i></span>'}}
 function stateFor(data,index){{const step=data.current_step||'queued',status=data.status||'queued',components=data.components||{{}};if(status==='succeeded')return'done';if(index===0)return'done';if(index===1)return components.forgejo?.ok?'done':status==='failed'&&step==='provision'?'failed':step==='provision'?'running':'pending';if(index===2)return components.komodo?.ok?'done':status==='failed'&&components.forgejo?.ok?'failed':components.forgejo?.ok&&step==='provision'?'running':'pending';if(index===3)return !data.tenant?'done':components.supabase?.ok?'done':status==='failed'&&components.komodo?.ok?'failed':components.komodo?.ok&&step==='provision'?'running':'pending';if(status==='failed'){{const failIndex={{queued:0,'onboarding-reconcile':4,'tenant-availability-policy':5,'backup-configuration':5,template:6,'initial-publication':7}}[step]??index;return index<failIndex?'done':index===failIndex?'failed':'pending'}}if(index===4)return ['onboarding-reconcile','backup-configuration','template','initial-publication','complete'].includes(step)?(step==='onboarding-reconcile'?'running':'done'):'pending';if(index===5)return ['backup-configuration','template','initial-publication','complete'].includes(step)?(step==='backup-configuration'?'running':'done'):'pending';if(index===6)return ['template','initial-publication','complete'].includes(step)?(step==='template'?'running':'done'):'pending';if(index===7)return ['initial-publication','complete'].includes(step)?(step==='initial-publication'?'running':'done'):'pending';if(index===8)return status==='succeeded'?'done':'pending';return'pending'}}
 function drawLive(data){{lastProvision=data;const states=liveLabels.map((_,i)=>stateFor(data,i)),done=states.filter(x=>x==='done').length,progress=data.status==='succeeded'?100:Math.max(5,Math.round(done/liveLabels.length*100));const terminal=data.status==='succeeded'?'<div class="pm-live-terminal ok"><strong>Projeto provisionado.</strong><p>Repositório, recursos, identidade e publicação inicial foram reconciliados.</p></div>':data.status==='failed'?`<div class="pm-live-terminal bad"><strong>O provisionamento falhou.</strong><p>${{String(data.last_error||'Falha não identificada.').replace(/[&<>]/g,'')}}</p></div>`:'';live.innerHTML=`${{data.status==='queued'||data.status==='running'?`<div class="pm-live-status">${{liveDots()}}<div><strong>Provisionamento em andamento</strong><small>Etapa atual: ${{data.current_step||'preparando'}}. Não feche esta janela.</small></div></div>`:''}}<div class="pm-live-progress"><progress max="100" value="${{progress}}"></progress><small>${{progress}}% concluído</small></div><ol class="pm-live-steps">${{liveLabels.map((label,i)=>{{const st=states[i],icon=st==='done'?'✓':st==='failed'?'!':st==='running'?liveDots():String(i+1),badge=st==='done'?'Concluído':st==='failed'?'Falhou':st==='running'?'Executando':'Aguardando';return `<li class="pm-live-step ${{st}}"><span class="pm-live-step-icon">${{icon}}</span><div><strong>${{label}}</strong><small>${{st==='running'?'O agente está executando esta etapa.':st==='done'?'Etapa finalizada.':'Aguardando a etapa anterior.'}}</small></div><span class="pill ${{st==='done'?'ok':st==='failed'?'bad':'muted'}}">${{badge}}</span></li>`}}).join('')}}</ol>${{terminal}}<div class="pm-live-actions">${{data.status==='succeeded'?'<button class="btn primary" type="button" data-pm-open-project>Abrir projeto</button>':data.status==='failed'?'<button class="btn gray" type="button" data-pm-close-live>Fechar</button>':'<button class="btn gray" type="button" disabled>Provisionando…</button>'}}</div>`;live.querySelector('[data-pm-open-project]')?.addEventListener('click',()=>location.href='/cloudiff/portal/?tab=projetos&project='+encodeURIComponent(data.slug));live.querySelector('[data-pm-close-live]')?.addEventListener('click',()=>location.reload())}}
 function showProvisionReconnect(attempt){{
   headTitle.textContent='Confirmando provisionamento';
   headText.textContent='A plataforma está atualizando serviços. Recuperando o estado real do projeto.';
   if(lastProvision)drawLive(lastProvision);
   const banner=`<div class="pm-live-status" data-pm-reconnect>${{liveDots()}}<div><strong>Reconectando ao provisionador</strong><small>Tentativa ${{attempt+1}} de 75. O progresso anterior foi preservado.</small></div></div>`;
   const old=live.querySelector('[data-pm-reconnect]');
   if(old)old.outerHTML=banner;else live.insertAdjacentHTML('afterbegin',banner);
 }}
 async function pollProvision(url,attempt=0){{
   const slug=(lastProvision&&lastProvision.slug)||new URL(url,location.origin).searchParams.get('slug')||'';
   const canonical='/cloudiff/portal/api/project-provision-status?slug='+encodeURIComponent(slug);
   try{{
     const r=await fetch(canonical,{{credentials:'same-origin',headers:{{Accept:'application/json','Cache-Control':'no-store'}}}});
     const type=(r.headers.get('content-type')||'').toLowerCase();
     const text=await r.text();
     if(!type.includes('application/json')){{const e=new Error('Resposta HTML transitória do Portal.');e.status=r.status;throw e}}
     let d;try{{d=JSON.parse(text)}}catch(_e){{const e=new Error('Resposta JSON incompleta do provisionador.');e.status=r.status;throw e}}
     if(!r.ok||!d.ok){{const e=new Error(d.detail||d.error||'Falha ao consultar provisionamento.');e.status=r.status;throw e}}
     drawLive(d);
     if(d.status==='queued'||d.status==='running')setTimeout(()=>pollProvision(canonical,0),1200);
   }}catch(err){{
     const transient=[0,404,408,425,429,500,502,503,504].includes(Number(err.status||0))||String(err.message||'').includes('HTML transitória')||String(err.message||'').includes('JSON incompleta');
     if(transient&&attempt<75){{showProvisionReconnect(attempt);setTimeout(()=>pollProvision(canonical,attempt+1),1200);return}}
     if(lastProvision){{
       drawLive(lastProvision);
       headTitle.textContent='Resultado ainda não confirmado';
       headText.textContent='O provisionamento pode continuar no servidor. Atualize a página para recuperar o estado.';
       live.insertAdjacentHTML('beforeend',`<div class="pm-live-terminal bad"><strong>Não foi possível confirmar o resultado.</strong><p>${{String(err.message||'Falha de comunicação.').replace(/[&<>]/g,'')}}</p></div>`);
       return;
     }}
     drawLive({{status:'failed',current_step:'queued',last_error:err.message,components:{{}}}});
   }}
 }}
...
if 'Portal' in globals() and not globals().get('_pm197_route_wrapped'):
    ...
    def _pm197_get(self):
        ...
        if tab=='projetos' and path in ('','/cloudiff/portal','/cloudif/portal'):
            user=self.user();selected=(query.get('project') or [''])[0].strip()
            doc=page(user,'projetos',_pm197_render(user,selected=selected))
...
if 'Portal' in globals() and not globals().get('_admin_project_delete_wrapped'):
    ...
    def _admin_project_delete_get(self):
        ...
        if path in ('/cloudiff/portal/api/admin-delete-project-status','/cloudif/portal/api/admin-delete-project-status'):
            user=self.user();job_id=(q.get('job_id') or [''])[0].strip()
            allowed,payload=_admin_project_delete.can_read_job(job_id,user.get('username') or '',_admin_project_delete_global(user))
            code=200 if payload.get('ok') and allowed else (403 if payload.get('ok') else 404)
            if payload.get('ok') and not allowed:payload={'ok':False,'error':'forbidden'}
            raw=json.dumps(payload,ensure_ascii=False,separators=(',',':')).encode('utf-8')
            self.send_response(code);self.send_header('Content-Type','application/json; charset=utf-8');self.send_header('Cache-Control','no-store');self.send_header('X-Content-Type-Options','nosniff');self.send_header('Content-Length',str(len(raw)));self.end_headers();self.wfile.write(raw);return
