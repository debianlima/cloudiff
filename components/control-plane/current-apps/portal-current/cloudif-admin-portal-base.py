...
(function(){
 ...
 function publicationStageContext(){
   ...
   if(environment==='homologation'){
     const items=release.candidates||[];const item=items.find(x=>['awaiting_homologation','homologated','published'].includes(x.status))||items[0]||null;if(!item)return {environment,label:'Homologação',stageCode:'H',url:publicationSafeSiteUrl(fallback.url||''),container:'',status:'Sem candidato',exists:Boolean(fallback.url)};
