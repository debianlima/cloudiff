...
.publication-configuration{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.publication-configuration>article{display:grid;align-content:start;gap:8px;padding:16px;border:1px solid var(--cif-border);border-radius:12px;background:var(--cif-surface)}.publication-configuration>article>span{font-size:.7rem;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--cif-muted)}.publication-configuration p{margin:0;color:var(--cif-muted);font-size:.8rem;line-height:1.5}.publication-configuration form{margin-top:4px}html.env-wizard-open{overflow:hidden}.env-wizard-dialog{width:min(860px,calc(100vw - 32px));max-width:none;max-height:none;margin:auto;padding:0;border:0;background:transparent;overflow:visible}.env-wizard-dialog::backdrop{background:color-mix(in srgb,var(--c-text,var(--cif-text)) 55%,transparent)}.env-wizard{width:100%;max-height:min(92vh,860px);overflow:auto;background:var(--c-surface,var(--cif-surface));border:1px solid var(--c-border,var(--cif-border));border-radius:16px;padding:22px;box-shadow:var(--c-shadow,var(--cif-shadow))}.env-wizard header{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;padding-bottom:16px;border-bottom:1px solid var(--c-border,var(--cif-border))}.env-wizard header h2{margin:3px 0}.env-wizard header p{margin:0;color:var(--c-muted,var(--cif-muted))}.env-wizard-close{border:0;background:transparent;color:inherit;font-size:1.6rem;cursor:pointer}.env-toolbar{display:flex;align-items:end;justify-content:space-between;gap:12px;margin:18px 0}.env-toolbar label{display:grid;gap:5px}.env-toolbar select{min-width:190px}.env-revision{font-size:.8rem;color:var(--c-muted,var(--cif-muted))}.env-list{display:grid;gap:8px}.env-row{display:grid;grid-template-columns:minmax(160px,.8fr) minmax(220px,1.4fr) auto;gap:9px;align-items:center;padding:10px;border:1px solid var(--c-border,var(--cif-border));border-radius:10px}.env-row input{width:100%}.env-row code{font-size:.8rem}.env-row.secret{background:var(--c-surface-2,var(--cif-bg))}.env-row .env-type{font-size:.72rem;color:var(--c-muted,var(--cif-muted))}.env-empty{padding:24px;border:1px dashed var(--c-border,var(--cif-border));border-radius:10px;text-align:center;color:var(--c-muted,var(--cif-muted))}.env-actions{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-top:18px;padding-top:16px;border-top:1px solid var(--c-border,var(--cif-border))}.env-actions>div{display:flex;gap:8px}.env-status{margin-top:14px;padding:11px 12px;border-radius:9px;background:var(--c-surface-2,var(--cif-bg));font-size:.82rem}.env-status.bad{background:var(--c-danger-soft);color:var(--c-danger)}.env-status.pending{background:var(--c-warning-soft);color:var(--c-warning)}@media(max-width:760px){.publication-configuration{grid-template-columns:1fr}.env-wizard-dialog{width:calc(100vw - 16px)}.env-wizard{padding:16px}.env-row{grid-template-columns:1fr}.env-toolbar,.env-actions{align-items:stretch;display:grid}.env-actions>div{display:grid;grid-template-columns:1fr 1fr}}
</style>
"""
...
(function(){
 ...
 const envLayer=document.createElement('dialog');envLayer.className='env-wizard-dialog';envLayer.setAttribute('aria-labelledby','env-wizard-title');envLayer.innerHTML='<section class="env-wizard"><header><div><span class="env-type">Configuração versionada</span><h2 id="env-wizard-title">Variáveis de ambiente</h2><p data-env-project></p></div><button type="button" class="env-wizard-close" aria-label="Fechar">×</button></header><div class="env-toolbar"><label><span>Ambiente</span><select data-env-environment><option value="preview">Preview</option><option value="homologation">Homologação</option><option value="production" selected>Produção</option></select></label><span class="env-revision" data-env-revision></span></div><div class="env-list" data-env-list></div><div class="env-status" data-env-status hidden></div><div class="env-actions"><button type="button" class="btn light" data-env-add>+ Nova variável</button><div><button type="button" class="btn light" data-env-cancel>Cancelar</button><button type="button" class="btn" data-env-save>Salvar alterações</button></div></div></section>';document.body.appendChild(envLayer);
 ...
 function envClose(){if(envLayer.open&&typeof envLayer.close==='function')envLayer.close();else envLayer.removeAttribute('open');document.documentElement.classList.remove('env-wizard-open');envMessage('')}
 ...
 function envDraw(){let html=envModel.rows.map((x,index)=>x.secret?'<div class="env-row secret"><div><code>'+envEsc(x.name)+'</code><div class="env-type">Secret protegido · '+envEsc(x.service||'Projeto')+'</div></div><strong>••••••••</strong><span class="env-type">Use o fluxo de Secrets para substituir ou revogar.</span></div>':'<div class="env-row" data-env-public-row data-model-index="'+index+'" data-env-service="'+envEsc(x.service||'')+'"><div><input aria-label="Nome da variável" data-env-name value="'+envEsc(x.name)+'" placeholder="NOME_DA_VARIAVEL"><div class="env-type">'+envEsc(x.service?'Serviço '+x.service:'Projeto')+'</div></div><input aria-label="Valor" data-env-value value="'+envEsc(x.value)+'" placeholder="valor"><button type="button" class="btn light" data-env-delete>Excluir</button></div>').join('');envList.innerHTML=html||'<div class="env-empty">Nenhuma variável configurada neste ambiente.</div>';envList.querySelectorAll('[data-env-public-row]').forEach(row=>{row.querySelector('[data-env-delete]').onclick=()=>{envModel.rows.splice(Number(row.dataset.modelIndex),1);envDraw()}});envRevision.textContent='Revisão '+envModel.revision+' · valores secretos nunca são carregados'}
 ...
 async function envWaitApproval(planDigest,approval){envMessage('Aguardando aprovação humana. Você pode aprovar na aba Aprovações; este wizard continuará acompanhando.','pending');const deadline=Date.now()+10*60*1000;while(envLayer.open&&Date.now()<deadline){await new Promise(r=>setTimeout(r,2500));const status=await envFetch('/cloudiff/portal/api/projects/'+encodeURIComponent(envModel.slug)+'/environment/approval/status?approvalId='+encodeURIComponent(approval.approvalId));if(status.status==='approved')return envExecute(planDigest,approval.approvalId);if(['rejected','expired'].includes(status.status))throw new Error('A aprovação foi '+(status.status==='rejected'?'rejeitada':'expirada')+'.')}if(envLayer.open)throw new Error('Tempo de acompanhamento da aprovação encerrado. Abra o wizard novamente para continuar.')}
