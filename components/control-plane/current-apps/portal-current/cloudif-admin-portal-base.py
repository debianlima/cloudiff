...
(function(){
 ...
 function publicationStageContext(){
   ...
   if(environment==='homologation'){
     const items=release.candidates||[],eligible=items.filter(x=>['awaiting_homologation','homologated','published'].includes(x.status)).sort((a,b)=>Number(b.candidate_number||0)-Number(a.candidate_number||0)),item=eligible[0]||items[0]||null;if(!item)return {environment,label:'Homologação',stageCode:'H',url:publicationSafeSiteUrl(fallback.url||''),container:'',status:'Sem candidato',exists:Boolean(fallback.url)};
   ...
   return {environment,label:'Produção',stageCode:active.stage_code||('P'+publication),url:publicationSafeSiteUrl(active.stableUrl||active.url||fallback.url||''),embedUrl:publicationSafeSiteUrl(active.url||active.stableUrl||fallback.url||''),container,status:String(active.status||'Publicado'),exists:true};
 }
 function publicationEnvironmentOverview(){
   const data=publicationEnvironmentModel.data,item=(data.environments||[]).find(x=>x.name===publicationEnvironmentModel.active)||{},ctx=publicationStageContext(),name=item.name||publicationEnvironmentModel.active,label=envLabels[name]||name,canonicalStatus=ctx.exists?String(ctx.status||''):'',status=envStatusLabels[canonicalStatus]||canonicalStatus||envStatusLabels[item.status]||item.status||'—',cfg=item.configuration||{},runtime=item.runtime||{},artifact=String(item.artifact||''),artifactLabel=item.deployNumber?'d'+item.deployNumber:(artifact?artifact.slice(0,12):'—'),missing=Number(cfg.missingRequired||0),detail=cfg.valid===true?'Configuração válida':(missing?missing+' obrigatória(s) pendente(s)':'Configuração ainda não validada'),expires=item.expiresAt?new Date(Number(item.expiresAt)*1000).toLocaleString('pt-BR'):'',url=ctx.url||publicationSafeSiteUrl(item.url||''),isOk=ctx.exists?(['active','running','published','homologated'].includes(canonicalStatus.toLowerCase())||/online|publicado|homologado/i.test(canonicalStatus)):(['active','running','published'].includes(item.status)||/online|publicado/i.test(String(item.status||''))),open=url?'<button type="button" class="btn" data-publication-tool-jump="site">Visualizar '+envEsc(label)+'</button>':'<span class="publication-environment-hint">Ainda não há URL pública para este ambiente.</span>',variables=data.canWrite?'<button type="button" class="btn light" data-publication-tool-jump="variables">Ver variáveis</button>':'';
 ...
 function publicationSiteRender(){
   ...
   const embedUrl=ctx.embedUrl||ctx.url;publicationEnvironmentBody.innerHTML='<section class="stage-site-preview"><header><div><span>'+envEsc(ctx.stageCode)+' · '+envEsc(ctx.label)+'</span><strong>Preview do site</strong><small>'+envEsc(ctx.url)+'</small></div><a class="btn light" href="'+envEsc(ctx.url)+'" target="_blank" rel="noopener">Abrir em nova aba</a></header><div class="stage-site-preview__frame"><iframe src="'+envEsc(embedUrl)+'" title="Site '+envEsc(ctx.label)+'" loading="eager" referrerpolicy="strict-origin-when-cross-origin" sandbox="allow-forms allow-modals allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts allow-downloads"></iframe></div><footer>A visualização interna usa a URL versionada do estágio; “Abrir em nova aba” usa o endereço público estável.</footer></section>';
 }
...
<details class="project-workspace-more"><summary>Detalhes técnicos</summary><div class="project-workspace-more__body"><section><h3>Descrição</h3><p>{h(p['description'] or 'Sem descrição.')}</p></section><section><h3>Publicação</h3><p>Preview → Homologação → Produção. A produção reutiliza o artefato homologado.</p><div class="project-final__actions"><a class="btn light" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}&amp;open=variables">Variáveis</a><a class="btn light" href="/cloudiff/portal/?tab=publicacao&amp;project={urllib.parse.quote(slug,safe='')}">Histórico de publicação</a></div></section><section><h3>Provisionamento</h3><p><strong>{h(provision_label)}</strong></p><p class="project-final__meta">Etapa: {h(provision_state['step'] or '—')} · Atualizado: {h(provision_state['updated_at'] or '—')}</p>{('<p class="project-final__meta">'+h(provision_state['error'])+'</p>') if provision_state['error'] else ''}{('<p class="project-final__meta"><strong>A infraestrutura está pronta.</strong> A retomada continuará da última etapa segura sem recriar projeto, repositório ou banco.</p>') if provision_state.get('recoverable') else ''}</section></div></details>
 ...
 function settleProvisionTerminal(data){{
   if(!data||!['succeeded','failed'].includes(data.status))return;
   provisioning=false;
   closeButton.hidden=false;
   if(data.status==='succeeded'){{
     headTitle.textContent='Projeto provisionado';
     headText.textContent='Recursos, integrações e publicação inicial foram reconciliados.';
   }}else{{
     headTitle.textContent='Provisionamento requer atenção';
     headText.textContent='O estado real foi preservado. Revise a falha abaixo antes de retomar.';
   }}
 }}
