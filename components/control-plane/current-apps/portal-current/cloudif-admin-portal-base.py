...
(function(){
 ...
 const envStatusLabels={active:'Ativo',validated:'Validado',expired:'Expirado',not_created:'Não criado',not_deployed:'Não publicado',not_published:'Não publicado',running:'Online',published:'Publicado',failed:'Falhou',removed:'Removido',unknown:'Desconhecido'};
 function envOverviewDraw(root,data){const slug=root.dataset.projectSlug||data.projectSlug||'',items=data.environments||[];root.innerHTML='<div class="environment-overview-head"><div><span>Ambientes</span><h3>Preview, Homologação e Produção</h3><p>Configuração e ciclo de vida independentes para cada estágio do projeto.</p></div><button type="button" class="btn light" data-environments-refresh>Atualizar</button></div><div class="environment-overview-grid">'+items.map(item=>{const name=item.name||'',label=envLabels[name]||name,status=envStatusLabels[item.status]||item.status||'—',cfg=item.configuration||{},artifact=String(item.artifact||''),artifactLabel=item.deployNumber?'d'+item.deployNumber:(artifact?artifact.slice(0,12):'—'),missing=Number(cfg.missingRequired||0),configLabel='Config r'+Number(cfg.revision||0),detail=cfg.valid===true?'Configuração válida':(missing?missing+' obrigatória(s) pendente(s)':'Configuração ainda não validada'),expires=item.expiresAt?new Date(Number(item.expiresAt)*1000).toLocaleString('pt-BR'):'',url=String(item.url||''),isOk=['active','running','published'].includes(item.status),open=url?'<a class="btn" href="'+envEsc(url)+'" target="_blank" rel="noopener">Abrir '+envEsc(label)+'</a>':'<span class="environment-card__hint">Sem URL pública</span>';return '<article class="environment-card environment-card--'+envEsc(name)+'"><div class="environment-card__head"><div><span>'+envEsc(label)+'</span><strong>'+envEsc(status)+'</strong></div><span class="environment-state '+(isOk?'is-ok':'')+'">'+envEsc(status)+'</span></div><dl><div><dt>Configuração</dt><dd>'+envEsc(configLabel)+'</dd></div><div><dt>Validação</dt><dd>'+envEsc(detail)+'</dd></div><div><dt>Artefato</dt><dd><code>'+envEsc(artifactLabel)+'</code></dd></div>'+(expires?'<div><dt>Expira</dt><dd>'+envEsc(expires)+'</dd></div>':'')+'</dl><div class="environment-card__actions">'+open+(data.canWrite?'<button type="button" class="btn light" data-env-wizard data-project-slug="'+envEsc(slug)+'" data-env-environment="'+envEsc(name)+'">Variáveis</button>':'')+'</div></article>'}).join('')+'</div>';root.querySelector('[data-environments-refresh]')?.addEventListener('click',()=>envOverviewLoad(root))}
 async function envOverviewLoad(root){const slug=root.dataset.projectSlug||'';if(!slug)return;const grid=root.querySelector('.environment-overview-grid');if(grid)grid.innerHTML='<article class="environment-card is-loading"><strong>Atualizando ambientes…</strong></article>';try{const response=await fetch('/cloudiff/portal/api/projects/'+encodeURIComponent(slug)+'/environments-overview',{credentials:'same-origin',headers:{Accept:'application/json','Cache-Control':'no-store'}}),data=await response.json();if(!response.ok||!data.ok)throw new Error((data.error||{}).message||'environment_overview_failed');envOverviewDraw(root,data)}catch(error){root.innerHTML='<div class="environment-overview-head"><div><span>Ambientes</span><h3>Preview, Homologação e Produção</h3><p>Não foi possível consultar os ambientes agora.</p></div><button type="button" class="btn light" data-environments-refresh>Tentar novamente</button></div>';root.querySelector('[data-environments-refresh]')?.addEventListener('click',()=>envOverviewLoad(root))}}
 document.querySelectorAll('[data-environments-overview]').forEach(envOverviewLoad);
