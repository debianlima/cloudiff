...
def shell(title,eyebrow,intro,body,accent='green'):
 return f'''<style id="section98-style">.s98-hero{{padding:28px;border-radius:22px;background:var(--paper);border:1px solid var(--rule);margin-bottom:18px}}.s98-hero[data-accent=blue]{{border-top:5px solid var(--focus)}}.s98-hero[data-accent=green]{{border-top:5px solid var(--iff)}}.s98-hero[data-accent=amber]{{border-top:5px solid var(--drift)}}.s98-eyebrow{{font-size:.75rem;font-weight:900;text-transform:uppercase;letter-spacing:.12em;color:var(--iff-dark)}}.s98-hero h1{{margin:6px 0 8px;font-size:clamp(1.9rem,4vw,2.8rem);letter-spacing:-.035em}}.s98-hero p{{margin:0;max-width:850px;color:var(--ink-2);font-size:1.02rem}}.s98-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}}.s98-kpi{{padding:18px;border:1px solid var(--rule);border-radius:16px;background:var(--surface);color:var(--ink)}}.s98-kpi span{{display:block;color:var(--ink-2);font-size:.78rem;text-transform:uppercase;letter-spacing:.06em;font-weight:850}}.s98-kpi strong{{display:block;margin-top:6px;font-size:1.8rem;color:var(--iff-dark)}}.s98-list{{display:grid;gap:10px}}.s98-item{{display:grid;grid-template-columns:36px 1fr;gap:11px;padding:14px;border:1px solid var(--rule);border-radius:14px;background:var(--surface);color:var(--ink)}}.s98-icon{{width:34px;height:34px;border-radius:10px;display:grid;place-items:center;background:var(--iff-wash);color:var(--iff-dark);font-weight:900}}.s98-item h3,.s98-item p{{margin:0}}.s98-item p{{margin-top:4px;color:var(--ink-2)}}.s98-note{{padding:15px;border-left:4px solid var(--focus);background:var(--rule-soft);border-radius:10px;color:var(--ink)}}.s98-code{{padding:13px;border-radius:12px;background:var(--ink);color:var(--surface);font-family:ui-monospace,monospace;overflow:auto}}@media(max-width:650px){{.s98-hero{{padding:21px}}}}</style><section class="s98-hero" data-accent="{accent}"><span class="s98-eyebrow">{e(eyebrow)}</span><h1>{e(title)}</h1><p>{e(intro)}</p></section>{body}'''
def cards(items):
