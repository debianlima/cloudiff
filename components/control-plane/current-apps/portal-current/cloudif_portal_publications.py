#!/usr/bin/env python3
import hashlib, json, sqlite3, ssl, time, urllib.error, urllib.request
...
def publish(slug,user):
 ...
 values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(slug,num,dep,f'd{dep}',commit,stable_host,version_host,'published',1,actor,now,now,publication_message,json.dumps({'komodo':kres,'npm':nres,'promotion':pres,'republished':republished,'republished_from':republished_from},ensure_ascii=False)))
    ...
    return {'ok':True,'slug':slug,'public_number':num,'deploy_number':dep,'stable_url':'https://'+stable_host+'/','version_url':'https://'+version_host+'/','commit':commit,'republished':republished,'republished_from':republished_from,'message':publication_message}
