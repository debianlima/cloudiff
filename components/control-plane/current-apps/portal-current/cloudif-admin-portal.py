...
(function(){
 ...
 document.querySelectorAll('.project-card').forEach((card,idx)=>{const line=card.querySelector('.project-line');if(!line)return;const cols=[...line.children];if(cols.length<4)return;cols.forEach(x=>{x.style.display='block'});const slugText=(cols[0].querySelector('.small')||{}).textContent||'';const slug=slugText.replace(/^Slug:\s*/i,'').trim();const services=document.createElement('section');services.className='project-services-panel';services.innerHTML='<div class="section-title"><div><h3>Serviços do projeto</h3><p class="small">Acessos vinculados exclusivamente a este projeto.</p></div></div><div class="project-service-grid"></div><div class="project-site-action"></div><div class="framework-status"><span>Framework</span><strong>Detecção disponível na área Ferramentas</strong><a class="btn light" href="/cloudiff/portal/?tab=capacidades">Ver framework detectado</a></div>';const serviceGrid=services.querySelector('.project-service-grid');serviceGrid.append(cols[0],cols[1],cols[2]);serviceGrid.querySelectorAll('a').forEach(a=>{if(/Git/i.test(a.textContent))a.textContent='Abrir repositório';if(/Komodo/i.test(a.textContent))a.textContent='Komodo Publicação'});const containers=document.createElement('section');containers.dataset.role='containers';containers.innerHTML='<h3>Containers</h3><div class="empty-state">Carregando containers do projeto...</div>';const pubs=document.createElement('section');pubs.className='project-publication-actions';pubs.innerHTML='<div class="section-title"><div><h3>Publicação e acesso</h3><p class="small">Sincronização, integração, edição e permissões do projeto.</p></div></div>';pubs.append(cols[3]);const backups=document.createElement('section');backups.dataset.role='backups';backups.dataset.projectIndex=idx;backups.innerHTML='<h3>Backups</h3><div class="empty-state">Carregando backups...</div>';const agent=document.createElement('section');agent.className='project-agent-panel';agent.innerHTML='<div class="section-title"><div><h3>Agente IA e MCP</h3><p class="small">Conecte agentes ao projeto com identidade, token de uso único e escopos controlados.</p></div></div><div class="project-agent-actions"><a class="btn light" href="/cloudiff/portal/?tab=agentes">Conectar agente</a><a class="btn light" href="/cloudiff/portal/?tab=documentacao-mcp">Ver funções MCP</a></div>';const identity=[...document.querySelectorAll('#project-identities article.project-card')].find(x=>((x.querySelector('h3')||{}).textContent||'').trim()===slug);if(identity)agent.append(identity);else agent.insertAdjacentHTML('beforeend','<div class="empty-state">Identidade do projeto ainda não disponível.</div>');tabs(card,{'Serviços':services,'Containers':containers,'Publicação':pubs,'Backups':backups,'Agente IA e MCP':agent});line.replaceWith(services,containers,pubs,backups,agent);card.dataset.projectIndex=idx;card.dataset.projectSlug=slug;card.classList.add('cpx-ready');const hint=document.createElement('div');hint.className='project-options-hint';hint.innerHTML='<strong>Projeto</strong><span>Serviços, containers, publicação, backups e agentes em um único lugar.</span>';card.insertBefore(hint,card.firstChild)});const identitySection=document.querySelector('#project-identities');if(identitySection&&!identitySection.querySelector('article.project-card'))identitySection.remove();
