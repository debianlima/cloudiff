...
if 'render_projects' in globals() and not globals().get('_ap_render_wrapped'):
    _ap_prev_render=render_projects
    def render_projects(user):return _ap_prev_render(user)+_ap_render(user)
    _ap_render_wrapped=True
...
if 'render_projects' in globals() and not globals().get('_tx_render_wrapped'):
    _tx_prev_render=render_projects
    def render_projects(user):return _tx_prev_render(user)+_tx_render(user)
    _tx_render_wrapped=True
...
if 'render_projects' in globals() and not globals().get('_ph_render_wrapped'):
    _ph_prev_render=render_projects
    def render_projects(user):return _ph_prev_render(user)+_ph_render(user)
    _ph_render_wrapped=True
...
if 'page' in globals() and not globals().get('_admin_resources139_wrapped'):
    ...
    def _admin_resources139_panel(user):
        ...
        return ('<section class="cm-card" id="admin-recursos-globais"><div class="cm-resource-title"><div><h2>Administração global</h2>'
          ...
          '<div class="cm-actions"><a class="btn" target="_blank" rel="noopener" href="https://komodoiff.duckdns.org/servers">Abrir todos os servidores no Komodo</a>'
          '<a class="btn light" target="_blank" rel="noopener" href="https://komodoiff.duckdns.org/containers">Abrir todos os contêineres</a></div>'
          '<h3>Tenants Supabase</h3><div class="cm-grid">'+cards+'</div></section>')
    def page(user,tab,body):
        if tab=='projetos': body=_admin_resources139_panel(user)+body
