#!/usr/bin/env python3
import os,sqlite3,json,hmac,time,uuid,re
...
def init():
 os.makedirs(os.path.dirname(DB),exist_ok=True);c=con();c.execute('pragma journal_mode=delete');c.executescript('''create table if not exists events(event_id text primary key,ts text not null,project_id text,project_slug text,actor_type text not null,actor_id text,delegated_user_id text,source text not null,action text not null,result text not null,duration_ms integer not null default 0,trace_id text,client_id text,attrs_json text not null default '{}');create index if not exists idx_events_project_ts on events(project_id,ts desc);create index if not exists idx_events_slug_ts on events(project_slug,ts desc);create index if not exists idx_events_actor_ts on events(actor_id,ts desc);create index if not exists idx_events_delegated_ts on events(delegated_user_id,ts desc);create index if not exists idx_events_source_action on events(source,action);''');c.commit();c.close()
def auth(h):return bool(TOKEN) and hmac.compare_digest(h.get('Authorization',''),'Bearer '+TOKEN)

SENSITIVE_ATTR=re.compile(r'(token|secret|password|authorization|cookie|credential|private[_-]?key|session)',re.I)
def clean_attrs(value,depth=0):
 if depth>3:return None
 if isinstance(value,dict):
  out={}
  for k,v in list(value.items())[:64]:
   key=str(k)[:96]
   if SENSITIVE_ATTR.search(key):continue
   cleaned=clean_attrs(v,depth+1)
   if cleaned is not None:out[key]=cleaned
  return out
 if isinstance(value,list):return [x for x in (clean_attrs(v,depth+1) for v in value[:32]) if x is not None]
...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p.path=='/v1/events':
   q=parse_qs(p.query);limit=max(1,min(int((q.get('limit')or['100'])[0]),500))
   slug=(q.get('project')or[''])[0].strip();actor=(q.get('actor')or[''])[0].strip();delegated=(q.get('delegated_user')or[''])[0].strip();subject=(q.get('subject')or[''])[0].strip();source=(q.get('source')or[''])[0].strip();action=(q.get('action')or[''])[0].strip()
   where=[];params=[]
   if slug:where.append('project_slug=?');params.append(slug)
   if actor:where.append('lower(actor_id)=lower(?)');params.append(actor)
   if delegated:where.append('lower(delegated_user_id)=lower(?)');params.append(delegated)
   if subject:where.append('(lower(actor_id)=lower(?) or lower(delegated_user_id)=lower(?))');params.extend([subject,subject])
   if source:where.append('source=?');params.append(source)
   if action:where.append('action=?');params.append(action)
   sql='select * from events'+((' where '+' and '.join(where)) if where else '')+' order by ts desc limit ?';params.append(limit)
   rows=[]
   for x in c.execute(sql,params):
    row=dict(x)
    try:row['attrs']=json.loads(row.get('attrs_json') or '{}')
    except Exception:row['attrs']={}
    rows.append(row)
   c.close();self.sendj(200,{'ok':True,'events':rows});return
 ...
 def do_POST(self):
  ...
  try:
   ...
   eid=str(d.get('event_id')or uuid.uuid4().hex);ts=str(d.get('ts')or time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()));attrs=clean_attrs(d.get('attrs')or{}) or {}
