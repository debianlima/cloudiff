...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if method=='initialize':result={'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'0.5.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}
                ...
                if name=='project.list': data=control('/v1/projects');allowed=set(authz.get('project_slugs') or []);content=[x for x in data.get('projects',[]) if not allowed or x.get('slug') in allowed]
                    ...
                    content['file_id']=file_meta['file_id'];content['input_field']='file';content['oauth_identity_reused']=True;content['portal_cookie_required']=False;content['browser_session_required']=False;content['download_url_persisted']=False;content['secrets_exposed']=False
