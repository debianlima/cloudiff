...
(()=>{const meta=document.getElementById('meta'),status=document.getElementById('status'),select=document.getElementById('select'),local=document.getElementById('local'),picker=document.getElementById('picker');let latest={};const readInput=()=>window.openai?.toolInput||latest||{};const setStatus=(t,c='')=>{status.textContent=t;status.className='status '+c};const render=()=>{const i=readInput();meta.textContent=i.artifact_id?`Projeto: ${i.slug||'—'} · Artifact: ${i.artifact_id}`:'Aguardando slug e artifact_id…'};window.openai?.notifyIntrinsicHeight?.()};window.addEventListener('openai:set_globals',e=>{if(e?.detail?.globals?.toolInput)latest=e.detail.globals.toolInput;render()},{passive:true});const send=async file=>{const i=readInput();if(!i.slug||!i.artifact_id)throw new Error('slug/artifact_id ainda não foram disponibilizados pelo host.');if(!window.openai?.getFileDownloadUrl||!window.openai?.callTool)throw new Error('Este host não expõe as APIs de arquivo necessárias.');setStatus('Obtendo URL temporária…');const r=await window.openai.getFileDownloadUrl({fileId:file.fileId});if(!r?.downloadUrl)throw new Error('O host não retornou downloadUrl.');setStatus('Enviando para a CloudIFF…');const out=await window.openai.callTool('workspace.artifact.upload.file.resolve',{slug:i.slug,artifact_id:i.artifact_id,download_url:r.downloadUrl,file_id:file.fileId,file_name:file.fileName||'',mime_type:file.mimeType||''});setStatus('Arquivo enviado e validado.','ok');window.openai?.sendFollowUpMessage?.({prompt:`O upload do artifact ${i.artifact_id} foi concluído pelo seletor de arquivos do ChatGPT. Consulte workspace.artifact.upload.status e, se estiver sealed, prossiga com workspace.artifact.commit.plan.`,scrollToBottom:true});return out};select.onclick=async()=>{try{if(!window.openai?.selectFiles)throw new Error('A biblioteca de arquivos não está disponível neste ChatGPT. Use “Escolher do dispositivo”.');const files=await window.openai.selectFiles();if(!files?.length){setStatus('Nenhum arquivo selecionado.');return}await send(files[0])}catch(e){setStatus(e?.message||'Falha ao selecionar o arquivo.','bad')}};local.onclick=()=>picker.click();picker.onchange=async()=>{try{const f=picker.files?.[0];if(!f)return;if(!window.openai?.uploadFile)throw new Error('uploadFile não está disponível neste host.');setStatus('Autorizando arquivo no ChatGPT…');const u=await window.openai.uploadFile(f,{library:false});if(!u?.fileId)throw new Error('O host não retornou fileId.');await send({fileId:u.fileId,fileName:f.name,mimeType:f.type||'application/octet-stream'})}catch(e){setStatus(e?.message||'Falha ao autorizar o arquivo.','bad')}};render()})();
</script></body></html>'''
...
TOOLS=[
 ...
 {'name':'workspace.artifact.upload.file','title':'Enviar arquivo para artifact existente','description':'Upload binário server-side autenticado pela identidade OAuth MCP. O caminho principal usa openai/fileParams. Se o host enviar /mnt/data como string, a chamada abre automaticamente o seletor de arquivos do ChatGPT, que resolve fileId + downloadUrl pelas APIs oficiais do host e conclui o mesmo upload sem cookie do Portal.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'},'file':{'type':'object','description':'Arquivo anexado/selecionado no cliente MCP. O host deve hidratá-lo com download_url e file_id.','properties':{'download_url':{'type':'string'},'file_id':{'type':'string'},'mime_type':{'type':'string'},'file_name':{'type':'string'}},'required':['download_url','file_id'],'additionalProperties':False}},'required':['slug','artifact_id','file'],'additionalProperties':False,'examples':[{'slug':'meu-projeto','artifact_id':'art_111111111111111111111111','file':{'download_url':'https://files.oaiusercontent.com/...','file_id':'file_1111111111111111','file_name':'documentos-anonimizados.zip'}}]},'_meta':{'openai/fileParams':['file'],'ui':{'resourceUri':ARTIFACT_UPLOAD_WIDGET_URI},'openai/outputTemplate':ARTIFACT_UPLOAD_WIDGET_URI}},
 {'name':'workspace.artifact.upload.file.select','title':'Selecionar arquivo no ChatGPT','description':'Abre o seletor nativo de arquivos do ChatGPT para um artifact_id existente. Use quando openai/fileParams não hidratar o anexo automaticamente. Não altera o artifact até o usuário selecionar um arquivo.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'}},'required':['slug','artifact_id'],'additionalProperties':False},'_meta':{'ui':{'resourceUri':ARTIFACT_UPLOAD_WIDGET_URI},'openai/outputTemplate':ARTIFACT_UPLOAD_WIDGET_URI}},
 {'name':'workspace.artifact.upload.file.resolve','title':'Resolver arquivo autorizado','description':'Helper interno do componente CloudIFF: recebe file_id + download_url obtidos diretamente das APIs de arquivo do ChatGPT e grava no artifact existente. Não deve ser chamado pelo modelo.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'},'download_url':{'type':'string','minLength':8,'maxLength':8192},'file_id':{'type':'string','minLength':6,'maxLength':192},'mime_type':{'type':'string','maxLength':255},'file_name':{'type':'string','maxLength':240}},'required':['slug','artifact_id','download_url','file_id'],'additionalProperties':False},'_meta':{'ui':{'visibility':['app']},'openai/widgetAccessible':True,'openai/visibility':'private'}},
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not authenticated and method=='initialize':return self.sendj(200,{'jsonrpc':'2.0','id':rid,'result':{'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'0.8.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}})
        ...
        try:
            ...
            if method=='initialize':result={'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'0.8.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}
                ...
                if name=='project.list': data=control('/v1/projects');allowed=set(authz.get('project_slugs') or []);content=[x for x in data.get('projects',[]) if not allowed or x.get('slug') in allowed]
                    ...
                    required={'slug','artifact_id','download_url','file_id'};allowed=required|{'mime_type','file_name'}
                    if not required.issubset(args) or not set(args).issubset(allowed):raise ValueError('Informe slug, artifact_id, download_url e file_id.')
                    slug=str(args.get('slug') or '').strip();artifact_id=str(args.get('artifact_id') or '').strip()
                    if not re.fullmatch(r'[a-z0-9][a-z0-9-]*',slug) or not re.fullmatch(r'art_[a-f0-9]{24}',artifact_id):raise ValueError('slug ou artifact_id inválido.')
                    file_ref={'download_url':str(args.get('download_url') or ''),'file_id':str(args.get('file_id') or '')}
                    if args.get('mime_type'):file_ref['mime_type']=str(args['mime_type'])
                    if args.get('file_name'):file_ref['file_name']=str(args['file_name'])
                    control('/v1/projects/'+urllib.parse.quote(slug,safe=''))
                    content,file_meta=workspace_artifact_upload_existing_https(slug,artifact_id,file_ref,trace_id)
                    content['file_id']=file_meta['file_id'];content['input_field']='widget_resolved_file';content['file_resolution']='window.openai.getFileDownloadUrl';content['mcp_identity_reused']=True;content['oauth_identity_reused']=bool(getattr(self,'_oauth',None));content['authentication_context']='oauth_mcp' if getattr(self,'_oauth',None) else 'mcp_client';content['portal_cookie_required']=False;content['browser_session_required']=False;content['download_url_persisted']=False;content['secrets_exposed']=False
