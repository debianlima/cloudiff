...
ARTIFACT_UPLOAD_WIDGET_HTML=r'''<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>
:root{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#111;background:#fff}*{box-sizing:border-box}body{margin:0;padding:24px}.wrap{max-width:520px;margin:0 auto}.eyebrow{font-size:12px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:#666}.title{font-size:24px;line-height:1.2;margin:10px 0 8px}.copy{font-size:14px;line-height:1.6;color:#555;margin:0 0 22px}.meta{font-size:13px;line-height:1.6;color:#444;margin:0 0 22px;word-break:break-word}.actions{display:flex;gap:10px;flex-wrap:wrap}.btn{appearance:none;border:0;border-radius:8px;padding:11px 16px;font:600 14px/1.2 inherit;cursor:pointer}.btn-primary{background:#168821;color:#fff}.btn-secondary{background:#f2f3f2;color:#111}.btn:disabled{opacity:.5;cursor:not-allowed}.status{margin-top:18px;font-size:13px;line-height:1.5;color:#555}.ok{color:#0b6b20}.bad{color:#a21d1d}@media(prefers-color-scheme:dark){:root{color:#edf2ee;background:#101a14}.copy,.meta,.status{color:#b8c4bb}.eyebrow{color:#9ba99f}.btn-secondary{background:#1b2a20;color:#edf2ee}.btn-primary{background:#39d353;color:#061009}.ok{color:#62e67b}.bad{color:#ff8f8f}}
</style></head><body><main class="wrap"><div class="eyebrow">CloudIFF</div><h1 class="title">Selecionar arquivo</h1><p class="copy">Escolha o arquivo já enviado ao ChatGPT. A URL temporária é resolvida pelo próprio host e usada somente para este upload.</p><div class="meta" id="meta">Aguardando dados do artifact…</div><div class="actions"><button class="btn btn-primary" id="select">Selecionar do ChatGPT</button><button class="btn btn-secondary" id="local">Escolher do dispositivo</button><input id="picker" type="file" hidden></div><div class="status" id="status"></div></main><script>
(()=>{const meta=document.getElementById('meta'),status=document.getElementById('status'),select=document.getElementById('select'),local=document.getElementById('local'),picker=document.getElementById('picker');let latestInput={},latestOutput={};const globals=()=>window.openai||{};const readState=()=>({...latestInput,...(globals().toolInput||{}),...latestOutput,...(globals().toolOutput||{})});const setStatus=(t,c='')=>{status.textContent=t;status.className='status '+c;globals().notifyIntrinsicHeight?.()};const render=()=>{const i=readState();meta.textContent=i.artifact_id?`Projeto: ${i.slug||'—'} · Artifact: ${i.artifact_id}${i.filename?' · '+i.filename:''}`:'Aguardando dados do artifact…';if(i.status==='sealed')setStatus(`Upload concluído${i.received_bytes!=null?' · '+i.received_bytes+' bytes':''}.`,'ok');globals().notifyIntrinsicHeight?.()};window.addEventListener('openai:set_globals',e=>{const g=e?.detail?.globals||{};if(g.toolInput)latestInput=g.toolInput;if(g.toolOutput)latestOutput=g.toolOutput;render()},{passive:true});const send=async file=>{const i=readState();if(!i.slug||!i.artifact_id)throw new Error('slug/artifact_id ainda não foram disponibilizados pelo host.');if(!globals().getFileDownloadUrl||!globals().callTool)throw new Error('Este host não expõe as APIs de arquivo necessárias.');setStatus('Obtendo URL temporária…');const r=await globals().getFileDownloadUrl({fileId:file.fileId});if(!r?.downloadUrl)throw new Error('O host não retornou downloadUrl.');setStatus('Enviando para a CloudIFF…');const out=await globals().callTool('workspace.artifact.upload.file.resolve',{slug:i.slug,artifact_id:i.artifact_id,file:{file_id:file.fileId,download_url:r.downloadUrl,mime_type:file.mimeType||'',file_name:file.fileName||''}});const final=out?.structuredContent||out?.toolOutput||out||{};latestOutput={...latestOutput,...final};render();if(final.status==='sealed')setStatus(`Arquivo validado e selado${final.received_bytes!=null?' · '+final.received_bytes+' bytes':''}.`,'ok');else setStatus('Arquivo enviado. Verificando estado…','ok');globals().sendFollowUpMessage?.({prompt:`O upload do artifact ${i.artifact_id} foi concluído pelo componente CloudIFF. Consulte workspace.artifact.upload.status e, se estiver sealed, prossiga com workspace.artifact.commit.plan.`,scrollToBottom:true});return out};select.onclick=async()=>{try{if(!globals().selectFiles)throw new Error('A biblioteca de arquivos não está disponível neste ChatGPT. Use “Escolher do dispositivo”.');const files=await globals().selectFiles();if(!files?.length){setStatus('Nenhum arquivo selecionado.');return}await send(files[0])}catch(e){setStatus(e?.message||'Falha ao selecionar o arquivo.','bad')}};local.onclick=()=>picker.click();picker.onchange=async()=>{try{const f=picker.files?.[0];if(!f)return;if(!globals().uploadFile)throw new Error('uploadFile não está disponível neste host.');setStatus('Autorizando arquivo no ChatGPT…');const u=await globals().uploadFile(f,{library:false});if(!u?.fileId)throw new Error('O host não retornou fileId.');await send({fileId:u.fileId,fileName:f.name,mimeType:f.type||'application/octet-stream'})}catch(e){setStatus(e?.message||'Falha ao autorizar o arquivo.','bad')}};render()})();
</script></body></html>'''
...
TOOLS=[
 ...
 {'name':'workspace.artifact.upload.start','description':'Inicia upload temporário de artefato binário e, para clientes identificados, já retorna artifact_id + upload_url HTTPS do Portal. É o fallback recomendado quando openai/fileParams chegar como /mnt/data; não requer Base64 nem chamada separada de ticket.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'filename':{'type':'string','minLength':1,'maxLength':240},'expected_size':{'type':'integer','minimum':0,'maximum':1073741824},'expected_sha256':{'type':'string','pattern':'^[a-f0-9]{64}$'},'ttl_seconds':{'type':'integer','minimum':300,'maximum':86400}},'required':['slug','filename','expected_size','expected_sha256'],'additionalProperties':False,'examples':[{'slug':'meu-projeto','filename':'documentos-anonimizados.zip','expected_size':1390970,'expected_sha256':'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','ttl_seconds':3600}]}},
 ...
 {'name':'workspace.artifact.import','title':'Importar arquivo da conversa','description':'CAMINHO CANÔNICO para arquivo anexado ao ChatGPT. Quando openai/fileParams vier hidratado, a CloudIFF baixa o objeto {download_url,file_id,...} por HTTPS. Se o host ainda enviar /mnt/data, sandbox: ou file://, a própria CloudIFF converte automaticamente a chamada em upload direto pelo Portal e devolve artifact_id + upload_url, sem tentar acessar o filesystem do host.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'file':{'type':'object','description':'Arquivo anexado/selecionado no ChatGPT. Este valor é preenchido pelo host MCP, não pelo modelo como caminho local.','properties':{'download_url':{'type':'string'},'file_id':{'type':'string'},'mime_type':{'type':'string'},'file_name':{'type':'string'}},'required':['download_url','file_id'],'additionalProperties':False},'filename':{'type':'string','minLength':1,'maxLength':240},'expected_size':{'type':'integer','minimum':0,'maximum':1073741824},'expected_sha256':{'type':'string','pattern':'^[a-f0-9]{64}$'},'ttl_seconds':{'type':'integer','minimum':300,'maximum':86400}},'required':['slug','file','filename','expected_size','expected_sha256'],'additionalProperties':False,'examples':[{'slug':'meu-projeto','file':{'download_url':'https://files.oaiusercontent.com/...','file_id':'file_1111111111111111'},'filename':'documentos-anonimizados.zip','expected_size':1390970,'expected_sha256':'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa','ttl_seconds':3600}]},'_meta':{'openai/fileParams':['file']}},
 {'name':'workspace.artifact.upload.file','title':'Enviar arquivo para artifact existente','description':'Upload binário server-side autenticado pela identidade OAuth MCP. O caminho principal usa openai/fileParams. Se o host enviar /mnt/data como string, a própria resposta monta o componente MCP Apps, que recebe structuredContent e resolve fileId + downloadUrl pelas APIs oficiais do ChatGPT.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'},'file':{'type':'object','properties':{'download_url':{'type':'string'},'file_id':{'type':'string'},'mime_type':{'type':'string'},'file_name':{'type':'string'}},'required':['download_url','file_id'],'additionalProperties':False}},'required':['slug','artifact_id','file'],'additionalProperties':False},'outputSchema':{'type':'object','properties':{'ok':{'type':'boolean'},'slug':{'type':'string'},'artifact_id':{'type':'string'},'filename':{'type':'string'},'expected_size':{'type':'integer'},'expected_sha256':{'type':'string'},'status':{'type':'string'},'received_bytes':{'type':'integer'},'sha256':{'type':'string'},'picker_required':{'type':'boolean'},'model_fileparams_required':{'type':'boolean'},'ui_resource':{'type':'string'},'next_tool':{'type':'string'},'sealed':{'type':'boolean'}},'required':['ok','slug','artifact_id','status','picker_required','model_fileparams_required','ui_resource','next_tool','sealed'],'additionalProperties':False},'_meta':{'openai/fileParams':['file'],'ui':{'resourceUri':ARTIFACT_UPLOAD_WIDGET_URI,'visibility':['model','app']},'openai/outputTemplate':ARTIFACT_UPLOAD_WIDGET_URI}},
 {'name':'workspace.artifact.upload.file.select','title':'Selecionar arquivo no ChatGPT','description':'Renderiza o componente MCP Apps para selecionar um arquivo autorizado do ChatGPT para um artifact_id existente. Não altera o artifact até a seleção e resolução do arquivo.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'}},'required':['slug','artifact_id'],'additionalProperties':False},'outputSchema':{'type':'object','properties':{'ok':{'type':'boolean'},'slug':{'type':'string'},'artifact_id':{'type':'string'},'filename':{'type':'string'},'expected_size':{'type':'integer'},'expected_sha256':{'type':'string'},'status':{'type':'string'},'received_bytes':{'type':'integer'},'sha256':{'type':'string'},'picker_required':{'type':'boolean'},'model_fileparams_required':{'type':'boolean'},'ui_resource':{'type':'string'},'next_tool':{'type':'string'},'sealed':{'type':'boolean'}},'required':['ok','slug','artifact_id','status','picker_required','model_fileparams_required','ui_resource','next_tool','sealed'],'additionalProperties':False},'_meta':{'ui':{'resourceUri':ARTIFACT_UPLOAD_WIDGET_URI,'visibility':['model','app']},'openai/outputTemplate':ARTIFACT_UPLOAD_WIDGET_URI}},
 {'name':'workspace.artifact.upload.file.resolve','title':'Resolver arquivo autorizado','description':'Helper privado do componente CloudIFF: recebe file estruturado com file_id + download_url obtidos diretamente das APIs de arquivo do ChatGPT e grava no artifact existente.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'},'file':{'type':'object','properties':{'download_url':{'type':'string'},'file_id':{'type':'string'},'mime_type':{'type':'string'},'file_name':{'type':'string'}},'required':['download_url','file_id'],'additionalProperties':False}},'required':['slug','artifact_id','file'],'additionalProperties':False},'outputSchema':{'type':'object','properties':{'ok':{'type':'boolean'},'slug':{'type':'string'},'artifact_id':{'type':'string'},'filename':{'type':'string'},'expected_size':{'type':'integer'},'expected_sha256':{'type':'string'},'status':{'type':'string'},'received_bytes':{'type':'integer'},'sha256':{'type':'string'},'picker_required':{'type':'boolean'},'model_fileparams_required':{'type':'boolean'},'ui_resource':{'type':'string'},'next_tool':{'type':'string'},'sealed':{'type':'boolean'}},'required':['ok','slug','artifact_id','status','picker_required','model_fileparams_required','ui_resource','next_tool','sealed'],'additionalProperties':False},'_meta':{'ui':{'visibility':['app']},'openai/widgetAccessible':True,'openai/visibility':'private'}},
 {'name':'workspace.artifact.upload.ticket','description':'Emite uma credencial temporária de upload do Portal, de uso único e vinculada ao artifact, após autorização MCP. A upload_url guarda a credencial apenas no fragmento (#), não exige cookie/login do Portal e não expõe OAuth. O Portal valida tamanho e SHA-256 antes de selar.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'},'ttl_seconds':{'type':'integer','minimum':60,'maximum':7200}},'required':['slug','artifact_id'],'additionalProperties':False,'examples':[{'slug':'meu-projeto','artifact_id':'art_111111111111111111111111','ttl_seconds':3600}]}},
 ...
 {'name':'workspace.artifact.commit.plan','description':'CAMINHO DIRETO para transformar um artifact_id já selado em um change set de um único arquivo pronto para aprovação e commit em branch/PR. Verifica o upload, monta create/update, valida o workspace e retorna workspace_id + change_set_digest + argumentos exatos da próxima aprovação.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'},'path':{'type':'string','minLength':1,'maxLength':240},'operation':{'type':'string','enum':['create','update']},'expected_sha256':{'type':'string','pattern':'^[a-f0-9]{64}$'},'title':{'type':'string','minLength':4,'maxLength':160},'description':{'type':'string','maxLength':4000},'ref':{'type':'string','pattern':'^[A-Za-z0-9._/-]+$'},'ttl_seconds':{'type':'integer','minimum':300,'maximum':86400}},'required':['slug','artifact_id','path','operation','title','description'],'additionalProperties':False,'examples':[{'slug':'meu-projeto','artifact_id':'art_111111111111111111111111','path':'uploads/documentos.zip','operation':'create','title':'Adicionar documentos enviados','description':'Commit do upload validado pelo CloudIFF.','ref':'main','ttl_seconds':3600}]}},
...
def _example_for_schema(schema, field='', include_optional=True, depth=0, root=None):
    ...
    if ref.startswith('#/$defs/'):
        ...
        try:
            ...
            return None
        return _example_for_schema(target,field,include_optional,depth+1,root)
    if 'const' in schema:return schema['const']
    enum=schema.get('enum')
    if isinstance(enum,list) and enum:return enum[0]
    variants=schema.get('oneOf') or schema.get('anyOf') or []
    if variants:return _example_for_schema(variants[0],field,include_optional,depth+1,root)
    ...
    if kind=='object' or ('properties' in schema and kind is None):
        ...
        for name,child in properties.items():
            if include_optional or name in required:
                result[name]=_example_for_schema(child,name,include_optional,depth+1,root)
        return result
    if kind=='array':
        item=_example_for_schema(schema.get('items') or {},field,include_optional,depth+1,root)
...
def _schema_contract(schema, required=False, depth=0, root=None):
    ...
    if ref.startswith('#/$defs/'):
        ...
        try:
            for part in ref[2:].split('/'):
                target=target[part]
        except Exception:return {'type':'any','required':bool(required)}
        return _schema_contract(target,required,depth+1,root)
    ...
    if schema.get('items') is not None:
        result['items']=_schema_contract(schema.get('items') or {},False,depth+1,root)
    if schema.get('properties'):
        child_required=set(schema.get('required') or [])
        result['properties']={name:_schema_contract(child,name in child_required,depth+1,root) for name,child in schema['properties'].items()}
    variants=schema.get('oneOf') or schema.get('anyOf')
    if variants:result['variants']=[_schema_contract(item,False,depth+1,root) for item in variants[:6]]
    return result
...
def tool_usage(tool_name):
    ...
    return {
        ...
        'parameters':{name:_schema_contract(child,name in set(required),root=schema) for name,child in properties.items()},
        'additionalPropertiesAllowed':schema.get('additionalProperties',True) is not False,
        'minimumExample':_example_for_schema(schema,'',False,root=schema),
        'completeExample':(schema.get('examples') or [None])[0] or _example_for_schema(schema,'',True,root=schema),
    }
...
READ_ONLY_TOOLS={
 'project.list','project.get','project.connectors','project.technologies.detect','project.manifest.validate','project.configuration.get','project.configuration.status','project.configuration.drift','project.configuration.reconcile.plan','project.observability.get','project.observability.alerts','project.environment.list','project.environment.get','project.environment.validate','project.environment.change.plan','project.environment.promote.plan','project.environment.history','project.environment.import.plan','project.environment.export','project.environment.secret.list','project.environment.secret.history','project.environment.secret.rotate.plan','project.environment.secret.revoke.plan','project.environment.secret.promote.plan','project.environment.secret.read.plan','workspace.normalize.plan','workspace.change-set.validate','workspace.artifact.commit.plan','workspace.artifact.upload.file.select','forgejo.proposal.change-set.plan','runtime.catalog','runtime.detect','runtime.plan','runtime.validate',
...
def _project_connector_catalog(identity,result):
    ...
    data['mcp']={
      ...
      'file_picker_bridge':{
        'tool':'workspace.artifact.upload.file.select',
        'helper':'workspace.artifact.upload.file.resolve',
        'authorized':'workspace.artifact.upload.file.select' in identity['tools'],
        'resource_uri':ARTIFACT_UPLOAD_WIDGET_URI,
        'host_apis':['selectFiles','uploadFile','getFileDownloadUrl','callTool'],
        'bypasses_model_fileparams_binding':True,
        'requires_portal_cookie':False,
      },
    }
    return data
...
class _SessionFileNoRedirect(urllib.request.HTTPRedirectHandler):
...
def _prepare_openai_file_param(tool_name, args):
    if tool_name not in {'workspace.artifact.import','workspace.artifact.upload.file'} or not isinstance(args,dict) or 'file' not in args:
        return args
...
def _session_file_download(download_url: str, expected_size: int):
    ...
    for redirects in range(4):
        ...
        request=urllib.request.Request(current,headers={'Accept':'application/octet-stream,*/*;q=0.8','User-Agent':'CloudIFF-Artifact-Importer/1.0'})
        try:
            with opener.open(request,timeout=120) as response:
                if int(response.getcode() or 0)!=200:raise ValueError('session_file_download_rejected')
                ...
                raw=response.read(int(expected_size)+1)
                return raw,response.headers
        except urllib.error.HTTPError as error:
            if error.code in {301,302,303,307,308}:
                location=str(error.headers.get('Location') or '').strip()
                if not location:raise ValueError('session_file_redirect_invalid') from error
...
def workspace_artifact_upload_existing_https(slug,artifact_id,file_ref,trace_id):
    ...
    status_code,status_data=workspace_broker_post('/v1/artifact/upload/status',{'artifact_id':artifact_id},timeout=30)
    if status_code not in {200,201} or not status_data.get('ok'):raise ValueError('artifact_status_unavailable')
    artifact=status_data.get('result') or status_data
    if str(artifact.get('project_slug') or '')!=slug:raise PermissionError('artifact_project_mismatch')
    if str(artifact.get('status') or '')!='uploading':
        if str(artifact.get('status') or '')=='sealed':
            return {**artifact,'uploaded':False,'idempotent':True,'ready_for_change_set':True,'source':'mcp_oauth_file_stream','download_url_persisted':False,'secrets_exposed':False}, {'file_id':file_id,'file_name':file_name,'mime_type':str(normalized.get('mime_type') or ''),'reference_mode':str(normalized.get('reference_mode') or 'unknown')}
        raise ToolStateError({'code':'artifact_not_uploading','message':'O artifact não está em estado de upload.','artifact_id':artifact_id,'status':artifact.get('status')})
    expected_size=int(artifact.get('expected_size') if artifact.get('expected_size') is not None else -1);expected_sha256=str(artifact.get('expected_sha256') or '').strip().lower();expected_filename=str(artifact.get('filename') or '').strip()
    if not (0<=expected_size<=ARTIFACT_MAX_BYTES) or not re.fullmatch(r'[a-f0-9]{64}',expected_sha256):raise ValueError('artifact_expected_integrity_invalid')
    if file_name and expected_filename and file_name!=expected_filename:raise ValueError('session_file_name_mismatch')
    ...
    if str(result.get('project_slug') or '')!=slug or str(result.get('artifact_id') or '')!=artifact_id:raise ValueError('artifact_upload_identity_mismatch')
    if str(result.get('status') or '')!='sealed' or int(result.get('size') or -1)!=expected_size or not hmac.compare_digest(str(result.get('sha256') or ''),expected_sha256):raise ValueError('artifact_upload_integrity_failed')
    mode=str(normalized.get('reference_mode') or 'unknown')
    result.update({'uploaded':True,'transport':'mcp_oauth_file_stream','source':'openai_file_param','file_id':file_id,'file_reference_mode':mode,'download_url_persisted':False,'secrets_exposed':False,'ready_for_change_set':True,'next_tool':'workspace.artifact.commit.plan'})
...
SCOPE_BY_TOOL={
 ...
 'forgejo.propose-edit':'forgejo:propose-edit','forgejo.propose-edit.plan':'forgejo:plan-edit','approval.request-proposal':'approval:request-proposal','workspace.normalize.plan':'workspace:change-set-plan','workspace.artifact.upload.start':'workspace:change-set-plan','workspace.artifact.upload.chunk':'workspace:change-set-plan','workspace.artifact.upload.batch':'workspace:change-set-plan','workspace.artifact.import':'workspace:change-set-plan','workspace.artifact.upload.file':'workspace:change-set-plan','workspace.artifact.upload.file.select':'workspace:change-set-plan','workspace.artifact.upload.file.resolve':'workspace:change-set-plan','workspace.artifact.upload.ticket':'workspace:change-set-plan','workspace.artifact.upload.status':'workspace:change-set-plan','workspace.artifact.commit.plan':'workspace:change-set-plan','workspace.artifact.upload.complete':'workspace:change-set-plan','workspace.change-set.validate':'workspace:change-set-plan','forgejo.proposal.change-set.plan':'workspace:change-set-plan','approval.request-change-set-proposal':'approval:request-change-set','forgejo.proposal.change-set.create':'forgejo:propose-change-set','approval.get':'approval:read-own','approval.cancel':'approval:read-own','forgejo.proposal.list':'forgejo:proposal-read','forgejo.proposal.get':'forgejo:proposal-read','forgejo.proposal.ready-for-review':'forgejo:proposal-merge','forgejo.proposal.close':'forgejo:proposal-close','forgejo.proposal.delete-branch':'forgejo:proposal-delete-branch','forgejo.proposal.merge.plan':'forgejo:proposal-merge-plan','approval.request-merge':'approval:request-merge','forgejo.proposal.merge':'forgejo:proposal-merge',
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            call_tool=tool;call_args=args;auth_args=args
            ...
            if method=='initialize':result={'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'1.0.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}
                ...
                if uri==ARTIFACT_UPLOAD_WIDGET_URI:
                    result={'contents':[{'uri':uri,'mimeType':'text/html;profile=mcp-app','text':ARTIFACT_UPLOAD_WIDGET_HTML,'_meta':{'ui':{'prefersBorder':True,'csp':{'connectDomains':[],'resourceDomains':[]}},'openai/widgetDescription':'Selecione um arquivo já autorizado pelo ChatGPT para enviá-lo diretamente ao artifact CloudIFF.','openai/widgetPrefersBorder':True}}]}
                else:
                ...
                if name=='project.list': data=control('/v1/projects');allowed=set(authz.get('project_slugs') or []);content=[x for x in data.get('projects',[]) if not allowed or x.get('slug') in allowed]
                    ...
                    slug=str(args.get('slug') or '').strip();artifact_id=str(args.get('artifact_id') or '').strip();file_ref=args.get('file')
                    if not re.fullmatch(r'[a-z0-9][a-z0-9-]*',slug) or not re.fullmatch(r'art_[a-f0-9]{24}',artifact_id):raise ValueError('slug ou artifact_id inválido.')
                    print(json.dumps({'event':'mcp_file_param_shape','tool':'workspace.artifact.upload.file',**_session_file_ref_shape(file_ref)},separators=(',',':')),flush=True)
                    control('/v1/projects/'+urllib.parse.quote(slug,safe=''))
                    content,file_meta=workspace_artifact_upload_existing_https(slug,artifact_id,file_ref,trace_id)
                    content['slug']=slug;content['file_id']=file_meta['file_id'];content['input_field']='file';content['mcp_identity_reused']=True;content['oauth_identity_reused']=bool(getattr(self,'_oauth',None));content['authentication_context']='oauth_mcp' if getattr(self,'_oauth',None) else 'mcp_client';content['portal_cookie_required']=False;content['browser_session_required']=False;content['download_url_persisted']=False;content['secrets_exposed']=False
                elif name=='workspace.artifact.upload.file.select':
                    if set(args)!={'slug','artifact_id'}:raise ValueError('Informe slug e artifact_id.')
                    slug=str(args.get('slug') or '').strip();artifact_id=str(args.get('artifact_id') or '').strip()
                    if not re.fullmatch(r'[a-z0-9][a-z0-9-]*',slug) or not re.fullmatch(r'art_[a-f0-9]{24}',artifact_id):raise ValueError('slug ou artifact_id inválido.')
                    control('/v1/projects/'+urllib.parse.quote(slug,safe=''))
                    code,data=workspace_broker_post('/v1/artifact/upload/status',{'artifact_id':artifact_id},timeout=30)
                    if code not in {200,201} or not data.get('ok'):raise ValueError('artifact_status_unavailable')
                    artifact=data.get('result') or data
                    if str(artifact.get('project_slug') or '')!=slug:raise PermissionError('artifact_project_mismatch')
                    content={'ok':True,'picker_required':str(artifact.get('status') or '')!='sealed','artifact_id':artifact_id,'slug':slug,'status':artifact.get('status'),'filename':artifact.get('filename'),'expected_size':artifact.get('expected_size'),'expected_sha256':artifact.get('expected_sha256'),'ui_resource':ARTIFACT_UPLOAD_WIDGET_URI,'host_file_apis':['selectFiles','uploadFile','getFileDownloadUrl','callTool'],'model_fileparams_required':False,'next_tool':'workspace.artifact.upload.file.resolve' if str(artifact.get('status') or '')!='sealed' else 'workspace.artifact.commit.plan'}
                elif name=='workspace.artifact.upload.file.resolve':
                    if set(args)!={'slug','artifact_id','file'} or not isinstance(args.get('file'),dict):raise ValueError('Informe slug, artifact_id e file.')
                    slug=str(args.get('slug') or '').strip();artifact_id=str(args.get('artifact_id') or '').strip();file_ref=args.get('file') or {}
                    if not re.fullmatch(r'[a-z0-9][a-z0-9-]*',slug) or not re.fullmatch(r'art_[a-f0-9]{24}',artifact_id):raise ValueError('slug ou artifact_id inválido.')
                    if not {'download_url','file_id'}.issubset(file_ref) or not set(file_ref).issubset({'download_url','file_id','mime_type','file_name'}):raise ValueError('file deve conter download_url e file_id.')
                    control('/v1/projects/'+urllib.parse.quote(slug,safe=''))
                    content,file_meta=workspace_artifact_upload_existing_https(slug,artifact_id,file_ref,trace_id)
                    content['slug']=slug;content['file_id']=file_meta['file_id'];content['input_field']='widget_resolved_file';content['file_resolution']='window.openai.getFileDownloadUrl';content['mcp_identity_reused']=True;content['oauth_identity_reused']=bool(getattr(self,'_oauth',None));content['authentication_context']='oauth_mcp' if getattr(self,'_oauth',None) else 'mcp_client';content['portal_cookie_required']=False;content['browser_session_required']=False;content['download_url_persisted']=False;content['secrets_exposed']=False
                    ...
                    if name=='workspace.artifact.upload.start':
                        ...
                        client_id=self.headers.get('X-CloudIF-Client','').strip()
                        if re.fullmatch(r'art_[a-f0-9]{24}',artifact_id) and client_id:
                            ticket_ttl=max(60,min(int(payload.get('ttl_seconds') or 3600),7200))
                            ticket_code,ticket_data=workspace_broker_post('/v1/artifact/ticket',{'project_slug':slug,'trace_id':trace_id,'artifact_id':artifact_id,'requested_by':client_id,'ttl_seconds':ticket_ttl},timeout=30)
                            if ticket_code in {200,201} and ticket_data.get('ok'):
                                ticket_result=ticket_data.get('result') or ticket_data
                                token=str(ticket_result.get('upload_ticket') or '')
                                if re.fullmatch(r'upt_[a-f0-9]{24}_[a-f0-9]{48}',token):
                    ...
                    if name=='workspace.artifact.upload.ticket':
                        ...
                        content['upload_url']=PUBLIC_ORIGIN+'/cloudiff/portal/artifact-upload-capability#'+urllib.parse.quote(token,safe='')
                        content['user_action_required']=True;content['upload_method']='portal_capability_direct';content['browser_secret_required']=False;content['secrets_exposed']=False
                        content['upload_url_audience']='human_user';content['agent_must_not_open_upload_url']=True;content['agent_followup_tool']='workspace.artifact.upload.status';content['human_portal_authentication_required']=False;content['portal_cookie_required']=False;content['csrf_required']=False;content['authentication']='mcp_delegated_one_time_portal_capability';content['credential_in_fragment']=True;content['next_action']='Entregue upload_url ao usuário. O link não exige login/cookie do Portal; a credencial temporária é validada pelo backend e expira ou é invalidada após uso.'
                elif name=='workspace.artifact.upload.status':
                    required={'slug','artifact_id'};allowed=required
                    if not required.issubset(args) or not set(args).issubset(allowed):raise ValueError('Informe slug e artifact_id.')
                    slug=str(args.get('slug') or '').strip();artifact_id=str(args.get('artifact_id') or '').strip()
                    if not re.fullmatch(r'[a-z0-9][a-z0-9-]*',slug) or not re.fullmatch(r'art_[a-f0-9]{24}',artifact_id):raise ValueError('Metadados do status inválidos.')
                    control('/v1/projects/'+urllib.parse.quote(slug,safe=''))
                    code,data=workspace_broker_post('/v1/artifact/upload/status',{'artifact_id':artifact_id},timeout=30)
                    if code not in {200,201} or not data.get('ok'):
                        error=data.get('error') if isinstance(data,dict) else None
                        message=(error.get('message') if isinstance(error,dict) else str(error or '')) or 'Não foi possível consultar o upload.'
                        raise ValueError(message)
                    result=data.get('result') or data
                    if str(result.get('project_slug') or '')!=slug:raise PermissionError('artifact_project_mismatch')
                    content={k:result.get(k) for k in ('artifact_id','project_slug','filename','status','expected_size','expected_sha256','received_bytes','expires_at','upload_ticket_status','upload_ticket_expires_at','size','sha256','sealed_at','upload_transport') if result.get(k) is not None}
                    content['ready_for_change_set']=str(content.get('status') or '')=='sealed'
                    if content['ready_for_change_set']:
                        content['next_action']='Para commit direto de um único upload, use este artifact_id em workspace.artifact.commit.plan. Para múltiplos arquivos, use workspace.change-set.validate.'
                    else:
                        content['next_action']='Aguardando o usuário concluir o upload no Portal. Não abra a upload_url como agente.'
                    content['agent_must_not_open_upload_url']=True;content['secrets_exposed']=False
                elif name=='workspace.artifact.commit.plan':
                    required={'slug','artifact_id','path','operation','title','description'};allowed=required|{'expected_sha256','ref','ttl_seconds'}
                    if not required.issubset(args) or not set(args).issubset(allowed):raise ValueError('Informe slug, artifact_id, path, operation, title e description.')
                    slug=str(args.get('slug') or '').strip();artifact_id=str(args.get('artifact_id') or '').strip();path_value=str(args.get('path') or '').strip();operation=str(args.get('operation') or '').strip();title=str(args.get('title') or '').strip();description=str(args.get('description') or '');ref=str(args.get('ref') or 'main').strip();ttl=int(args.get('ttl_seconds') or 3600);expected=str(args.get('expected_sha256') or '').strip().lower()
                    if not re.fullmatch(r'[a-z0-9][a-z0-9-]*',slug) or not re.fullmatch(r'art_[a-f0-9]{24}',artifact_id):raise ValueError('slug ou artifact_id inválido.')
                    if operation not in {'create','update'} or not path_value or path_value.startswith('/') or '..' in path_value.split('/') or len(path_value)>240:raise ValueError('operation/path inválido.')
                    if operation=='update' and not re.fullmatch(r'[a-f0-9]{64}',expected):raise ValueError('expected_sha256 é obrigatório para update.')
                    if operation=='create' and expected:raise ValueError('expected_sha256 não deve ser enviado em create.')
                    if not ref or '..' in ref or ref.startswith('/') or ref.endswith('/') or not (4<=len(title)<=160) or len(description)>4000 or not (300<=ttl<=86400):raise ValueError('ref, title, description ou ttl_seconds incompatível.')
                    client_id=self.headers.get('X-CloudIF-Client','').strip()
                    if not client_id:raise ValueError('identified_client_required')
                    control('/v1/projects/'+urllib.parse.quote(slug,safe=''))
                    status_code,status_data=workspace_broker_post('/v1/artifact/upload/status',{'artifact_id':artifact_id},timeout=30)
                    ...
                    artifact=status_data.get('result') or status_data
                    if str(artifact.get('project_slug') or '')!=slug:raise PermissionError('artifact_project_mismatch')
                    if str(artifact.get('status') or '')!='sealed':raise ToolStateError({'code':'artifact_not_sealed','message':'O upload ainda não está selado. Conclua o envio no Portal e repita workspace.artifact.commit.plan.','artifact_id':artifact_id,'status':artifact.get('status'),'next_tool':'workspace.artifact.upload.status'})
                    artifact_size=int(artifact.get('size') if artifact.get('size') is not None else artifact.get('expected_size') or 0)
                    if artifact_size>64*1024*1024:raise ToolStateError({'code':'artifact_too_large_for_forgejo_commit','message':'O artifact está selado, mas excede 64 MiB, limite atual para commit binário no Forgejo.','artifact_id':artifact_id,'size':artifact_size,'maximum_size':64*1024*1024})
                    change={'operation':operation,'path':path_value,'artifact_id':artifact_id}
                    if operation=='update':change['expected_sha256']=expected
                    validate_payload={'project_slug':slug,'ref':ref,'trace_id':trace_id,'title':title,'description':description,'changes':[change],'ttl_seconds':ttl}
                    code,data=workspace_broker_post('/v1/change-set/validate',validate_payload,timeout=180)
                    if code not in {200,422} or not data.get('ok'):
                    ...
                    validated=data.get('result') or data
                    workspace_id=str(validated.get('workspace_id') or '');digest_value=str(validated.get('change_set_digest') or '').lower()
                    if not re.fullmatch(r'ws_[a-f0-9]{24}',workspace_id) or not re.fullmatch(r'[a-f0-9]{64}',digest_value):raise ValueError('change_set_validation_incomplete')
                    sealed=change_set_resolve(slug,workspace_id,digest_value,trace_id)
                    if sealed.get('ref')!='main':raise ValueError('Commit controlado só pode ser criado a partir da referência main.')
                    operation_plan={'action':'forgejo.propose-change-set','project_slug':slug,'base_branch':'main','workspace_id':workspace_id,'change_set_digest':digest_value,'archive_sha256':sealed.get('archive_sha256'),'summary':sealed.get('summary') or {}}
                    approval_args={'slug':slug,'workspace_id':workspace_id,'change_set_digest':digest_value,'reason':'Commitar o upload validado no repositório'}
                    create_args={'slug':slug,'workspace_id':workspace_id,'change_set_digest':digest_value,'approval_id':'<approval_id>'}
                    content={'ok':True,'side_effect_free':True,'artifact':{k:artifact.get(k) for k in ('artifact_id','filename','status','size','sha256','expected_size','expected_sha256') if artifact.get(k) is not None},'operation':operation_plan,'workspace_id':workspace_id,'change_set_digest':digest_value,'validation':validated,'next_tool':'approval.request-change-set-proposal','next_arguments':approval_args,'after_approval_tool':'forgejo.proposal.change-set.create','after_approval_arguments':create_args,'commit_target':'proposal_branch','main_modified':False}
                ...
                if requested_tool in {'workspace.artifact.upload.file','workspace.artifact.upload.file.select','workspace.artifact.upload.file.resolve'} or name in {'workspace.artifact.upload.file','workspace.artifact.upload.file.select','workspace.artifact.upload.file.resolve'}:
                    status_value=str(content.get('status') or 'uploading');artifact_value=str(content.get('artifact_id') or args.get('artifact_id') or '');slug_value=str(content.get('slug') or content.get('project_slug') or args.get('slug') or '')
                    structured={'ok':bool(content.get('ok',True)),'slug':slug_value,'artifact_id':artifact_value,'status':status_value,'picker_required':bool(content.get('picker_required',False)),'model_fileparams_required':bool(content.get('model_fileparams_required',False)),'ui_resource':ARTIFACT_UPLOAD_WIDGET_URI,'next_tool':str(content.get('next_tool') or ('workspace.artifact.commit.plan' if status_value=='sealed' else 'workspace.artifact.upload.file.resolve')),'sealed':status_value=='sealed'}
                    filename_value=content.get('filename') or content.get('file_name');expected_size_value=content.get('expected_size');expected_sha_value=content.get('expected_sha256');received_value=content.get('received_bytes') if content.get('received_bytes') is not None else content.get('size');sha_value=content.get('sha256')
                    if filename_value is not None:structured['filename']=str(filename_value)
                    if expected_size_value is not None:structured['expected_size']=int(expected_size_value)
                    if expected_sha_value is not None:structured['expected_sha256']=str(expected_sha_value)
                    if received_value is not None:structured['received_bytes']=int(received_value)
                    if sha_value is not None:structured['sha256']=str(sha_value)
