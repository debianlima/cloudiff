...
TOOLS=[
 ...
 {'name':'workspace.artifact.upload.file','title':'Enviar arquivo para artifact existente','description':'Upload binário server-side autenticado pela identidade OAuth MCP já autorizada. Recebe artifact_id + file via openai/fileParams, baixa o arquivo temporário por HTTPS e grava diretamente no artifact existente pelo Workspace Broker, sem cookie do Portal, sem Base64 e sem expor tokens. O artifact é selado somente se tamanho e SHA-256 já registrados no artifact coincidirem integralmente.','inputSchema':{'type':'object','properties':{'slug':{'type':'string','pattern':'^[a-z0-9][a-z0-9-]*$'},'artifact_id':{'type':'string','pattern':'^art_[a-f0-9]{24}$'},'file':{'type':'object','description':'Arquivo anexado/selecionado no cliente MCP. O host deve hidratá-lo com download_url e file_id.','properties':{'download_url':{'type':'string'},'file_id':{'type':'string'},'mime_type':{'type':'string'},'file_name':{'type':'string'}},'required':['download_url','file_id'],'additionalProperties':False}},'required':['slug','artifact_id','file'],'additionalProperties':False,'examples':[{'slug':'meu-projeto','artifact_id':'art_111111111111111111111111','file':{'download_url':'https://files.oaiusercontent.com/...','file_id':'file_1111111111111111','file_name':'documentos-anonimizados.zip'}}]},'_meta':{'openai/fileParams':['file']}},
...
READ_ONLY_TOOLS={
 'project.list','project.get','project.connectors','project.technologies.detect','project.manifest.validate','project.configuration.get','project.configuration.status','project.configuration.drift','project.configuration.reconcile.plan','project.observability.get','project.observability.alerts','project.environment.list','project.environment.get','project.environment.validate','project.environment.change.plan','project.environment.promote.plan','project.environment.history','project.environment.import.plan','project.environment.export','project.environment.secret.list','project.environment.secret.history','project.environment.secret.rotate.plan','project.environment.secret.revoke.plan','project.environment.secret.promote.plan','project.environment.secret.read.plan','workspace.normalize.plan','workspace.change-set.validate','workspace.artifact.commit.plan','forgejo.proposal.change-set.plan','runtime.catalog','runtime.detect','runtime.plan','runtime.validate',
...
SCOPE_BY_TOOL={
 ...
 'forgejo.propose-edit':'forgejo:propose-edit','forgejo.propose-edit.plan':'forgejo:plan-edit','approval.request-proposal':'approval:request-proposal','workspace.normalize.plan':'workspace:change-set-plan','workspace.artifact.upload.start':'workspace:change-set-plan','workspace.artifact.upload.chunk':'workspace:change-set-plan','workspace.artifact.upload.batch':'workspace:change-set-plan','workspace.artifact.import':'workspace:change-set-plan','workspace.artifact.upload.file':'workspace:change-set-plan','workspace.artifact.upload.ticket':'workspace:change-set-plan','workspace.artifact.upload.status':'workspace:change-set-plan','workspace.artifact.commit.plan':'workspace:change-set-plan','workspace.artifact.upload.complete':'workspace:change-set-plan','workspace.change-set.validate':'workspace:change-set-plan','forgejo.proposal.change-set.plan':'workspace:change-set-plan','approval.request-change-set-proposal':'approval:request-change-set','forgejo.proposal.change-set.create':'forgejo:propose-change-set','approval.get':'approval:read-own','approval.cancel':'approval:read-own','forgejo.proposal.list':'forgejo:proposal-read','forgejo.proposal.get':'forgejo:proposal-read','forgejo.proposal.ready-for-review':'forgejo:proposal-merge','forgejo.proposal.close':'forgejo:proposal-close','forgejo.proposal.delete-branch':'forgejo:proposal-delete-branch','forgejo.proposal.merge.plan':'forgejo:proposal-merge-plan','approval.request-merge':'approval:request-merge','forgejo.proposal.merge':'forgejo:proposal-merge',
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not authenticated and method=='initialize':return self.sendj(200,{'jsonrpc':'2.0','id':rid,'result':{'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'0.7.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}})
        ...
        try:
            ...
            args,input_wrappers=_unwrap_tool_arguments(raw_args);artifact_import_upload_fallback=False
            try:
            ...
            self._audit_ctx={'event_id':uuid.uuid4().hex,'source':'mcp','action':str(tool or method or 'unknown'),'actor_type':'agent' if self.headers.get('X-CloudIF-Client') else 'api_client','actor_id':self.headers.get('X-CloudIF-User','') or authz.get('owner_user',''),'delegated_user_id':self.headers.get('X-CloudIF-Delegated-User',''),'client_id':self.headers.get('X-CloudIF-Client','internal'),'project_slug':slug,'trace_id':trace_id,'attrs':{'rpc_method':method,'requested_tool':requested_tool if method=='tools/call' else None,'effective_tool':tool if method=='tools/call' else None,'automatic_upload_fallback':artifact_import_upload_fallback,'quota':{'minute_calls':authz.get('minute_calls'),'daily_calls':authz.get('daily_calls')}},'_start':time.monotonic()}
            if method=='initialize':result={'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'0.7.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}
