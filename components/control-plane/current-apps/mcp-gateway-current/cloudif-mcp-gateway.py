...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not authenticated and method=='initialize':return self.sendj(200,{'jsonrpc':'2.0','id':rid,'result':{'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'0.6.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}})
        ...
        try:
            ...
            if method=='initialize':result={'protocolVersion':'2025-03-26','serverInfo':{'name':'cloudif-mcp-gateway','version':'0.6.0'},'capabilities':{'tools':{},'resources':{},'prompts':{}},'instructions':AGENT_INSTRUCTIONS}
                ...
                if name=='project.list': data=control('/v1/projects');allowed=set(authz.get('project_slugs') or []);content=[x for x in data.get('projects',[]) if not allowed or x.get('slug') in allowed]
                        ...
                        if re.fullmatch(r'art_[a-f0-9]{24}',artifact_id) and client_id:
                            ...
                            if ticket_code in {200,201} and ticket_data.get('ok'):
                                ...
                                if re.fullmatch(r'upt_[a-f0-9]{24}_[a-f0-9]{48}',token):
                                    ...
                                    content['upload_method']='browser_direct';content['upload_ticket_created']=True;content['user_action_required']=True;content['browser_secret_required']=False;content['secrets_exposed']=False;content['upload_url_audience']='human_user';content['human_portal_authentication_required']=True
                                    content['agent_must_not_open_upload_url']=True;content['agent_followup_tool']='workspace.artifact.commit.plan';content['next_action']='Entregue upload_url ao usuário. Após o Portal selar o arquivo, chame workspace.artifact.commit.plan com este artifact_id.'
                    ...
                    if name=='workspace.artifact.upload.ticket':
                        ...
                        content['upload_url_audience']='human_user';content['agent_must_not_open_upload_url']=True;content['agent_followup_tool']='workspace.artifact.upload.status';content['human_portal_authentication_required']=True
