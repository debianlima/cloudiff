...
def instructions(slug,client_id,role_profile,environment):
 ...
 return {'mcp_endpoint':'https://cloudiff.duckdns.org/cloudiff/mcp','client_id':client_id,'project_slug':slug,'role_profile':role_profile,'environment':environment,'authentication':'OAuth 2.1 ou Bearer token + X-CloudIF-Client','oauth':{'issuer':'https://cloudiff.duckdns.org','authorization_url':'https://cloudiff.duckdns.org/cloudiff/mcp/oauth/authorize','token_url':'https://cloudiff.duckdns.org/cloudiff/mcp/oauth/token','revoke_url':'https://cloudiff.duckdns.org/cloudiff/mcp/oauth/revoke','client_secret':'Use a chave gerada no Portal','callbacks':{'claude':'https://claude.ai/api/mcp/auth_callback','chatgpt':'Copie a URL exata exibida pelo ChatGPT: https://chatgpt.com/connector/oauth/<callback_id>','llama_ollama':'Use a callback local do cliente, por exemplo http://127.0.0.1:<porta>/callback'}},'secret_delivery':'Disponível somente por ação autenticada de rotação/entrega; nunca exibido neste painel.','example_headers':{'Authorization':'Bearer <TOKEN_DO_PROJETO>','X-CloudIF-Client':client_id},'first_tools':tools,'production':False}
