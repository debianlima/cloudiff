...
def instructions(slug,client_id,role_profile,environment):
 ...
 return {'mcp_endpoint':'https://cloudiff.duckdns.org/cloudiff/mcp','client_id':client_id,'project_slug':slug,'role_profile':role_profile,'environment':environment,'authentication':'Bearer token + X-CloudIF-Client','secret_delivery':'Disponível somente por ação autenticada de rotação/entrega; nunca exibido neste painel.','example_headers':{'Authorization':'Bearer <TOKEN_DO_PROJETO>','X-CloudIF-Client':client_id},'first_tools':tools,'production':False}
