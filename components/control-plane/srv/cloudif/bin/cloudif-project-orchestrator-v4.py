...
def run(cmd, timeout=60):
    try:
        ...
        return {"ok": p.returncode == 0, "returncode": p.returncode, "output": p.stdout[-8000:]}
...
def ensure_supabase_tenant(tenant, username):
    ...
    return run(f"{shlex.quote(str(ensure))} {shlex.quote(tenant)} create {shlex.quote(username)}", timeout=900)
...
def main():
    ...
    add_lines(PROJECT_ACCESS / f"{project_slug}.users", [args.user] + args.allow_user)
    add_lines(PROJECT_ACCESS / f"{project_slug}.groups", args.allow_group)

    project.setdefault("steps", [])
    supa = ensure_supabase_tenant(tenant, args.user)
    project["supabase"] = supa
    project["steps"].append({"time": now(), "title": "supabase", "result": supa})
    write_json(project_file, project)

    client = BASE / "bin" / "cloudif-forja-client.py"
    if client.exists():
        forja = run(f"{shlex.quote(str(client))} project-ensure --file {shlex.quote(str(project_file))}", timeout=60)
        project["forja_client"] = forja
        project["steps"].append({"time": now(), "title": "forja", "result": forja})
        try:
            parsed=json.loads(forja.get("output","{}"))
    ...
    project["status"]="ok" if project.get("supabase",{}).get("ok") else "attention"
    project["finished_at"]=now()
    write_json(project_file, project)

    print(json.dumps({"ok": True, "project_file": str(project_file), "project": project}, ensure_ascii=False, indent=2))
    return 0
