...
def render_page(csrf_token: str, artifact_id: str = '') -> bytes:
...
(()=>{const csrf=__CSRF__,artifactId=__ARTIFACT__,legacyKey='cloudif-artifact-upload-ticket',legacyHash=decodeURIComponent(location.hash.replace(/^#/,''));if(!artifactId&&legacyHash){sessionStorage.setItem(legacyKey,legacyHash);history.replaceState(null,'',location.pathname)}const ticket=artifactId?'':(sessionStorage.getItem(legacyKey)||''),file=document.getElementById('file'),send=document.getElementById('send'),status=document.getElementById('status'),progress=document.getElementById('progress');let meta=null;const show=(text,kind='')=>{status.textContent=text;status.className='status'+(kind?' '+kind:'')};const api=async(url,opt={})=>{const r=await fetch(url,{credentials:'same-origin',...opt}),text=await r.text();let d={};try{d=JSON.parse(text)}catch(_){}if(!r.ok||!d.ok)throw new Error((d.error&&d.error.message)||d.error||('HTTP '+r.status));return d};async function load(){if(!artifactId&&!ticket){show('Link de upload incompleto. Gere um novo link pelo agente.','bad');return}try{const payload=artifactId?{artifact_id:artifactId}:{upload_ticket:ticket};const d=await api('/cloudiff/portal/api/artifact-upload/status',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify(payload)});meta=d.artifact;document.getElementById('filename').textContent=meta.filename;document.getElementById('size').textContent=new Intl.NumberFormat('pt-BR').format(meta.expected_size)+' bytes';document.getElementById('sha').textContent=meta.expected_sha256;if(meta.status==='sealed'){sessionStorage.removeItem(legacyKey);show('Artifact já está selado e pronto para o change set.','ok');return}file.disabled=false;show('Selecione exatamente o arquivo informado acima.')}catch(e){show(e.message||'Não foi possível validar o link de upload.','bad')}}file.onchange=()=>{send.disabled=true;if(!meta||!file.files.length)return;const f=file.files[0];if(f.name!==meta.filename){show('O nome do arquivo selecionado não corresponde ao esperado.','bad');return}if(f.size!==Number(meta.expected_size)){show('O tamanho do arquivo selecionado não corresponde ao esperado.','bad');return}send.disabled=false;show('Arquivo conferido por nome e tamanho. Pronto para enviar.')} ;send.onclick=async()=>{const f=file.files[0];if(!f||!meta)return;send.disabled=true;file.disabled=true;progress.hidden=false;progress.value=20;show('Enviando arquivo diretamente para a CloudIFF…');try{const headers={'Content-Type':'application/octet-stream','X-CSRF-Token':csrf};if(artifactId)headers['X-CloudIF-Artifact-Id']=artifactId;else headers['X-CloudIF-Upload-Ticket']=ticket;await api('/cloudiff/portal/api/artifact-upload/content',{method:'POST',headers,body:f});progress.value=100;sessionStorage.removeItem(legacyKey);show('Arquivo validado e artifact selado. Você pode voltar ao chat e continuar o change set.','ok')}catch(e){progress.hidden=true;file.disabled=false;send.disabled=false;show(e.message||'O upload não foi concluído.','bad')}};load()})();
</script></body></html>'''.replace('__CSRF__',csrf).replace('__ARTIFACT__',artifact)
