...
<style>
.tenant-delete-tool{{display:grid;gap:16px}}.tenant-delete-tool form{{display:grid;gap:10px;max-width:620px}}.tenant-delete-preview-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin:14px 0}}.tenant-delete-preview-grid>div{{padding:12px;border:1px solid var(--c-border,#dce3ed);border-radius:10px;background:var(--c-surface-2,#f8fafc)}}.tenant-delete-steps{{display:grid;gap:8px;padding:0;list-style:none}}.tenant-delete-steps li{{display:grid;grid-template-columns:auto 1fr;gap:8px 12px;align-items:center;padding:10px;border:1px solid var(--c-border,#dce3ed);border-radius:10px}}.tenant-delete-steps small{{grid-column:2;color:var(--c-muted,#64748b)}}
</style>
<script>
(() => {{
 ...
 async function jsonFetch(url,options={{}}){{const r=await fetch(url,{{credentials:'same-origin',...options}});const type=(r.headers.get('content-type')||'').toLowerCase();if(!type.includes('application/json'))throw new Error(`Resposta inválida do servidor (HTTP ${{r.status}})`);const data=await r.json();if(!r.ok)throw new Error(data.error||data.detail||`HTTP ${{r.status}}`);return data}}
 ...
 async function poll(id){{try{{const job=await jsonFetch(`/cloudiff/portal/api/admin-delete-tenant-status?job_id=${{encodeURIComponent(id)}}`,{{headers:{{Accept:'application/json'}}}});drawJob(job);if(job.status==='queued'||job.status==='running')setTimeout(()=>poll(id),1200)}}catch(e){{progressBox.hidden=false;progressBox.innerHTML=`<p class="pill bad">${{esc(e.message)}}</p>`}}}}
