...
ASSET_ALLOW = {"tokens.css", "base.css", "components.css", "publications.css", "app.js", "publication-release.js"}
...
def _install() -> None:
    ...
    from cloudif_portal_artifact_upload import artifact_status as artifact_upload_status, forward_upload as artifact_forward_upload, forward_upload_by_id as artifact_forward_upload_by_id, project_allowed as artifact_project_allowed, render_page as artifact_upload_page, safe_metadata as artifact_safe_metadata, ticket_status as artifact_ticket_status
    ...
    def project_terminal_page(slug: str, csrf_token: str, kind: str = '') -> bytes:
        ...
        </style></head><body data-project="{safe_slug}" data-csrf="{safe_csrf}" data-kind="{safe_kind}"><main><span class="eyebrow">Terminal do projeto</span><h1>Preparando o terminal no Komodo</h1><p>A CloudIFF validará o projeto, confirmará o container em execução e abrirá o terminal automaticamente.</p><progress></progress><ol><li class="active"><span class="dot"></span><span>Validando acesso ao projeto</span></li><li><span class="dot"></span><span>Confirmando stack e container</span></li><li><span class="dot"></span><span>Preparando o terminal</span></li></ol><div class="status" id="status" role="status">Iniciando…</div><div class="error" id="error" hidden></div><div class="actions" id="actions" hidden><button type="button" id="retry">Tentar novamente</button><a href="/cloudiff/portal/repair-dashboard">Verificar ambiente</a><a href="/cloudiff/portal/?tab=projetos">Voltar aos projetos</a></div></main><script>(()=>{{const slug=document.body.dataset.project,csrf=document.body.dataset.csrf,kind=document.body.dataset.kind,steps=[...document.querySelectorAll('ol li')],status=document.getElementById('status'),error=document.getElementById('error'),actions=document.getElementById('actions'),retry=document.getElementById('retry');let timer=0,start=0;function mark(n){{steps.forEach((step,i)=>{{step.className=i<n?'done':(i===n?'active':'')}})}}function tick(){{const sec=Math.floor((Date.now()-start)/1000);status.textContent='Preparando há '+sec+'s…';if(sec>=2)mark(1);if(sec>=5)mark(2)}}async function run(){{clearInterval(timer);start=Date.now();mark(0);error.hidden=true;actions.hidden=true;tick();timer=setInterval(tick,1000);const body=new URLSearchParams({{csrf_token:csrf,kind}});try{{const response=await fetch('/cloudiff/portal/api/projects/'+encodeURIComponent(slug)+'/terminal/prepare',{{method:'POST',credentials:'same-origin',headers:{{'Content-Type':'application/x-www-form-urlencoded','Accept':'application/json'}},body}});const data=await response.json().catch(()=>({{ok:false,error:'Resposta inválida da CloudIFF.'}}));if(!response.ok||!data.ok||!data.terminalReady||!data.terminalUrl)throw new Error(data.message||data.error||('HTTP '+response.status));clearInterval(timer);steps.forEach(x=>x.className='done');status.textContent=data.terminalSource==='base_workspace'?'Runtime da publicação indisponível. Abrindo o container-base no Komodo…':'Terminal pronto. Abrindo o Komodo…';setTimeout(()=>location.replace('https://komodoiff.duckdns.org/auth/oidc/login?redirect='+encodeURIComponent(data.terminalUrl)),450)}}catch(err){{clearInterval(timer);steps.forEach((x,i)=>{{if(x.classList.contains('active'))x.className='failed'}});status.textContent='O terminal não foi aberto.';error.hidden=false;error.textContent=String(err.message||err);actions.hidden=false}}}}retry.onclick=run;run()}})();</script></body></html>'''
        return markup.encode('utf-8')
...
<style>
.admin-operation-center{{--c-primary:#8fb8e8;--c-primary-hover:#6f9fd6;--c-primary-soft:#edf6ff;--c-primary-border:#b7d5f5;--c-accent:#8fb8e8;--c-accent-soft:#edf6ff;display:grid;gap:18px;margin:18px 0 24px!important;overflow:hidden;background:var(--surface)!important;border-color:var(--rule)!important;color:var(--ink)!important}}.admin-operation-center .btn:not(.danger):not(.red),.admin-operation-center button:not(.danger):not(.red){{--c-primary:#8fb8e8;--c-primary-hover:#6f9fd6}}.admin-eyebrow{{display:block;margin-bottom:5px;font-size:.72rem;font-weight:850;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-2)}}.admin-wizard-tabs{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}}.admin-wizard-tabs button{{display:grid;grid-template-columns:34px 1fr;gap:2px 10px;align-items:center;min-height:82px;padding:13px;border:1px solid var(--rule);border-radius:12px;background:var(--surface)!important;color:var(--ink)!important;text-align:left;box-shadow:none!important}}.admin-wizard-tabs button>span{{grid-row:1/3;width:32px;height:32px;display:grid;place-items:center;border-radius:10px;background:var(--rule-soft);color:var(--ink);font-weight:850}}.admin-wizard-tabs button strong{{font-size:.9rem}}.admin-wizard-tabs button small{{color:var(--ink-2)!important;line-height:1.25}}.admin-wizard-tabs button[aria-selected="true"]{{border-color:var(--iff)!important;background:var(--iff-wash)!important;color:var(--ink)!important}}.admin-wizard-tabs button[aria-selected="true"]>span{{background:var(--iff-wash)!important;color:var(--iff-dark)!important}}.admin-wizard-panels{{border:1px solid var(--rule);border-radius:14px;background:var(--surface);overflow:hidden}}.admin-wizard-panel{{display:none;padding:20px;background:var(--surface);color:var(--ink)}}.admin-wizard-panel.active{{display:grid;gap:16px}}.admin-panel-heading{{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}}.admin-panel-heading span{{font-size:.72rem;font-weight:850;text-transform:uppercase;color:var(--ink-2)}}.admin-panel-heading h3,.admin-panel-heading p{{margin:3px 0 0}}.admin-panel-heading p{{max-width:580px;color:var(--ink-2)!important}}.admin-tenant-actions{{display:grid;gap:14px}}.admin-tenant-actions>label{{display:grid;gap:6px;max-width:520px}}.admin-action-cards{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}}.admin-operation-center .admin-action-card{{display:grid;gap:5px;min-height:112px;padding:16px!important;border:1px solid var(--c-border,#dce3ed)!important;border-radius:12px!important;background:var(--surface)!important;color:var(--ink)!important;text-align:left!important}}.admin-operation-center .admin-action-card:hover{{border-color:var(--iff)!important;background:var(--iff-wash)!important}}.admin-operation-center .admin-action-card span{{font-weight:850}}.admin-operation-center .admin-action-card small{{color:var(--ink-2)!important;line-height:1.4}}.admin-action-status{{padding:13px 14px;border:1px solid var(--c-border,#dce3ed);border-radius:10px;background:var(--surface);color:var(--ink);white-space:pre-wrap}}.admin-action-status.running{{border-color:var(--iff);background:var(--iff-wash);color:var(--ink)}}.admin-action-status.ok{{border-color:var(--iff);background:var(--iff-wash);color:var(--ink)}}.admin-action-status.bad{{border-color:var(--halt);background:var(--halt-wash);color:var(--halt)}}.admin-wizard-panel .admin-ad-console,.admin-wizard-panel .tenant-delete-tool{{display:grid;gap:16px;min-width:0}}.admin-wizard-panel .admin-ad-console>.section-title,.admin-wizard-panel .tenant-delete-tool>.section-title{{padding-bottom:12px;border-bottom:1px solid var(--c-border,#dce3ed)}}@media(max-width:860px){{.admin-wizard-tabs,.admin-action-cards{{grid-template-columns:1fr}}.admin-wizard-tabs button{{min-height:68px}}.admin-panel-heading{{display:grid}}}}
</style>
    ...
    def global_services_body(owner, user: dict) -> str:
        ...
        for project in projects:
            ...
            container_cards.append(
                ...
                '<a class="btn light" target="_blank" rel="noopener" href="https://komodoiff.duckdns.org/auth/oidc/login?redirect=https%3A%2F%2Fkomodoiff.duckdns.org%2Fcontainers">Abrir no Komodo</a></article>'
            )
        ...
        return (
            ...
            '<nav class="global-admin-shortcuts" aria-label="Atalhos globais"><a class="btn" target="_blank" rel="noopener" href="https://komodoiff.duckdns.org/auth/oidc/login?redirect=https%3A%2F%2Fkomodoiff.duckdns.org%2Fservers">Servidores</a><a class="btn light" target="_blank" rel="noopener" href="https://komodoiff.duckdns.org/auth/oidc/login?redirect=https%3A%2F%2Fkomodoiff.duckdns.org%2Fcontainers">Todos os containers</a><a class="btn light" target="_blank" rel="noopener" href="https://cloudiff.duckdns.org/git/explore/repos">Todos os repositórios</a></nav>'
            ...
            '</section>'
            '<style>.global-admin-hub{display:grid;gap:20px}.global-admin-hero{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:24px;border:1px solid var(--rule);border-radius:18px;background:var(--surface)}.global-admin-hero h1{margin:4px 0 7px}.global-admin-hero p{margin:0;color:var(--muted)}.global-admin-kicker,.global-resource-type{font-size:.7rem;font-weight:850;letter-spacing:.08em;text-transform:uppercase;color:var(--iff-dark)}.global-admin-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.global-admin-summary article{padding:16px;border:1px solid var(--border);border-radius:13px;background:var(--surface)}.global-admin-summary small{display:block;color:var(--muted)}.global-admin-summary strong{font-size:1.8rem}.global-admin-shortcuts{display:flex;gap:8px;flex-wrap:wrap}.global-admin-section{display:grid;gap:13px}.global-resource-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:10px}.global-resource{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:16px;border:1px solid var(--border);border-radius:13px;background:var(--surface)}.global-resource h3,.global-resource p{margin:3px 0}.global-resource p{color:var(--muted)}.global-resource-actions{display:flex;gap:8px;flex-wrap:wrap}.global-tenant-list{display:grid;gap:9px}.global-tenant{border:1px solid var(--border);border-radius:13px;background:var(--surface);overflow:hidden}.global-tenant>summary{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:15px;cursor:pointer;list-style:none}.global-tenant>summary::-webkit-details-marker{display:none}.global-tenant>summary span:first-child{display:grid;gap:3px}.global-tenant>summary small{color:var(--muted)}.global-tenant-body{display:grid;gap:13px;padding:15px;border-top:1px solid var(--rule);background:var(--paper)}.global-tenant-body ul{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:7px;margin:0;padding:0;list-style:none}.global-tenant-body li{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:9px;border-radius:9px;background:var(--surface)}.backup-remote-config-form{display:grid;gap:14px;padding:18px;border:1px solid var(--border);border-radius:13px;background:var(--surface)}.backup-remote-fields{display:grid;grid-template-columns:2fr .7fr 1fr 1.5fr;gap:10px}.backup-remote-fields label,.backup-key-field,.backup-toggle{display:grid;gap:6px}.backup-key-field{grid-column:1/-1}.backup-toggle{display:flex;align-items:center;gap:8px}.backup-toggle input{min-height:auto!important;width:auto}@media(max-width:900px){.backup-remote-fields{grid-template-columns:1fr 1fr}}@media(max-width:760px){.backup-remote-fields{grid-template-columns:1fr}.global-admin-summary{grid-template-columns:repeat(2,1fr)}.global-admin-hero,.global-resource{align-items:flex-start;display:grid}}</style>'
        )

    def help_body(show_admin_link: bool = False) -> str:
        body = r"""
            ...
            try:
                ...
                if path in {'/cloudiff/portal/artifact-upload','/cloudif/portal/artifact-upload'} or artifact_upload_match:
                    try:
                        ...
                        artifact_id=artifact_upload_match.group(1) if artifact_upload_match else ''
                        if artifact_id:
                            meta=artifact_upload_status(artifact_id);slug=str(meta.get('project_slug') or '')
                            if not artifact_project_allowed(user,slug):return send(self,403,'text/html; charset=utf-8',recovery_page('Acesso negado','Você não possui acesso a este projeto.',self.path))
                        return send(self,200,'text/html; charset=utf-8',artifact_upload_page(csrf_factory(user),artifact_id))
                    except Exception as exc:
                ...
                if path in PORTAL_PATHS and tab == "admin-manutencao":
                    groups = {group.strip().lower() for group in identity(self.headers).groups}
                    if not groups.intersection({"cloudif-tenants-admin", "cloudif-professor"}):
            ...
            def do_POST(self):
                ...
                if parsed.path in {'/cloudiff/portal/api/artifact-upload/status','/cloudif/portal/api/artifact-upload/status'}:
                    try:
                        ...
                        artifact_id=str(payload.get('artifact_id') or '').strip();ticket=str(payload.get('upload_ticket') or '').strip()
                        if artifact_id:
                            meta=artifact_upload_status(artifact_id)
                        elif ticket:
                            meta=artifact_ticket_status(ticket)
                        else:
                            return send_json(self,400,{'ok':False,'error':{'message':'Informe artifact_id ou upload_ticket.'}})
                        slug=str(meta.get('project_slug') or '')
                ...
                if parsed.path in {'/cloudiff/portal/api/artifact-upload/content','/cloudif/portal/api/artifact-upload/content'}:
                    try:
                        owner=sys.modules.get(handler_class.__module__);user=self.user();provided=str(self.headers.get('X-CSRF-Token') or '');artifact_id=str(self.headers.get('X-CloudIF-Artifact-Id') or '').strip();ticket=str(self.headers.get('X-CloudIF-Upload-Ticket') or '').strip()
                        if not getattr(owner,'_prod_csrf_equal')(provided,getattr(owner,'_prod_csrf_token')(user)):
                            return send_json(self,403,{'ok':False,'error':{'message':'Token CSRF inválido.'}})
                        if artifact_id:
                            meta=artifact_upload_status(artifact_id)
                            ...
                            return send_json(self,400,{'ok':False,'error':{'message':'Link de upload incompleto.'}})
                        slug=str(meta.get('project_slug') or '')
                        if not artifact_project_allowed(user,slug):return send_json(self,403,{'ok':False,'error':{'message':'Você não possui acesso a este projeto.'}})
                        expected=int(meta.get('expected_size') or -1)
                        try:
                            status,data=artifact_forward_upload_by_id(self,artifact_id,expected) if artifact_id else artifact_forward_upload(self,ticket,expected)
                        except TypeError:return send_json(self,415,{'ok':False,'error':{'message':'Use application/octet-stream.'}})
                        except ValueError:return send_json(self,422,{'ok':False,'error':{'message':'O tamanho enviado não corresponde ao artifact.'}})
                        if status!=200 or not data.get('ok'):
                            error=data.get('error') if isinstance(data,dict) else None;message=(error.get('message') if isinstance(error,dict) else str(error or '')) or 'O arquivo não passou na validação de integridade.'
                ...
                if terminal_prepare_match:
                    ...
                    try:
                        ...
                        try: agent_payload=json.loads(exc.read() or b'{}')
                        except Exception: agent_payload={}
                        agent_code=str(agent_payload.get('error') or 'terminal_prepare_failed')
                        audit=agent_payload.get('audit') if isinstance(agent_payload.get('audit'),dict) else {}
                        stack_meta=agent_payload.get('stack_metadata') if isinstance(agent_payload.get('stack_metadata'),dict) else {}
                        missing=[str(x) for x in (audit.get('missing_files') or []) if isinstance(x,str)][:12]
                        messages={
                            'container_not_running':'O container do projeto não está em execução.',
                            'missing_compose':'A stack do projeto não possui docker-compose.yml.',
                            'stack_metadata_reconcile_failed':'A stack do projeto não possui metadados válidos para abrir o terminal.',
                            'actor_permission_sync_failed':'Não foi possível sincronizar as permissões do terminal.',
                            'actor_not_authorized':'Você não possui permissão para abrir este terminal.',
                            'terminal_create_failed':'O Komodo não conseguiu criar o terminal.',
                        }
                        if kind=='' and agent_code in {'container_not_running','missing_compose','stack_metadata_reconcile_failed'}:
                            try:
                                import cloudif_portal_publications as publications
                                fallback=publications.ensure_base_workspace(slug,user)
                                fallback_url=str(fallback.get('terminalUrl') or '').strip();fallback_target=urllib.parse.urlparse(fallback_url)
                                if fallback.get('ok') is True and fallback.get('terminalReady') is True and fallback_target.scheme=='https' and fallback_target.hostname=='komodoiff.duckdns.org' and '/terminal/' in fallback_target.path:
                                    print(f'cloudif_project_terminal_base_fallback slug={slug} from={agent_code}',flush=True)
                                    return send_json(self,200,{'ok':True,'terminalReady':True,'terminalUrl':fallback_url,'terminalSource':'base_workspace','container':str(fallback.get('container') or ''),'baseRevision':int(fallback.get('baseRevision') or 0),'message':'Runtime da publicação indisponível. Abrindo o container-base editável.','secretValuesIncluded':False,'secretReferencesIncluded':False})
                            except Exception as fallback_exc:
                                print(f'cloudif_project_terminal_base_fallback_failed slug={slug} type={type(fallback_exc).__name__}',flush=True)
                        if 'docker-compose.yml' in missing: message='A stack do projeto não possui docker-compose.yml.'
                        else: message=messages.get(agent_code,'O Komodo não conseguiu preparar o terminal deste projeto.')
                        print(f'cloudif_project_terminal_prepare_rejected slug={slug} status={exc.code} code={agent_code}',flush=True)
                        return send_json(self,exc.code if 400<=exc.code<500 else 503,{'ok':False,'error':agent_code,'message':message,'terminalReady':False,'missingFiles':missing,'runtimeRunning':bool(audit.get('running')),'stackMetadataError':str(stack_meta.get('error') or ''),'secretValuesIncluded':False,'secretReferencesIncluded':False})
                    except Exception as exc:
                        print(f'cloudif_project_terminal_prepare_failed slug={slug} type={type(exc).__name__}',flush=True)
                        return send_json(self,503,{'ok':False,'error':'terminal_prepare_failed','message':'Não foi possível concluir a preparação do terminal.','terminalReady':False,'secretValuesIncluded':False,'secretReferencesIncluded':False})
                ...
                if value("async") == "1":
                    try:
                        ...
                        if not getattr(owner,"_admin_project_delete_allowed")(user,slug):
                            return send_json(self, 403, {"ok": False, "error": "forbidden"})
                        from cloudif_admin_project_delete import confirmation_matches,consume_wizard_token,start_job
                        confirmation=value("confirm_text")
                        if not confirmation_matches(slug,confirmation):
                            return send_json(self, 409, {"ok": False, "error": "invalid_confirmation", "expected": "EXCLUIR "+slug})
                        if not consume_wizard_token(slug,value("wizard_token")):
                            return send_json(self, 409, {"ok": False, "error": "wizard_required"})
                        job = start_job(slug, confirmation, actor.username or "admin")
                        return send_json(self, 202 if job.get("ok") else 409, job)
                    except Exception as exc:
                        return send_json(self, 500, {"ok": False, "error": type(exc).__name__, "detail": str(exc)[:300]})
