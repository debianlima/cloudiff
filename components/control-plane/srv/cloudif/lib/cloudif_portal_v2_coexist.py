...
<section class="card platform-guide">
  ...
  <nav class="guide-index" aria-label="Seções do guia"><a href="#guia-runtime">Ambiente do projeto</a><a href="#guia-publicacoes">Publicações</a><a href="#guia-projetos">Projetos</a><a href="#guia-bancos">Bancos</a><a href="#guia-backup">Backup</a><a href="#guia-conectores">Conectores</a><a href="#guia-ad">Administração</a><a href="#guia-grupos">Grupos</a><a href="#guia-regras">Regras</a></nav>
  <div class="guide-grid">
    <article id="guia-runtime"><span>00</span><h2>Ambiente do projeto</h2><p>Todo projeto novo recebe um container isolado com Apache, PHP e Node.js. O usuário trabalha em <code>site/</code>; a plataforma mantém <code>.cloudif/</code>.</p><ul><li><strong>PHP:</strong> arquivos em <code>site/</code> são interpretados pelo Apache.</li><li><strong>Node:</strong> APIs ficam em <code>site/api/</code> e são publicadas em <code>/api/</code>.</li><li><strong>HTTPS:</strong> portas públicas 80 e 443 ficam no proxy; o container atende internamente na porta 80.</li><li><strong>Infraestrutura:</strong> Compose, Dockerfile, Apache, Supervisor e healthcheck ficam em <code>.cloudif/</code>.</li></ul><p>É permitido substituir todo o conteúdo de <code>site/</code>. Não apague <code>.cloudif/</code> sem fornecer configuração equivalente.</p></article>
...
"""
    ...
    def backup_inventory(owner, user: dict) -> dict:
        ...
        return {
            ...
            "platform": {"items": _platform_backup_items(), "can_manage": bool(user.get("admin"))},
            "tenants": {"items": _tenant_backup_items(), "can_download": bool(user.get("admin")), "schedule": _backup_schedule("cloudif-tenant-db-backup-v2.timer", "Backup dos bancos e tenants")},
            "schedules": [
...
<div class="backup-progress-layer" id="backup-progress-layer" hidden>
  <section class="backup-progress-modal" role="dialog" aria-modal="true" aria-labelledby="backup-progress-title">
    <header><div><small>BACKUP DO PROJETO</small><h2 id="backup-progress-title">Preparando backup</h2><p id="backup-progress-subtitle">Aguarde enquanto a plataforma acompanha a geração do arquivo.</p></div><button class="btn light" id="backup-progress-close" type="button" hidden>Fechar</button></header>
