...
def main():
 ...
 if marker.exists():
  try:
   ...
   if old_marker.get('kind')==kind and old_marker.get('runtime_template')==runtime and old_marker.get('version')==3:
 ...
 marker.write_text(json.dumps({'kind':kind,'runtime_template':runtime,'version':3,'project':slug,'public_number':num,'applied_at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'files':results},ensure_ascii=False,indent=2)+'\n')
