...
def main():
 job=json.loads(Path(sys.argv[1]).read_text()); kind=job.get('template_kind','none')
 ...
 if marker.exists():
  try:
   ...
   if old_marker.get('kind')==kind and old_marker.get('version')==2:
 ...
 results=[]
 for path,content in build(kind,slug,owner,tenant,num):
 ...
 marker.write_text(json.dumps({'kind':kind,'version':2,'project':slug,'public_number':num,'applied_at':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'files':results},ensure_ascii=False,indent=2)+'\n')
