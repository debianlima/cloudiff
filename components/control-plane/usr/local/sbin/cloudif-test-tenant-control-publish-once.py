...
h={'X-authentik-username':user,'X-authentik-email':'iff1742962@laboratorios.bomjesus.iff.edu.br','X-authentik-groups':groups,'Host':'cloudiff.duckdns.org','X-Forwarded-Host':'cloudiff.duckdns.org','Origin':'https://cloudiff.duckdns.org'}
html=urllib.request.urlopen(urllib.request.Request(base+'/cloudiff/portal/?tab=projetos',headers=h),timeout=40).read().decode()
...
try:r=urllib.request.build_opener(NR).open(req,timeout=800);code=r.status;loc=r.headers.get('Location')
...
out={'ok':True,'username':user,'groups':groups.split(','),'status':code,'location':loc,'tenant_admin':True,'professor':True,'secrets_exposed':False}
open('/var/lib/cloudif/portal/tenant-control-publish-result.json','w').write(json.dumps(out,ensure_ascii=False,indent=2)+'\n')
