...
grep -q 'certificate revoked' /tmp/cloudif-dr-old-forja.log
grep -q 'certificate revoked' /tmp/cloudif-dr-old-mauricio.log
...
PY
python3 -m py_compile ./usr/local/sbin/cloudif-machine-controller.py ./usr/local/sbin/cloudif-machine-harvester.py ./usr/local/sbin/cloudif-machine-guardian.py ./usr/local/sbin/cloudif-machine-executor.py ./usr/local/sbin/cloudif-agent-pki.py ./usr/local/sbin/cloudif-certificate-alert-dispatcher.py
...
python3 - "$LATEST" "$MANIFEST" <<'PY'
import json,sys,os
archive,manifest=sys.argv[1:]
d=json.load(open(manifest))
assert d['scope']=='machine-admin-control-plane-and-pki'
assert {'portal database','tenant directories','user data'} <= set(d['excludes'])
assert d['latest_postgres_backup'] and d['latest_postgres_sha256']
print({'archive':archive,'size':os.path.getsize(archive),'file_entries':d['file_entries'],'restore_validation':'ok'})
PY
MARKER=$BASE/last-restore-validation.json
python3 - "$MARKER" "$LATEST" <<'PY'
import json,sys,datetime as dt,hashlib,os
out,archive=sys.argv[1:]
data={'validated_at':dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat().replace('+00:00','Z'),'archive':archive,'archive_sha256':hashlib.sha256(open(archive,'rb').read()).hexdigest(),'result':'ok'}
open(out,'w').write(json.dumps(data,indent=2)+'\n')
os.chmod(out,0o600)
PY
printf 'restore_validation=ok archive=%s internal_checksums=%s marker=%s\n' "$LATEST" "$(grep -c ': OK$' /tmp/cloudif-dr-internal-sha.log)" "$MARKER"
