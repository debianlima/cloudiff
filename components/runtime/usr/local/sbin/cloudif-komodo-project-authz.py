...
def main():
    ...
    if not project or not owner or not stack_id or not repo_id:
    ...
    data={'project':project,'owner':owner,'stack_id':stack_id,'repo_id':repo_id,'desired':desired,'previous':previous}
    js=r'''
...
for(const item of p.desired){
 ...
 for(const resource of [{type:'Stack',id:p.stack_id},{type:'Repo',id:p.repo_id}]){
...
'''
    ...
    try:
        ...
        result=json.loads(marker.split('=',1)[1]);result.update({'project':project,'owner':owner,'stack_id':stack_id,'repo_id':repo_id})
