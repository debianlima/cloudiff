...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p.path=='/v1/hosts':
   rows=q("select host,sum(requests) requests,count(distinct visitor_hash) unique_visitors,sum(bytes) bytes,sum(case when status_class like '4%' or status_class like '5%' then requests else 0 end) errors,sum(case when source='public' then requests else 0 end) public_requests,sum(case when source='internal' then requests else 0 end) internal_requests,sum(case when source='public' and (status_class like '4%' or status_class like '5%') then requests else 0 end) public_errors,sum(case when source='internal' and (status_class like '4%' or status_class like '5%') then requests else 0 end) internal_errors,count(distinct case when source='public' then visitor_hash end) public_unique_visitors,count(distinct case when source='internal' then visitor_hash end) internal_unique_visitors,max(last_seen) last_seen from access_daily where day>=date('now',?) group by host order by requests desc",(f'-{days-1} days',));self.out(200,{'ok':True,'days':days,'hosts':rows});return
  if p.path=='/v1/routes':
   host=(parse_qs(p.query).get('host') or [''])[0];rows=q("select host,route,sum(requests) requests,count(distinct visitor_hash) unique_visitors,sum(bytes) bytes,sum(case when source='public' then requests else 0 end) public_requests,sum(case when source='internal' then requests else 0 end) internal_requests,sum(case when source='public' and (status_class like '4%' or status_class like '5%') then requests else 0 end) public_errors,sum(case when source='internal' and (status_class like '4%' or status_class like '5%') then requests else 0 end) internal_errors,count(distinct case when source='public' then visitor_hash end) public_unique_visitors,max(last_seen) last_seen from access_daily where day>=date('now',?) and (?='' or host=?) group by host,route order by requests desc limit 200",(f'-{days-1} days',host,host));self.out(200,{'ok':True,'days':days,'routes':rows});return
